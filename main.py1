...
def handle_media_common(message,bot_obj,bot_token,bot_index=0):
    ...
    try:
        if not PROCESSING_SEMAPHORE.acquire(blocking=False):
            bot_obj.send_message(message.chat.id,f"⚠️ The server is under load, try again after {RETRY_AFTER_MINUTES} minutes.",reply_to_message_id=message.message_id)
            return
        acquired=True
...
def upload_large_file(token):
    ...
    def process_uploaded_file(chat_id_inner,lang_inner,path,bot_index_inner):
        ...
        try:
            if not PROCESSING_SEMAPHORE.acquire(blocking=False):
                try: bot_to_use.send_message(chat_id_inner,f"⚠️ The server is under load, try again after {RETRY_AFTER_MINUTES} minutes.")
                except: pass
                try: os.remove(path)
                except: pass
                return
            ...
            try:
                if acquired: PROCESSING_SEMAPHORE.release()
            except: pass
            try: os.remove(path)
            except: pass
