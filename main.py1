...
def split_wav_to_chunks_with_overlap(wav_path: str, out_dir: str, chunk_duration_sec: int, overlap_sec: float):
    if FFMPEG_BINARY is None: raise RuntimeError("ffmpeg binary not found")
    ...
    if step <= 0:
        raise ValueError("chunk_duration_sec must be greater than overlap_sec")
    ...
    for i in range(num):
        ...
        if os.path.exists(out_name) and os.path.getsize(out_name) > 100:
