...
POSSIBLE_FFMPEG_PATHS = [os.environ.get("FFMPEG_BINARY", ""), "./ffmpeg", "/usr/bin/ffmpeg", "/usr/local/bin/ffmpeg", "ffmpeg"]
FFMPEG_BINARY = None
...
def update_user_activity(user_id: int):
    try:
        now = datetime.now()
        if client:
            users_collection.update_one({"_id": str(user_id)}, {"$set": {"last_active": now}, "$setOnInsert": {"first_seen": now, "stt_conversion_count": 0}}, upsert=True)
    except Exception:
        pass
...
def split_wav_to_chunks(wav_path: str, out_dir: str, chunk_duration_sec: int):
    ...
    cmd = [FFMPEG_BINARY, "-y", "-i", wav_path, "-ar", str(AUDIO_SAMPLE_RATE), "-ac", str(AUDIO_CHANNELS), "-f", "segment", "-segment_time", str(chunk_duration_sec), "-reset_timestamps", "1", pattern]
    subprocess.run(cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=True)
...
def prepend_silence_to_wav(original_wav: str, output_wav: str, silence_sec: int):
    ...
    cmd_create_silence = [FFMPEG_BINARY, "-y", "-f", "lavfi", "-i", f"anullsrc=channel_layout=mono:sample_rate={AUDIO_SAMPLE_RATE}", "-t", str(silence_sec), "-ar", str(AUDIO_SAMPLE_RATE), "-ac", str(AUDIO_CHANNELS), silence_file]
    subprocess.run(cmd_create_silence, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=True)
    cmd_concat = [FFMPEG_BINARY, "-y", "-i", silence_file, "-i", original_wav, "-filter_complex", "[0:0][1:0]concat=n=2:v=0:a=1[out]", "-map", "[out]", output_wav]
...
def transcribe_with_assemblyai(file_path: str, language_code: str, timeout_seconds: int = REQUEST_TIMEOUT_ASSEMBLY):
    ...
    with open(file_path, "rb") as f:
        try:
            ...
            resp.raise_for_status()
            j = resp.json()
            upload_url = j.get("upload_url") or j.get("url") or None
            if not upload_url:
...
def send_subscription_message(chat_id: int, bot_obj):
    ...
    try:
        ...
        markup.add(InlineKeyboardButton("Click here to join the Channel", url=f"https://t.me/{REQUIRED_CHANNEL.lstrip('@')}"))
        bot_obj.send_message(chat_id, "🔒 Access Locked You cannot use this bot until you join the Channel.", reply_markup=markup)
    except Exception:
        pass

def ensure_subscription_for_user(user_id: int, chat_id: int, bot_obj) -> bool:
    try:
        if not check_subscription(user_id, bot_obj):
            try:
                send_subscription_message(chat_id, bot_obj)
            except Exception:
                pass
            return False
        return True
...
<body>
    <div class="container">
        ...
        <div class="file-upload-wrapper">
            <input type="file" id="file-input" class="file-upload-input" accept=".mp3,.wav,.m4a,.ogg,.webm,.flac,.mp4,.mkv,.avi,.mov,.aac,.aiff,.amr,.wma,.opus,.m4v,.ts,.flv,.3gp">
...
def upload_large_file(token):
    ...
    def process_uploaded_file(chat_id_inner, lang_inner, path, bot_index_inner):
        try:
            ...
            if not ensure_subscription_for_user(chat_id_inner, chat_id_inner, bot_to_use):
                try:
                    bot_to_use.send_message(chat_id_inner, "🔒 You are not a member of the required channel. Please join to receive transcripts.")
                except Exception:
                    pass
                return
            try:
            ...
            try:
                ...
                threading.Thread(target=lambda: time.sleep(86400) or user_transcriptions.get(uid_key, {}).pop(sent_msg.message_id, None), daemon=True).start()
...
def call_gemini_with_retries(text: str, instruction: str, retries: int = 3, timeout: int = REQUEST_TIMEOUT_GEMINI):
    last_exc = None
    for attempt in range(1, retries + 1):
        try:
            return ask_gemini(text, instruction, timeout=timeout)
        except Exception as e:
            last_exc = e
...
def register_handlers(bot_obj, bot_token, bot_index):
    ...
    def start_handler(message):
        try:
            ...
            if message.chat.type == 'private' and not ensure_subscription_for_user(message.from_user.id, message.chat.id, bot_obj):
                return
    ...
    def start_select_lang_callback(call):
        try:
            if call.message.chat.type == 'private' and not ensure_subscription_for_user(call.from_user.id, call.message.chat.id, bot_obj):
                try:
    ...
    def handle_text_messages(message):
        try:
            if message.chat.type == 'private' and not ensure_subscription_for_user(message.from_user.id, message.chat.id, bot_obj):
                return
            bot_obj.send_message(message.chat.id, "For Text to Audio Use another bot")
        except Exception:
    ...
    def get_key_points_callback(call):
        try:
            ...
            try:
                ...
                try:
                    summary = call_gemini_with_retries(stored, instruction, retries=3)
                except Exception:
    ...
    def clean_up_callback(call):
        try:
            ...
            try:
                ...
                try:
                    cleaned = call_gemini_with_retries(stored, instruction, retries=3)
                except Exception:
            ...
            if len(cleaned) > 4000:
                if user_mode == "file":
                    ...
                    try:
                        ...
                        threading.Thread(target=lambda: time.sleep(86400) or user_transcriptions.get(uid_key, {}).pop(sent.message_id, None), daemon=True).start()
                    except Exception:
                        pass
                else:
                    try:
                    ...
                    try:
                        ...
                        threading.Thread(target=lambda: time.sleep(86400) or user_transcriptions.get(uid_key, {}).pop(last_sent.message_id, None), daemon=True).start()
                    except Exception:
                        pass
            else:
                try:
                    ...
                    threading.Thread(target=lambda: time.sleep(86400) or user_transcriptions.get(uid_key, {}).pop(status_msg.message_id, None), daemon=True).start()
