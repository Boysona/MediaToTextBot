...
def update_user_activity(user_id: int, bot_index: int):
    user_id_str = str(user_id)
    query = {"user_id": user_id_str, "bot_index": bot_index}
    now = datetime.now()
    users_collection.update_one(query, {"$set": {"last_active": now}, "$setOnInsert": {"first_seen": now, "stt_conversion_count": 0, "bot_index": bot_index}}, upsert=True)

def increment_processing_count(user_id: str, service_type: str, bot_index: int):
    users_collection.update_one({"user_id": str(user_id), "bot_index": bot_index}, {"$inc": {f"{service_type}_conversion_count": 1}})

def get_stt_user_lang(user_id: str, bot_index: int) -> str:
    user_data = users_collection.find_one({"user_id": user_id, "bot_index": bot_index})
    return user_data.get("stt_language", "en") if user_data else "en"

def set_stt_user_lang(user_id: str, lang_code: str, bot_index: int):
    users_collection.update_one({"user_id": user_id, "bot_index": bot_index}, {"$set": {"stt_language": lang_code}}, upsert=True)

def get_user_send_mode(user_id: str, bot_index: int) -> str:
    user_data = users_collection.find_one({"user_id": user_id, "bot_index": bot_index})
    return user_data.get("stt_send_mode", "file") if user_data else "file"

def set_user_send_mode(user_id: str, mode: str, bot_index: int):
    if mode not in ("file", "split"): mode = "file"
    users_collection.update_one({"user_id": user_id, "bot_index": bot_index}, {"$set": {"stt_send_mode": mode}}, upsert=True)
...
def admin_send_ads():
    ...
    bot_index_str = request.form.get("bot_index")
    bot_index_filter = None
    if bot_index_str and bot_index_str.strip():
        try: bot_index_filter = int(bot_index_str.strip())
        except Exception: pass
    ...
    if file:
        ...
        try:
            ...
            tmp_path = None
    t = threading.Thread(target=broadcast_worker, args=(target, msg_type, message_text, tmp_path, bot_index_filter), daemon=True)
    t.start()
...
def register_handlers(bot_obj, bot_token, bot_index):
    ...
    def handle_admin(message):
        try:
            ...
            query_filter = {"bot_index": bot_index}
            total_users = users_collection.count_documents(query_filter)
            total_groups = groups_collection.count_documents(query_filter)
            total_stt_conversions = sum(user.get("stt_conversion_count", 0) for user in users_collection.find(query_filter))
            seven_days_ago = datetime.now() - timedelta(days=7)
            active_users_7d = users_collection.count_documents({"last_active": {"$gte": seven_days_ago}, "bot_index": bot_index})
            report_text = f"<b>📊 Bot Admin Panel (Bot Index: {bot_index})</b>\n\n👤 Total Users: <b>{total_users}</b>\n👥 Total Groups: <b>{total_groups}</b>\n🗣️ Total Transcriptions: <b>{total_stt_conversions:,}</b>\n🟢 Active Users (Last 7 Days): <b>{active_users_7d}</b>\n\n"
    ...
    def handle_new_chat_members(message):
        try:
            if message.new_chat_members[0].id == bot_obj.get_me().id:
                group_data = {'_id': str(message.chat.id), 'title': message.chat.title, 'type': message.chat.type, 'added_date': datetime.now(), 'bot_index': bot_index}
    ...
    def handle_left_chat_member(message):
        try:
            if message.left_chat_member.id == bot_obj.get_me().id: 
                groups_collection.delete_one({'_id': str(message.chat.id), 'bot_index': bot_index})
