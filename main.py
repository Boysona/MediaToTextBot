...
DBS = []
for idx in range(max(1, len(BOT_TOKENS))):
    db_name_for_bot = f"{DB_NAME}_bot{idx}"
    DBS.append(client[db_name_for_bot])
COLS_BY_BOT = []
for db in DBS:
    COLS_BY_BOT.append({"users": db["users"], "groups": db["groups"], "settings": db["settings"]})
...
def update_user_activity(user_id: int, bot_index: int = 0):
    try:
        users_col = COLS_BY_BOT[bot_index]["users"]
    except Exception:
        users_col = COLS_BY_BOT[0]["users"]
    user_id_str = str(user_id)
    now = datetime.now()
    users_col.update_one({"user_id": user_id_str}, {"$set": {"last_active": now}, "$setOnInsert": {"first_seen": now, "stt_conversion_count": 0}}, upsert=True)

def increment_processing_count(user_id: str, service_type: str, bot_index: int = 0):
    try:
        users_col = COLS_BY_BOT[bot_index]["users"]
    except Exception:
        users_col = COLS_BY_BOT[0]["users"]
    users_col.update_one({"user_id": str(user_id)}, {"$inc": {f"{service_type}_conversion_count": 1}})

def get_stt_user_lang(user_id: str, bot_index: int = 0) -> str:
    try:
        users_col = COLS_BY_BOT[bot_index]["users"]
    except Exception:
        users_col = COLS_BY_BOT[0]["users"]
...
def set_stt_user_lang(user_id: str, lang_code: str, bot_index: int = 0):
    try:
        users_col = COLS_BY_BOT[bot_index]["users"]
    except Exception:
        users_col = COLS_BY_BOT[0]["users"]
    users_col.update_one({"user_id": user_id}, {"$set": {"stt_language": lang_code}}, upsert=True)

def get_user_send_mode(user_id: str, bot_index: int = 0) -> str:
    try:
        users_col = COLS_BY_BOT[bot_index]["users"]
    except Exception:
        users_col = COLS_BY_BOT[0]["users"]
...
def set_user_send_mode(user_id: str, mode: str, bot_index: int = 0):
    ...
    try:
        ...
        users_col = COLS_BY_BOT[0]["users"]
    users_col.update_one({"user_id": user_id}, {"$set": {"stt_send_mode": mode}}, upsert=True)
...
def build_lang_keyboard(callback_prefix: str, row_width: int = 3, message_id: int = None):
    ...
    buttons = [telebot.types.InlineKeyboardButton(label, callback_data=f"{callback_prefix}|{code}|{message_id}" if message_id else f"{callback_prefix}|{code}") for label, code in LANG_OPTIONS]
...
def build_result_mode_keyboard(prefix: str = "result_mode"):
    ...
    markup.add(telebot.types.InlineKeyboardButton("📄 .txt file", callback_data=f"{prefix}|file"), telebot.types.InlineKeyboardButton("💬 Split messages", callback_data=f"{prefix}|split"))
    return markup
...
def transcribe_file_with_speech_recognition(input_file_path: str, language_code: str):
    ...
    try:
        ...
        def transcribe_chunk(chunk_path):
            return recognize_chunk_file(sr.Recognizer(), chunk_path, language_code)
...
def transcribe_via_selected_service(input_path: str, lang_code: str):
    ...
    if use_assembly:
        try:
            ...
            try:
                return transcribe_file_with_speech_recognition(input_path, lang_code), "speech_recognition"
        ...
        try:
            ...
            try:
                return transcribe_with_assemblyai(input_path, lang_code), "assemblyai"
            except Exception as e2:
...
def attach_action_buttons(bot_obj, chat_id, message_id, text):
    try:
        ...
        markup = telebot.types.InlineKeyboardMarkup()
        markup.add(telebot.types.InlineKeyboardButton("⭐️Clean transcript", callback_data=f"clean_up|{chat_id}|{message_id}"))
        if include_summarize:
            markup.add(telebot.types.InlineKeyboardButton("Get Summarize", callback_data=f"get_key_points|{chat_id}|{message_id}"))
        try:
...
def handle_media_common(message, bot_obj, bot_token, bot_index=0):
    ...
    update_user_activity(message.from_user.id, bot_index)
    ...
    lang = get_stt_user_lang(user_id_str, bot_index)
        ...
        try:
            ...
            try:
                ...
                if "ffmpeg" in error_msg.lower():
                    bot_obj.send_message(message.chat.id, "⚠️ Server error: ffmpeg not found or conversion failed. Contact admin @laki343.", reply_to_message_id=message.message_id)
            ...
            user_mode = get_user_send_mode(uid_key, bot_index)
            if len(corrected_text) > 4000:
                ...
                try:
                    ...
                    pass
            increment_processing_count(user_id_str, "stt", bot_index)
        finally:
            try:
                os.remove(tmpf.name)
            except Exception:
...
def broadcast_worker(target_type, msg_type, text, file_path, bot_index):
    ...
    try:
        if target_type in ("all", "users"):
            for u in COLS_BY_BOT[bot_index]["users"].find({}, {"_id": 1}):
                try:
                    recipients.append(int(u["_id"]))
                except Exception:
                    try:
        ...
        if target_type in ("all", "groups"):
            for g in COLS_BY_BOT[bot_index]["groups"].find({}, {"_id": 1}):
                try:
...
def admin_ui():
    ...
    if not secret or secret != ADMIN_PANEL_SECRET:
        return abort(403)
    try:
        users_count = sum(COLS_BY_BOT[i]["users"].count_documents({}) for i in range(len(COLS_BY_BOT)))
    except Exception:
        users_count = 0
    try:
        groups_count = sum(COLS_BY_BOT[i]["groups"].count_documents({}) for i in range(len(COLS_BY_BOT)))
    except Exception:
        groups_count = 0
...
def admin_save_settings():
    ...
    if new_secret:
        try:
            for dbcols in COLS_BY_BOT:
                dbcols["settings"].update_one({"_id": "admin_panel_secret"}, {"$set": {"value": new_secret}}, upsert=True)
    ...
    def process_uploaded_file(chat_id_inner, lang_inner, path, bot_index_inner):
        try:
            ...
            try:
                ...
                user_mode = get_user_send_mode(uid_key, bot_index_inner)
                if len(corrected_text) > 4000:
                    if user_mode == "file":
                        fobj = io.BytesIO(corrected_text.encode("utf-8"))
                        fobj.name = "transcription.txt"
                        sent_msg = bot_to_use.send_document(chat_id_inner, fobj, reply_markup=telebot.types.InlineKeyboardMarkup())
                    else:
                        chunks = split_text_into_chunks(corrected_text, limit=4096)
                        last_sent = None
                        for idx, chunk in enumerate(chunks):
                    ...
                    sent_msg = bot_to_use.send_message(chat_id_inner, corrected_text or "⚠️ Warning Make sure the voice is clear or speaking in the language you Choosed.", reply_markup=telebot.types.InlineKeyboardMarkup())
                try:
            ...
            try:
                ...
                user_transcriptions.setdefault(uid_key, {})[sent_msg.message_id] = corrected_text
                threading.Thread(target=delete_transcription_later, args=(uid_key, sent_msg.message_id), daemon=True).start()
                increment_processing_count(str(chat_id_inner), "stt", bot_index_inner)
            except Exception:
                pass
        finally:
            try:
...
def assemblyai_endpoint():
    ...
    try:
        ...
        tmp.close()
        try:
            return jsonify({"text": transcribe_with_assemblyai(tmp.name, lang)}), 200
        except Exception as e:
            try:
                return jsonify({"text": transcribe_file_with_speech_recognition(tmp.name, lang), "fallback": "speech_recognition"}), 200
            except Exception as e2:
...
def register_handlers(bot_obj, bot_token, bot_index):
    ...
    def start_select_lang_callback(call):
        try:
            ...
            set_stt_user_lang(uid, lang_code, bot_index)
            try:
    ...
    def handle_mode(message):
        try:
            if message.chat.type == 'private' and str(message.from_user.id) not in ADMIN_USER_IDS and not check_subscription(message.from_user.id, bot_obj):
                send_subscription_message(message.chat.id, bot_obj)
                return
            current_mode = get_user_send_mode(str(message.from_user.id), bot_index)
    ...
    def handle_admin(message):
        try:
            if message.from_user.id not in ADMIN_USER_IDS:
                ...
                return
            total_users = sum(COLS_BY_BOT[i]["users"].count_documents({}) for i in range(len(COLS_BY_BOT)))
            total_groups = sum(COLS_BY_BOT[i]["groups"].count_documents({}) for i in range(len(COLS_BY_BOT)))
            total_stt_conversions = 0
            for i in range(len(COLS_BY_BOT)):
                for user in COLS_BY_BOT[i]["users"].find({}):
                    total_stt_conversions += user.get("stt_conversion_count", 0) if isinstance(user.get("stt_conversion_count", 0), int) else 0
            seven_days_ago = datetime.now() - timedelta(days=7)
            active_users_7d = sum(COLS_BY_BOT[i]["users"].count_documents({"last_active": {"$gte": seven_days_ago}}) for i in range(len(COLS_BY_BOT)))
            ...
            markup = telebot.types.InlineKeyboardMarkup()
            markup.add(telebot.types.InlineKeyboardButton("Open Admin Panel", url=link))
    ...
    def on_result_mode_select(call):
        try:
            ...
            set_user_send_mode(uid, mode, bot_index)
    ...
    def handle_new_chat_members(message):
        try:
            if message.new_chat_members[0].id == bot_obj.get_me().id:
                ...
                COLS_BY_BOT[bot_index]["groups"].update_one({'_id': group_data['_id']}, {'$set': group_data}, upsert=True)
    ...
    def handle_left_chat_member(message):
        try:
            if message.left_chat_member.id == bot_obj.get_me().id:
                COLS_BY_BOT[bot_index]["groups"].delete_one({'_id': str(message.chat.id)})
        except Exception:
    ...
    def get_key_points_callback(call):
        try:
            ...
            animation_thread.start()
            try:
                lang = get_stt_user_lang(str(chat_id_val), bot_index) or "en"
...
for idx, bot_obj in enumerate(bots):
    db_for_this_bot = DBS[idx] if idx < len(DBS) else DBS[0]
