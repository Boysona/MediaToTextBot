...
def check_subscription(user_id:int, bot_obj)->bool:
    ...
    except telebot.apihelper.ApiTelegramException: return False
...
def register_handlers(bot_obj, bot_token, bot_index):
    ...
    def get_key_points_callback(call):
        try:
            if call.message.chat.type=='private' and not check_subscription(call.message.chat.id, bot_obj): send_subscription_message(call.message.chat.id, bot_obj); try: bot_obj.answer_callback_query(call.id)
    ...
    def clean_up_callback(call):
        try:
            if call.message.chat.type=='private' and not check_subscription(call.from_user.id, bot_obj): send_subscription_message(call.message.chat.id, bot_obj); try: bot_obj.answer_callback_query(call.id)
            except Exception: pass; return
            parts=call.data.split("|")
            if len(parts)==3: _,chat_id_part,msg_id_part=parts
            elif len(parts)==2: _,msg_id_part=parts; chat_id_part=str(call.message.chat.id)
