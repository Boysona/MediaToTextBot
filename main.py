...
GEMINI_API_KEY = os.environ.get("GEMINI_API_KEY", "AIzaSyBRWP4BaPLPpYdB5_E3C3TVDGqiHrjv4gQ")
...
ADMIN_SECRET = os.environ.get("ADMIN_SECRET", "waaadminkadhabtaa")
...
in_memory_data = {"pending_media": {}, "admin_pending_auth": {}, "admin_sessions": {}, "admin_broadcast_pending": {}}
action_usage = {}
            ...
            if len(corrected_text) > 4000:
                if user_mode == "file":
                    ...
                    try:
                        ...
                        pass
                    try:
                        action_usage[f"{message.chat.id}|{sent.message_id}|clean_up"] = 0
                        action_usage[f"{message.chat.id}|{sent.message_id}|get_key_points"] = 0
                    except Exception:
                        pass
                else:
                    ...
                    try:
                        ...
                        pass
                    try:
                        action_usage[f"{message.chat.id}|{last_sent.message_id}|clean_up"] = 0
                        action_usage[f"{message.chat.id}|{last_sent.message_id}|get_key_points"] = 0
                    except Exception:
                        pass
            else:
                markup = InlineKeyboardMarkup()
                ...
                try:
                    ...
                    pass
                try:
                    action_usage[f"{message.chat.id}|{sent_msg.message_id}|clean_up"] = 0
                    action_usage[f"{message.chat.id}|{sent_msg.message_id}|get_key_points"] = 0
                except Exception:
                    pass
...
def upload_large_file(token):
    ...
    def process_uploaded_file(chat_id_inner, lang_inner, path, bot_index_inner):
        try:
            ...
            try:
                text = transcribe_via_selected_service(path, lang_inner)[0]
            except Exception:
                try:
...
def build_admin_panel_markup():
    markup = InlineKeyboardMarkup(row_width=2)
    markup.add(InlineKeyboardButton("📣 Broadcast", callback_data="adminpanel|broadcast"))
    markup.add(InlineKeyboardButton("👥 Recent users (3d)", callback_data="adminpanel|recent"))
    markup.add(InlineKeyboardButton("❌ Close admin", callback_data="adminpanel|close"))
    return markup
...
def open_admin_panel(bot_obj, chat_id, user):
    total_users = users_collection.count_documents({})
    agg = list(users_collection.aggregate([{"$group": {"_id": None, "total": {"$sum": {"$ifNull": ["$stt_conversion_count", 0]}}}}]))
    total_files = agg[0]["total"] if agg else 0
    uptime = format_timedelta(datetime.now() - START_TIME)
    three_days_ago = datetime.now() - timedelta(days=3)
    recent = list(users_collection.find({"last_active": {"$gte": three_days_ago}, "stt_conversion_count": {"$gt": 0}}).sort("last_active", -1))
    recent_lines = []
    for r in recent:
        uid = r.get("_id")
        last = r.get("last_active")
        cnt = r.get("stt_conversion_count", 0)
        last_str = last.strftime("%Y-%m-%d %H:%M:%S") if isinstance(last, datetime) else str(last)
        recent_lines.append(f"{uid}  files:{cnt}  last:{last_str}")
    recent_text = "\n".join(recent_lines) if recent_lines else "No recent file processors in last 3 days"
    text = f"Admin Panel\n\nTotal users: {total_users}\nFiles processed: {total_files}\nUptime: {uptime}\n\nRecent users (last 3 days):\n{recent_text}\n\nYou can broadcast text, voice, audio, video or documents to all groups where the bot is admin and to users."
    sent = bot_obj.send_message(chat_id, text, reply_markup=build_admin_panel_markup())
    with memory_lock:
        in_memory_data["admin_sessions"][str(user.id)] = {"chat_id": chat_id, "message_id": sent.message_id}
    return sent

def close_admin_panel(bot_obj, user_id):
    sess = None
    with memory_lock:
        sess = in_memory_data["admin_sessions"].pop(str(user_id), None)
    if sess:
        try:
            m = bot_obj.send_message(sess["chat_id"], "Admin panel closed. This message will be deleted in 10 seconds.")
            def delayed_delete(chat_id, msg_id, bot):
                time.sleep(10)
                try:
                    bot.delete_message(chat_id, msg_id)
                except Exception:
                    pass
            threading.Thread(target=delayed_delete, args=(sess["chat_id"], m.message_id, bot_obj), daemon=True).start()
        except Exception:
            pass
...
def register_handlers(bot_obj, bot_token, bot_index):
    ...
    def handle_admin_command(message):
        try:
            ...
            if is_admin(uid):
                sess = in_memory_data["admin_sessions"].get(str(uid))
                if sess:
                    close_admin_panel(bot_obj, uid)
                    return
                open_admin_panel(bot_obj, message.chat.id, message.from_user)
                return
            with memory_lock:
                in_memory_data["admin_pending_auth"][str(uid)] = {"awaiting": True, "chat_id": message.chat.id}
            bot_obj.send_message(message.chat.id, "Please send the secret key to become admin.")
        except Exception:
            logging.exception("Error in handle_admin_command")
    ...
    def handle_text_messages(message):
        try:
            ...
            uid = message.from_user.id
            with memory_lock:
                pending = in_memory_data["admin_pending_auth"].get(str(uid))
            if pending and pending.get("awaiting"):
                text = (message.text or "").strip()
                if text == ADMIN_SECRET:
                    admins_collection.update_one({"_id": str(uid)}, {"$set": {"added_at": datetime.now(), "username": getattr(message.from_user, "username", None), "first_name": getattr(message.from_user, "first_name", None), "last_name": getattr(message.from_user, "last_name", None)}}, upsert=True)
                    with memory_lock:
                        in_memory_data["admin_pending_auth"].pop(str(uid), None)
                    bot_obj.send_message(message.chat.id, "✅ You are now an admin.")
                    open_admin_panel(bot_obj, message.chat.id, message.from_user)
                else:
                    bot_obj.send_message(message.chat.id, "❌ Invalid secret key.")
                return
            with memory_lock:
                b_pending = in_memory_data["admin_broadcast_pending"].get(str(uid))
            if b_pending and b_pending.get("waiting_for_content"):
                target = b_pending.get("targets", "all")
                in_memory_data["admin_broadcast_pending"].pop(str(uid), None)
                bot_obj.send_message(message.chat.id, "Broadcast is starting. This may take some time.")
                def perform_broadcast(content_type, content_payload, bot_instance, target_selection):
                    groups = list(groups_collection.find({}))
                    users = list(users_collection.find({}))
                    def send_to_chat(chat_id):
                        try:
                            if content_type == "text":
                                bot_instance.send_message(chat_id, content_payload, disable_web_page_preview=True)
                            elif content_type == "photo":
                                bot_instance.send_photo(chat_id, content_payload)
                            elif content_type == "audio":
                                bot_instance.send_audio(chat_id, content_payload)
                            elif content_type == "voice":
                                bot_instance.send_voice(chat_id, content_payload)
                            elif content_type == "video":
                                bot_instance.send_video(chat_id, content_payload)
                            elif content_type == "document":
                                bot_instance.send_document(chat_id, content_payload)
                        except Exception:
                            pass
                    if target_selection in ("all", "groups"):
                        for g in groups:
                            try:
                                send_to_chat(int(g["_id"]))
                            except Exception:
                                pass
                            time.sleep(0.05)
                    if target_selection in ("all", "users"):
                        for u in users:
                            try:
                                send_to_chat(int(u["_id"]))
                            except Exception:
                                pass
                            time.sleep(0.05)
                    try:
                        bot_instance.send_message(message.chat.id, "Broadcast completed.")
                    except Exception:
                        pass
                if message.text:
                    threading.Thread(target=perform_broadcast, args=("text", message.text, bot_obj, target), daemon=True).start()
                    return
    ...
    @bot_obj.callback_query_handler(lambda c: c.data and c.data.startswith("adminpanel|"))
    def adminpanel_callback(call):
        try:
            parts = call.data.split("|")
            if len(parts) < 2:
                bot_obj.answer_callback_query(call.id, "Invalid action", show_alert=True)
                return
            action = parts[1]
            uid = call.from_user.id
            if not is_admin(uid):
                bot_obj.answer_callback_query(call.id, "You are not an admin", show_alert=True)
                return
            if action == "broadcast":
                markup = InlineKeyboardMarkup(row_width=2)
                markup.add(InlineKeyboardButton("All (groups+users)", callback_data="adminpanel|broadcast_target|all"))
                markup.add(InlineKeyboardButton("Groups only", callback_data="adminpanel|broadcast_target|groups"))
                markup.add(InlineKeyboardButton("Users only", callback_data="adminpanel|broadcast_target|users"))
                bot_obj.send_message(call.message.chat.id, "Choose broadcast targets:", reply_markup=markup)
            elif action == "recent":
                three_days_ago = datetime.now() - timedelta(days=3)
                recent = list(users_collection.find({"last_active": {"$gte": three_days_ago}, "stt_conversion_count": {"$gt": 0}}).sort("last_active", -1))
                lines = []
                for r in recent:
                    ...
                    cnt = r.get("stt_conversion_count", 0)
                    last_str = last.strftime("%Y-%m-%d %H:%M:%S") if isinstance(last, datetime) else str(last)
                    lines.append(f"{uid}  files:{cnt}  last:{last_str}")
                text = "\n".join(lines) if lines else "No recent file processors in last 3 days"
                bot_obj.send_message(call.message.chat.id, text)
            elif action == "close":
                close_admin_panel(bot_obj, uid)
            bot_obj.answer_callback_query(call.id)
        except Exception:
            logging.exception("Error in adminpanel_callback")
            try:
                bot_obj.answer_callback_query(call.id, "Error", show_alert=True)
            except Exception:
                pass

    @bot_obj.callback_query_handler(lambda c: c.data and c.data.startswith("adminpanel|broadcast_target|"))
    def admin_broadcast_target_callback(call):
        try:
            parts = call.data.split("|")
            if len(parts) < 3:
                bot_obj.answer_callback_query(call.id, "Invalid broadcast target", show_alert=True)
                return
            target = parts[2]
            uid = call.from_user.id
            if not is_admin(uid):
                ...
                return
            with memory_lock:
                in_memory_data["admin_broadcast_pending"][str(uid)] = {"waiting_for_content": True, "targets": target}
            bot_obj.send_message(call.message.chat.id, "Now send the content to broadcast. You can send text, photo, audio, voice, video or document.")
            bot_obj.answer_callback_query(call.id)
        except Exception:
            logging.exception("Error in admin_broadcast_target_callback")
            try:
                bot_obj.answer_callback_query(call.id, "Error", show_alert=True)
            except Exception:
                pass

    @bot_obj.message_handler(content_types=['photo', 'audio', 'voice', 'video', 'document'])
    def handle_media_broadcast(message):
        try:
            uid = message.from_user.id
            with memory_lock:
                b_pending = in_memory_data["admin_broadcast_pending"].get(str(uid))
            if not b_pending or not b_pending.get("waiting_for_content"):
                return
            target = b_pending.get("targets", "all")
            with memory_lock:
                in_memory_data["admin_broadcast_pending"].pop(str(uid), None)
            bot_obj.send_message(message.chat.id, "Broadcast is starting. This may take some time.")
            def perform_broadcast_media(content_type, file_id, bot_instance, target_selection):
                groups = list(groups_collection.find({}))
                users = list(users_collection.find({}))
                def send_to_chat(chat_id):
                    try:
                        if content_type == "photo":
                            bot_instance.send_photo(chat_id, file_id)
                        elif content_type == "audio":
                            bot_instance.send_audio(chat_id, file_id)
                        elif content_type == "voice":
                            bot_instance.send_voice(chat_id, file_id)
                        elif content_type == "video":
                            bot_instance.send_video(chat_id, file_id)
                        elif content_type == "document":
                            bot_instance.send_document(chat_id, file_id)
                    except Exception:
                        pass
                if target_selection in ("all", "groups"):
                    for g in groups:
            ...
            if message.photo:
                ...
                threading.Thread(target=perform_broadcast_media, args=("photo", file_id, bot_obj, target), daemon=True).start()
            elif message.audio:
                file_id = message.audio.file_id
                threading.Thread(target=perform_broadcast_media, args=("audio", file_id, bot_obj, target), daemon=True).start()
            elif message.voice:
                file_id = message.voice.file_id
                threading.Thread(target=perform_broadcast_media, args=("voice", file_id, bot_obj, target), daemon=True).start()
            elif message.video:
                file_id = message.video.file_id
                threading.Thread(target=perform_broadcast_media, args=("video", file_id, bot_obj, target), daemon=True).start()
            elif message.document:
                file_id = message.document.file_id
                threading.Thread(target=perform_broadcast_media, args=("document", file_id, bot_obj, target), daemon=True).start()
        except Exception:
            logging.exception("Error in handle_media_broadcast")
