...
# **Function cusub oo lagu dhiso markup-ka badhamada**
def get_new_markup(chat_id_val, msg_id, action_usage, text_length):
    markup = InlineKeyboardMarkup()
    buttons = []
    
    # Check current usage for 'clean_up'
    clean_key = f"{chat_id_val}|{msg_id}|clean_up"
    clean_used = action_usage.get(clean_key, 0) >= 1
    
    # Check current usage for 'get_key_points'
    summary_key = f"{chat_id_val}|{msg_id}|get_key_points"
    summary_used = action_usage.get(summary_key, 0) >= 1
    
    # Create the 'Clean transcript' button
    if not clean_used:
        buttons.append(InlineKeyboardButton("⭐️Clean transcript", callback_data=clean_key))
    else:
        buttons.append(InlineKeyboardButton("✅ Cleaned", callback_data="used_action"))
        
    # Create the 'Get Summarize' button (only show if it was originally eligible based on text length > 1000)
    if text_length > 1000:
        if not summary_used:
            buttons.append(InlineKeyboardButton("Get Summarize", callback_data=summary_key)) 
        else:
            buttons.append(InlineKeyboardButton("✅ Summarized", callback_data="used_action"))
...
def handle_media_common(message, bot_obj, bot_token, bot_index=0):
    ...
    try:
        ...
        try:
            ...
            # Badhamada cusub
            initial_markup = get_new_markup(message.chat.id, 0, action_usage, len(corrected_text)) # msg_id 0 will be replaced
            
            if len(corrected_text) > 4000:
                if user_mode == "file":
                    f = io.BytesIO(corrected_text.encode("utf-8"))
                    f.name = "transcription.txt"
                    # markup halkan waa inuu ahaado mid aan badhamo lahayn
                    sent = bot_obj.send_document(message.chat.id, f, reply_to_message_id=message.message_id)
                    # Halkan waxaan ku cusbooneysiinaynaa badhamada
                    final_markup = get_new_markup(message.chat.id, sent.message_id, action_usage, len(corrected_text))
                    try:
                    ...
                    # Halkan waxaan ku cusbooneysiinaynaa badhamada
                    final_markup = get_new_markup(message.chat.id, last_sent.message_id, action_usage, len(corrected_text))
                    try:
                        bot_obj.edit_message_reply_markup(message.chat.id, last_sent.message_id, reply_markup=final_markup)
                    except Exception: pass
                    try:
                ...
                sent_msg = bot_obj.send_message(message.chat.id, corrected_text or "⚠️ Make sure the voice is speaking in the language you Choosed or clear.", reply_to_message_id=message.message_id)
                # Halkan waxaan ku cusbooneysiinaynaa badhamada
                final_markup = get_new_markup(message.chat.id, sent_msg.message_id, action_usage, len(corrected_text))
                try:
                    bot_obj.edit_message_reply_markup(message.chat.id, sent_msg.message_id, reply_markup=final_markup)
                except Exception: pass
                try:
...
def upload_large_file(token):
    ...
    def process_uploaded_file(chat_id_inner, lang_inner, path, bot_index_inner):
        try:
            ...
            sent_msg = None
            try:
                # Markup waa inaan ka helno function-ka get_new_markup, laakin waa fariin cusub, sidaa darteed action_usage halkan waa 0
                ...
                if len(corrected_text) > 4000:
                    if user_mode == "file":
                        ...
                        sent_msg = bot_to_use.send_document(chat_id_inner, fobj) # Send without markup
                    else:
                        chunks = split_text_into_chunks(corrected_text, limit=4096)
                        last_sent = None
                        for idx, chunk in enumerate(chunks):
                        ...
                        sent_msg = last_sent
                else: sent_msg = bot_to_use.send_message(chat_id_inner, corrected_text or "No transcription text was returned.") # Send without markup
                
                # Halkan waxaan ku cusbooneysiinaynaa badhamada
                final_markup = get_new_markup(chat_id_inner, sent_msg.message_id, action_usage, len(corrected_text))
                try:
            ...
            try:
                # Marka la isticmaalo function-ka get_new_markup, waxaan u baahanahay inaan dib u cusboonaysiino action_usage
...
def register_handlers(bot_obj, bot_token, bot_index):
    ...
    # **Callback Handler-ka Cusub ee Marka la Taabto Mar Labaad**
    @bot_obj.callback_query_handler(lambda c: c.data == "used_action")
    def used_action_callback(call):
        bot_obj.answer_callback_query(call.id, "❌ Action-kaan horey ayaa loo sameeyay ama waa dhacday.", show_alert=True)
    ...
    def get_key_points_callback(call):
        try:
            ...
            # 1. Halkan waxaan ku eegaynaa in horey loo isticmaalay (waa inaan hubinaa in xitaa haddii aysan ku jirin user_transcriptions)
            usage = action_usage.get(usage_key, 0)
            if usage >= 1: 
                # 2. Halkan waxaan soo saaraynaa fariinta error-ka
                bot_obj.answer_callback_query(call.id, "❌ Summarize-nta horey ayaa loo sameeyay ama waa dhacday.", show_alert=True)
                return
                
            action_usage[usage_key] = usage + 1
            
            # 3. Halkan waxaan badalaynaa badhamada si uusan mar kale u taaban
            stored_text = user_transcriptions.get(str(chat_id_val), {}).get(msg_id, "")
            try:
                new_markup = get_new_markup(chat_id_val, msg_id, action_usage, len(stored_text))
                bot_obj.edit_message_reply_markup(call.message.chat.id, call.message.message_id, reply_markup=new_markup)
            except Exception:
                logging.exception("Failed to edit markup after first use of summarize")
            
            # Qeybta kale ee shaqada
    ...
    def clean_up_callback(call):
        try:
            ...
            # 1. Halkan waxaan ku eegaynaa in horey loo isticmaalay
            usage = action_usage.get(usage_key, 0)
            if usage >= 1:
                # 2. Halkan waxaan soo saaraynaa fariinta error-ka
                bot_obj.answer_callback_query(call.id, "❌ Nadiifinta horey ayaa loo sameeyay ama waa dhacday.", show_alert=True)
                return
            ...
            try:
                ...
                logging.exception("Failed to edit markup after first use of clean_up")
            ...
            if cleaned_text_length > 4000:
                if user_mode == "file":
                    ...
                    # U dir badhamo cusub fariinta la soo diray.
                    final_markup = get_new_markup(chat_id_val, sent.message_id, action_usage, cleaned_text_length)
                    try:
                        bot_obj.edit_message_reply_markup(call.message.chat.id, sent.message_id, reply_markup=final_markup)
                    except Exception: pass
                    ...
                    # U dir badhamo cusub fariinta la soo diray.
                    final_markup = get_new_markup(chat_id_val, last_sent.message_id, action_usage, cleaned_text_length)
                    try:
                        bot_obj.edit_message_reply_markup(call.message.chat.id, last_sent.message_id, reply_markup=final_markup)
                    except Exception: pass
                ...
                try:
                    ...
                    # U dir badhamo cusub fariinta la badalay
                    final_markup = get_new_markup(chat_id_val, status_msg.message_id, action_usage, cleaned_text_length)
                    try:
                        bot_obj.edit_message_reply_markup(call.message.chat.id, status_msg.message_id, reply_markup=final_markup)
                    except Exception: pass
