...
def transcribe_with_vosk(input_file_path):
    tmpdir=tempfile.mkdtemp(prefix="vosk_")
    try:
        wav_path=os.path.join(tmpdir,"converted_vosk.wav")
        try: convert_to_wav(input_file_path,wav_path)
        except Exception as e: raise RuntimeError("Conversion to WAV failed: "+str(e))
        ensure_vosk_model_loaded()
        wf=wave.open(wav_path,"rb")
        frames=wf.readframes(wf.getnframes())
        wf.close()
        if not frames:
            return ""
        rec=vosk.KaldiRecognizer(VOSK_MODEL,AUDIO_SAMPLE_RATE)
        rec.AcceptWaveform(frames)
        res_json=rec.FinalResult()
        try:
            res_obj=json.loads(res_json)
            return res_obj.get("text","")
        except:
            return ""
    finally:
        try:
...
def transcribe_via_selected_service(input_path,lang_code):
    try:
        if lang_code and str(lang_code).lower().startswith("en"):
            try:
                text=transcribe_with_vosk(input_path)
                if text is None: raise RuntimeError("VOSK returned no text")
                return text,"vosk"
            except Exception as e:
                logging.exception("VOSK failed, falling back to speech_recognition")
                try:
                    ...
                    raise RuntimeError("Both VOSK and speech_recognition failed: "+str(e2))
        else:
            text=transcribe_file_with_speech_recognition(input_path,lang_code)
            if text is None: raise RuntimeError("Speech recognition returned no text")
            return text,"speech_recognition"
    except Exception as e:
        logging.exception("transcription service error")
        raise
