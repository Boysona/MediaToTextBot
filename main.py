...
ASSEMBLYAI_MODEL = os.environ.get("ASSEMBLYAI_MODEL", "large_v2")
...
def transcribe_with_assemblyai(file_path: str, language_code: str, timeout_seconds: int = REQUEST_TIMEOUT_ASSEMBLY):
    ...
    try:
        payload = {"audio_url": upload_url, "model": ASSEMBLYAI_MODEL}
        if language_code:
    ...
    def admin_handler(message):
        try:
            ...
            if message.chat.type != 'private':
                bot_obj.send_message(message.chat.id, "Please use /admin in a private chat with the bot.")
                return
            ...
            if user_doc and user_doc.get("admin_authed"):
                def show_admin_panel(msg_context):
                    try:
                        total_users = users_collection.count_documents({})
                        agg = list(users_collection.aggregate([
                            {"$group": {"_id": None, "total_processed": {"$sum": {"$ifNull": ["$stt_conversion_count", 0]}}}}
                        ]))
                        files_processed = agg[0]["total_processed"] if agg and "total_processed" in agg[0] else 0
                        now = datetime.utcnow()
                        ...
                        last_users_cursor = users_collection.find({"last_active": {"$exists": True}}).sort("last_active", -1).limit(8)
                        last_users = []
                        for u in last_users_cursor:
                            uid_val = u.get("_id")
                            la = u.get("last_active")
                            if isinstance(la, datetime):
                                la_str = la.strftime("%Y-%m-%d %H:%M:%S")
                            else:
                                la_str = str(la)
                            last_users.append(f"{uid_val} — {la_str}")
                        last_users_text = "\n".join(last_users) if last_users else "No recent users"
                        panel_text = (
                            f"🔐 Admin Panel\n\n"
                            f"• Total users: {total_users}\n"
                            f"• Files processed (stt): {files_processed}\n"
                            f"• Uptime: {uptime_str}\n\n"
                            f"Last active users:\n{last_users_text}"
                        )
                        bot_obj.send_message(msg_context.chat.id, panel_text)
                    except Exception:
                        logging.exception("Error generating admin panel")
                        bot_obj.send_message(message.chat.id, "Error generating admin panel.")
                show_admin_panel(message)
                return
            prompt_msg = bot_obj.send_message(message.chat.id, "🔐 Enter admin password:")
            def handle_admin_password(next_msg):
                try:
                    if next_msg.from_user.id != message.from_user.id:
                        return
                    if not check_subscription(next_msg.from_user.id, bot_obj):
                        send_subscription_message(next_msg.chat.id, bot_obj)
                        return
                    provided = (next_msg.text or "").strip()
                    if provided == ADMIN_PASSWORD:
                        users_collection.update_one({"_id": uid}, {"$set": {"admin_authed": True, "admin_authed_at": datetime.now()}}, upsert=True)
                        bot_obj.send_message(next_msg.chat.id, "✅ Access granted. You are now authenticated as admin.")
                        def show_admin_panel2(msg_context):
                            try:
                                total_users = users_collection.count_documents({})
                                agg = list(users_collection.aggregate([
                                    {"$group": {"_id": None, "total_processed": {"$sum": {"$ifNull": ["$stt_conversion_count", 0]}}}}
                                ...
                                logging.exception("Error generating admin panel")
                                bot_obj.send_message(msg_context.chat.id, "Error generating admin panel.")
                        show_admin_panel2(next_msg)
                    else:
                        bot_obj.send_message(next_msg.chat.id, "❌ Wrong password.")
                except Exception:
                    logging.exception("Error in admin password handler")
                    try:
                        bot_obj.send_message(next_msg.chat.id, "Error processing admin password.")
                    except Exception:
                        pass
            try:
                bot_obj.register_next_step_handler(prompt_msg, handle_admin_password)
            except Exception:
                pass
        except Exception:
