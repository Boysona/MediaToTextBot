import os,logging,requests,telebot,json,threading,time,io,tempfile,glob,re,wave
...
if FFMPEG_BINARY is None: logging.warning("ffmpeg binary not found. Set FFMPEG_BINARY env var or place ffmpeg in path")
...
def start_typing(bot_obj,chat_id,stop_cb):
    while not stop_cb():
        try: bot_obj.send_chat_action(chat_id,'typing')
        except: pass
        time.sleep(3)

def make_action_markup(chat_id,msg_id,text):
    markup = InlineKeyboardMarkup()
    buttons = [InlineKeyboardButton("⭐️Clean transcript", callback_data=f"clean_up|{chat_id}|{msg_id}")]
    if len(text) > 1000: buttons.append(InlineKeyboardButton("Get Summarize", callback_data=f"get_key_points|{chat_id}|{msg_id}"))
    for b in buttons: markup.add(b)
    return markup

def attach_action_buttons(bot_obj,chat_id,msg_obj,text):
    try:
        markup = make_action_markup(chat_id,msg_obj.message_id,text)
        bot_obj.edit_message_reply_markup(chat_id,msg_obj.message_id,reply_markup=markup)
    except: pass
...
def transcribe_with_assemblyai(file_path,language_code,timeout_seconds=REQUEST_TIMEOUT_ASSEMBLY):
...
def handle_media_common(message,bot_obj,bot_token,bot_index=0):
    ...
    t = threading.Thread(target=start_typing,args=(bot_obj,message.chat.id,lambda:stop_typing["stop"]))
    t.start()
    try:
        ...
        try:
            ...
            try: text,used_service = transcribe_via_selected_service(tmpf.name,lang)
                ...
                if "ffmpeg" in error_msg.lower(): bot_obj.send_message(message.chat.id,"⚠️ Server error: ffmpeg not found or conversion failed. Contact admin @boyso.")
                elif is_transcoding_like_error(error_msg): bot_obj.send_message(message.chat.id,"⚠️ Transcription error: file is not audible. Please send a different file.")
                else: bot_obj.send_message(message.chat.id,f"Error during transcription: {error_msg}")
                return
            ...
            if len(corrected_text) > 4000:
                if user_mode == "file":
                    ...
                    try: attach_action_buttons(bot_obj,message.chat.id,sent,corrected_text)
                    except: pass
                    try:
                    ...
                    try: attach_action_buttons(bot_obj,message.chat.id,last_sent,corrected_text)
                    except: pass
                    try:
                ...
                try: attach_action_buttons(bot_obj,message.chat.id,sent_msg,corrected_text)
                except: pass
                try:
        ...
        if is_transcoding_like_error(error_msg): bot_obj.send_message(message.chat.id,"⚠️ Transcription error: file is not audible. Please send a different file.")
        else: bot_obj.send_message(message.chat.id,f"Error during transcription: {error_msg}")
    finally:
            ...
            try:
                ...
                if len(corrected_text) > 4000:
                    if user_mode == "file":
                        ...
                        sent_msg = bot_to_use.send_document(chat_id_inner,fobj)
                    else:
                        chunks = split_text_into_chunks(corrected_text,limit=4096)
                        last_sent=None
                        for idx,chunk in enumerate(chunks):
                    ...
                    sent_msg = bot_to_use.send_message(chat_id_inner, corrected_text or "No transcription text was returned.")
                try: attach_action_buttons(bot_to_use,chat_id_inner,sent_msg,corrected_text)
                except: pass
            except:
...
def register_handlers(bot_obj,bot_token,bot_index):
    ...
    def get_key_points_callback(call):
        try:
            ...
            stop_typing={"stop":False}
            t = threading.Thread(target=start_typing,args=(bot_obj,call.message.chat.id,lambda:stop_typing["stop"]))
            t.start()
            try:
            ...
            if not summary:
                try: bot_obj.send_message(call.message.chat.id,"No Summary returned.",reply_to_message_id=call.message.message_id)
                except: pass
            else:
    ...
    def clean_up_callback(call):
        try:
            ...
            stop_typing["stop"]=True; t.join()
            if not cleaned:
                try: bot_obj.send_message(call.message.chat.id,"No cleaned text returned.",reply_to_message_id=call.message.message_id)
                except: pass
                return
            uid_key = str(chat_id_val)
            user_mode = get_user_send_mode(uid_key)
