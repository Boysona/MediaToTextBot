...
def telegram_file_info_and_url(bot_token,file_id):
    ...
    if not file_path: raise RuntimeError("Could not get file_path from Telegram API")
...
def recognize_chunk_file(recognizer,file_path,language):
    ...
    for attempt in range(1,RECOGNITION_MAX_RETRIES+1):
        try:
            if language and language.startswith("en") and HAS_POCKETSPHINX:
                ...
                try:
                    ...
                    try:
                        with sr.AudioFile(file_path) as source:
                            audio_fb=recognizer.record(source)
                        try:
                            text=recognizer.recognize_google(audio_fb,language=language)
                        except sr.UnknownValueError:
                            return ""
