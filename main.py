...
RETRY_AFTER_MINUTES=int(os.environ.get("RETRY_AFTER_MINUTES","2"))
MAX_PENDING_QUEUE_SIZE=int(os.environ.get("MAX_PENDING_QUEUE_SIZE","100"))

PROCESSING_SEMAPHORE=threading.BoundedSemaphore(MAX_CONCURRENT_PROCESSES)
PENDING_QUEUE=deque()
...
def enqueue_job(job):
    with QUEUE_LOCK:
        if len(PENDING_QUEUE)>=MAX_PENDING_QUEUE_SIZE: return False
        PENDING_QUEUE.append(job);QUEUE_EVENT.set();return True

def dequeue_job():
    with QUEUE_LOCK:
        if not PENDING_QUEUE:
            QUEUE_EVENT.clear();return None
        return PENDING_QUEUE.popleft()

def process_job_dict(job):
    chat_id=job.get("chat_id");bot_index=job.get("bot_index",0);bot_obj=bots[bot_index] if 0<=bot_index<len(bots) else bots[0]
    orig_message_id=job.get("orig_message_id")
    lang=job.get("lang","en");filename=job.get("filename","file");tmp_path=job.get("tmp_path")
    processing_msg=None;stop_animation={"stop":False}
    try:
        processing_msg=bot_obj.send_message(chat_id,"🔄 Processing your queued file...",reply_to_message_id=orig_message_id) if orig_message_id else None
        processing_msg_id=processing_msg.message_id if processing_msg else None
        animation_thread=threading.Thread(target=animate_processing_message,args=(bot_obj,chat_id,processing_msg_id,lambda:stop_animation["stop"])) if processing_msg else None
        if animation_thread: animation_thread.start()
        try:
            try: text,used_service=transcribe_via_selected_service(tmp_path,lang)
            except Exception as e:
                error_msg=str(e);logging.exception("Queued transcription error")
                if "ffmpeg" in error_msg.lower():
                    try: bot_obj.send_message(chat_id,"⚠️ Server error: ffmpeg not found or conversion failed. Contact admin.",reply_to_message_id=orig_message_id)
                    except: pass
                elif is_transcoding_like_error(error_msg):
                    try: bot_obj.send_message(chat_id,"⚠️ Transcription error: file is not audible. Please send a different file.",reply_to_message_id=orig_message_id)
                    except: pass
                else:
                    try: bot_obj.send_message(chat_id,f"Error during transcription: {error_msg}",reply_to_message_id=orig_message_id)
                    except: pass
                return
            corrected_text=normalize_text_offline(text);uid_key=str(chat_id);user_mode=get_user_send_mode(uid_key)
            if len(corrected_text)>4000:
                if user_mode=="file":
                    f=io.BytesIO(corrected_text.encode("utf-8"));f.name="transcription.txt"
                    sent=bot_obj.send_document(chat_id,f,reply_to_message_id=orig_message_id)
                    ...
                    for idx,chunk in enumerate(chunks):
                        if idx==0: last_sent=bot_obj.send_message(chat_id,chunk,reply_to_message_id=orig_message_id)
                ...
                sent_msg=bot_obj.send_message(chat_id,corrected_text or "⚠️ Warning Make sure the voice is clear or speaking in the language you Choosed.",reply_to_message_id=orig_message_id)
            ...
            increment_processing_count(chat_id,"stt")
        finally:
        ...
        try:
            if processing_msg: bot_obj.delete_message(chat_id,processing_msg.message_id)
        except: pass
...
def queue_worker_loop():
    while True:
        ...
        while True:
            ...
            acquired=PROCESSING_SEMAPHORE.acquire(blocking=True)
            if not acquired:
                time.sleep(1)
                continue
            try:
                try:
                    process_job_dict(job)
                except Exception as e:
                    logging.exception("Error processing queued job")
            finally:
                try: PROCESSING_SEMAPHORE.release()
                except: pass
            job=None
        time.sleep(0.5)
...
def handle_media_common(message,bot_obj,bot_token,bot_index=0):
    ...
    try:
        ...
        bot_obj.send_message(message.chat.id,"Unable to fetch file info.");return
    ...
    try:
        ...
        try: tmpf.close();os.remove(tmpf.name)
        except: pass
        bot_obj.send_message(message.chat.id,"Error downloading file. Try again.");return

    job={"chat_id":message.chat.id,"bot_index":bot_index,"lang":lang,"filename":filename,"tmp_path":tmpf.name,"orig_message_id":message.message_id}
        ...
        if len(PENDING_QUEUE)==0:
            if PROCESSING_SEMAPHORE.acquire(blocking=False):
                try:
                    ...
                    logging.exception("Immediate processing failed, enqueueing instead")
                    enqueued=enqueue_job(job)
                    if not enqueued:
...
def upload_large_file(token):
    ...
    job={"chat_id":chat_id,"bot_index":bot_index,"lang":lang,"filename":getattr(file,"filename",f"upload_{int(time.time()*1000)}"),"tmp_path":tmp_path,"orig_message_id":None}
    with QUEUE_LOCK:
        if PROCESSING_SEMAPHORE.acquire(blocking=False):
            try:
                try:
                    processing_thread=threading.Thread(target=lambda: (process_job_dict(job),PROCESSING_SEMAPHORE.release()),daemon=True)
            ...
            if not enqueued:
                ...
                return "Server queue is full. Try again later.",503
    return jsonify({"status":"accepted","message":"Upload accepted. Processing will start when the server is free."})
