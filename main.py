...
def transcribe_via_selected_service(input_path: str, lang_code: str):
    try:
        ...
        raise RuntimeError("SpeechRecognition failed: " + str(e))
...
def create_action_markup(chat_id, message_id, text_length):
    markup = InlineKeyboardMarkup()
...
def register_transcription(uid_key, message_id, text):
    ...
    action_usage[f"{uid_key}|{message_id}|clean_up"] = 0
    action_usage[f"{uid_key}|{message_id}|get_key_points"] = 0

def telegram_send_file_via_url(bot_obj, url, out_path):
    tmpf = tempfile.NamedTemporaryFile(delete=False, suffix=".download")
    try:
        with requests.get(url, stream=True, timeout=REQUEST_TIMEOUT_TELEGRAM) as r:
            r.raise_for_status()
...
def handle_media_common(message, bot_obj, bot_token, bot_index=0):
    ...
    try:
        bot_obj.send_chat_action(message.chat.id, 'typing')
        tf, file_url = telegram_file_info_and_url(bot_token, file_id)
        tmp_path = telegram_send_file_via_url(bot_obj, file_url)
        try:
            text, used_service = transcribe_via_selected_service(tmp_path, lang)
        except Exception as e:
            error_msg = str(e)
        ...
        if len(corrected_text) > 4000:
            if user_mode == "file":
                ...
                try:
                    markup = create_action_markup(message.chat.id, sent.message_id, len(corrected_text))
                    bot_obj.edit_message_reply_markup(message.chat.id, sent.message_id, reply_markup=markup)
                except Exception: pass
                try:
                    register_transcription(uid_key, sent.message_id, corrected_text)
                except Exception: pass
            else:
                ...
                try:
                    markup = create_action_markup(message.chat.id, last_sent.message_id, len(corrected_text))
                    bot_obj.edit_message_reply_markup(message.chat.id, last_sent.message_id, reply_markup=markup)
                except Exception: pass
                try:
                    register_transcription(uid_key, last_sent.message_id, corrected_text)
                except Exception: pass
        else:
            markup = InlineKeyboardMarkup()
            ...
            try:
                markup = create_action_markup(message.chat.id, sent_msg.message_id, len(corrected_text))
                bot_obj.edit_message_reply_markup(message.chat.id, sent_msg.message_id, reply_markup=markup)
            except Exception: pass
            try:
                register_transcription(uid_key, sent_msg.message_id, corrected_text)
            except Exception: pass
    ...
    def process_uploaded_file(chat_id_inner, lang_inner, path, bot_index_inner):
        try:
            ...
            try:
                ...
                try:
                    markup = create_action_markup(chat_id_inner, sent_msg.message_id, len(corrected_text))
...
def register_handlers(bot_obj, bot_token, bot_index):
    ...
    def get_key_points_callback(call):
        try:
            ...
            try:
                bot_obj.send_chat_action(call.message.chat.id, 'typing')
    ...
    def clean_up_callback(call):
        try:
            ...
            if len(cleaned) > 4000:
                ...
                try:
                    bot_obj.send_message(call.message.chat.id, f"{cleaned}", reply_to_message_id=call.message.message_id)
                    uid_key = str(chat_id_val)
                    user_transcriptions.setdefault(uid_key, {})[call.message.message_id] = cleaned
                    threading.Thread(target=delete_transcription_later, args=(uid_key, call.message.message_id), daemon=True).start()
                    action_usage[f"{call.message.chat.id}|{call.message.message_id}|clean_up"] = 0
