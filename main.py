...
def typing_action_loop(bot_obj, chat_id, stop_event):
...
def create_action_buttons(chat_id: int, message_id: int, text_len: int = 0):
    markup = InlineKeyboardMarkup()
    buttons = [InlineKeyboardButton("⭐️Clean transcript", callback_data=f"clean_up|{chat_id}|{message_id}")]
    if text_len > 1000: buttons.append(InlineKeyboardButton("Get Summarize", callback_data=f"get_key_points|{chat_id}|{message_id}"))
...
def handle_media_common(message, bot_obj, bot_token, bot_index=0):
    ...
    typing_thread = threading.Thread(target=typing_action_loop, args=(bot_obj, message.chat.id, lambda: stop_typing["stop"]))
    typing_thread.start()
    try:
        ...
        try:
            ...
            tmpf.close()
            try:
                text = transcribe_file_with_speech_recognition(tmpf.name, lang)
                used_service = "speech_recognition"
            ...
            if len(corrected_text) > 4000:
                if user_mode == "file":
                    ...
                    sent = bot_obj.send_document(message.chat.id, f, reply_to_message_id=message.message_id, reply_markup=create_action_buttons(message.chat.id, 0, len(corrected_text)))
                    try:
                    ...
                    for idx, chunk in enumerate(chunks):
                        if idx == 0:
                            if idx == len(chunks) - 1:
                                last_sent = bot_obj.send_message(message.chat.id, chunk, reply_to_message_id=message.message_id, reply_markup=create_action_buttons(message.chat.id, 0, len(corrected_text)))
                            else:
                                last_sent = bot_obj.send_message(message.chat.id, chunk, reply_to_message_id=message.message_id)
                        else:
                            if idx == len(chunks) - 1:
                                last_sent = bot_obj.send_message(message.chat.id, chunk, reply_markup=create_action_buttons(message.chat.id, 0, len(corrected_text)))
                            else:
                ...
                markup = create_action_buttons(message.chat.id, 0, len(corrected_text))
...
def upload_large_file(token):
    ...
    def process_uploaded_file(chat_id_inner, lang_inner, path, bot_index_inner):
        try:
            ...
            try:
                text = transcribe_file_with_speech_recognition(path, lang_inner)
            except Exception:
                try:
            ...
            try:
                ...
                if len(corrected_text) > 4000:
                    if user_mode == "file":
                        ...
                        sent_msg = bot_to_use.send_document(chat_id_inner, fobj, reply_markup=create_action_buttons(chat_id_inner, 0, len(corrected_text)))
                    else:
                        chunks = split_text_into_chunks(corrected_text, limit=4096)
                        last_sent = None
                        for idx, chunk in enumerate(chunks):
                            if idx == len(chunks) - 1:
                                last_sent = bot_to_use.send_message(chat_id_inner, chunk, reply_markup=create_action_buttons(chat_id_inner, 0, len(corrected_text)))
                            else:
                                last_sent = bot_to_use.send_message(chat_id_inner, chunk)
                        sent_msg = last_sent
                else: sent_msg = bot_to_use.send_message(chat_id_inner, corrected_text or "No transcription text was returned.", reply_markup=create_action_buttons(chat_id_inner, 0, len(corrected_text)))
                try:
                    uid_key = str(chat_id_inner)
                    user_transcriptions.setdefault(uid_key, {})[sent_msg.message_id] = corrected_text
                    threading.Thread(target=delete_transcription_later, args=(uid_key, sent_msg.message_id), daemon=True).start()
...
def register_handlers(bot_obj, bot_token, bot_index):
    ...
    def get_key_points_callback(call):
        try:
            ...
            stop_typing = {"stop": False}
            typing_thread = threading.Thread(target=typing_action_loop, args=(bot_obj, call.message.chat.id, lambda: stop_typing["stop"]))
            typing_thread.start()
            try:
            ...
            if not summary:
                ...
                sent = bot_obj.send_message(call.message.chat.id, f"{summary}")
                try:
                    user_transcriptions.setdefault(str(call.message.chat.id), {})[sent.message_id] = summary
                    threading.Thread(target=delete_transcription_later, args=(str(call.message.chat.id), sent.message_id), daemon=True).start()
        ...
        try:
            ...
            if len(cleaned) > 4000:
                if user_mode == "file":
                    ...
                    sent = bot_obj.send_document(call.message.chat.id, f, reply_to_message_id=call.message.message_id, reply_markup=create_action_buttons(call.message.chat.id, 0, len(cleaned)))
                    try:
                    ...
                    for idx, chunk in enumerate(chunks):
                        if idx == len(chunks) - 1:
                            last_sent = bot_obj.send_message(call.message.chat.id, chunk, reply_to_message_id=call.message.message_id, reply_markup=create_action_buttons(call.message.chat.id, 0, len(cleaned)))
                        else:
