...
DB_USER = os.environ.get("DB_USER", "hoskasii")
DB_PASSWORD = os.environ.get("DB_PASSWORD", "GHyCdwpI0PvNuLTg")
DB_APPNAME = os.environ.get("DB_APPNAME", "telegram_bot_db")
...
# Admin Settings
ADMIN_PASSWORD = os.environ.get("ADMIN_PASSWORD", "secret_admin_pass")
...
def is_admin(user_id: int) -> bool:
    return ADMIN_USERS_COLLECTION.find_one({"_id": str(user_id)}) is not None
...
def get_admin_stats() -> dict:
    ...
    total_stt_conversions = users_collection.aggregate([
        {"$group": {"_id": None, "total": {"$sum": "$stt_conversion_count"}}}
    ])
    total_processed_files = next(total_stt_conversions, {"total": 0})["total"]
    
    seven_days_ago = datetime.now() - timedelta(days=7)
    recent_users_with_process = users_collection.find({
        "last_active": {"$gte": seven_days_ago},
        "stt_conversion_count": {"$gt": 0}
    }).sort("last_active", -1).limit(10)
    
    recent_usernames = [
        f"@{u.get('username')}" 
        for u in recent_users_with_process 
        if u.get('username')
    ]
...
def update_user_activity(user_id: int, username: str = None):
    ...
    users_collection.update_one(
        {"_id": user_id_str},
        {"$set": update_data, "$setOnInsert": {"first_seen": now, "stt_conversion_count": 0}},
        upsert=True
    )
...
def select_speech_model_for_lang(language_code: str):
    return ASSEMBLYAI_SPEECH_MODEL
...
def transcribe_with_assemblyai(file_path: str, language_code: str, timeout_seconds: int = REQUEST_TIMEOUT_ASSEMBLY):
    ...
    try:
        ...
        if ASSEMBLYAI_SPEECH_MODEL:
            payload["model"] = ASSEMBLYAI_SPEECH_MODEL
...
def check_subscription(user_id: int, bot_obj) -> bool:
    ...
    try:
        ...
        # Hadii channel-ka la heli waayo ama bot-ka uusan ku jirin
        if "chat not found" in str(e) or "bot is not a member of the channel" in str(e):
            return True # Waxaan u ogolaanaynaa hadii channel-ka la waayo si uusan u jabin koodhka
        return False
    except Exception:
        return False
...
def send_subscription_message(chat_id: int, bot_obj):
    ...
    try:
        ...
        bot_obj.send_message(
            chat_id,
            f"🔒 Access Locked You cannot use this bot until you join the Channel {REQUIRED_CHANNEL}.",
            reply_markup=markup,
...
def handle_media_common(message, bot_obj, bot_token, bot_index=0):
    ...
    if message.chat.type == 'private' and not check_subscription(user_id, bot_obj):
    ...
    try:
        ...
        try:
            ...
            # Logic for sending transcription result
            markup = InlineKeyboardMarkup()
            buttons = []
            ...
            if len(corrected_text) > 4000:
                ...
                sent = bot_obj.send_message(message.chat.id, corrected_text or "No transcription text was returned.", reply_to_message_id=message.message_id, reply_markup=markup)

            # Add inline buttons to the final message
            if corrected_text:
                ...
                # Update markup for the sent message
                if buttons:
                    final_markup = InlineKeyboardMarkup()
                    for b in buttons:
                        final_markup.add(b)
                    
                    if sent.content_type == 'document':
                        # Can't edit a document's reply_markup without sending a new one, but we use edit_message_reply_markup.
                        # Telegram sometimes treats send_document as a send_message with media content.
                        try:
                            bot_obj.edit_message_reply_markup(message.chat.id, sent.message_id, reply_markup=final_markup)
                        except Exception:
                            pass
                    else:
                        try:
                ...
                # Store transcription and usage info
                try:
...
def upload_large_file(token):
    ...
    def process_uploaded_file(chat_id_inner, lang_inner, path, bot_index_inner):
        try:
            ...
            try:
                # Check subscription for uploaded files as well
                if not check_subscription(chat_id_inner, bot_to_use):
                    send_subscription_message(chat_id_inner, bot_to_use)
                    return
            ...
            try:
                ...
                # Add inline buttons to the final message
                if corrected_text and sent_msg:
                    buttons = []
                    ...
                    if buttons:
                        ...
                        for b in buttons:
                            final_markup.add(b)
                        try:
                            bot_to_use.edit_message_reply_markup(chat_id_inner, sent_msg.message_id, reply_markup=final_markup)
                        except Exception:
                            pass
...
def register_handlers(bot_obj, bot_token, bot_index):
    ...
    def admin_command_handler(message):
        try:
            if message.chat.type != 'private':
                bot_obj.send_message(message.chat.id, "Admin panel waa kaliya *private chat*.", parse_mode='Markdown')
                return
            
            if is_admin(message.from_user.id):
                stats = get_admin_stats()
                recent_users_list = "\n".join(stats['recent_usernames']) if stats['recent_usernames'] else "No recent users in the last 7 days."
                
                response = (
                    "**Admin Panel** 📊\n\n"
                    f"**Tirada Users-ka Guud:** `{stats['total_users']}`\n"
                    f"**Tirada Files la Process Gareeyay:** `{stats['total_processed_files']}`\n"
                    f"**Uptime:** `{stats['uptime']}`\n"
                    f"**ASSEMBLYAI Model:** `{ASSEMBLYAI_SPEECH_MODEL}`\n\n"
                    f"**Liiska dadka ugu dambeeyay (7 maalmood):**\n"
                    f"```\n{recent_users_list}\n```"
                )
                bot_obj.send_message(message.chat.id, response, parse_mode='Markdown')
            else:
                markup = ReplyKeyboardMarkup(resize_keyboard=True, one_time_keyboard=True)
                markup.add(KeyboardButton('Cancel'))
                msg = bot_obj.send_message(message.chat.id, "Fadlan soo dir **furaha sirta ah** (admin password) si aad u hesho ogolaansho:", reply_markup=markup, parse_mode='Markdown')
                bot_obj.register_next_step_handler(msg, admin_password_step)
        except Exception:
            logging.exception("Error in admin_command_handler")
            
    def admin_password_step(message):
        try:
            if message.text == 'Cancel':
                bot_obj.send_message(message.chat.id, "Waa la joojiyay.", reply_markup=telebot.types.ReplyKeyboardRemove())
                return

            if message.text == ADMIN_PASSWORD:
                grant_admin_access(message.from_user.id)
                bot_obj.send_message(message.chat.id, "✅ Ogolaansho waa la siiyay. Hadda isticmaal /admin mar kale si aad u hesho panelka.", reply_markup=telebot.types.ReplyKeyboardRemove())
            else:
                msg = bot_obj.send_message(message.chat.id, "❌ Furaha sirta ah ma saxna. Isku day mar kale ama qor 'Cancel':", reply_markup=message.reply_markup)
                bot_obj.register_next_step_handler(msg, admin_password_step)
        except Exception:
            logging.exception("Error in admin_password_step")
    ...
    def start_handler(message):
        try:
            username = message.from_user.username
            update_user_activity(message.from_user.id, username)
    ...
    def start_select_lang_callback(call):
        try:
            ...
            try:
                bot_obj.edit_message_text(f"✅ Language set to **{lang_label}**\n\n"
                                          "👋 Salaam! Send me **voice message**, **audio file**, or **video** to transcribe for free.", 
                                          chat_id=call.message.chat.id, message_id=call.message.message_id, parse_mode='Markdown')
            except Exception:
                bot_obj.send_message(call.message.chat.id, f"✅ Language set to **{lang_label}**\n\n"
                                          "👋 Salaam! Send me **voice message**, **audio file**, or **video** to transcribe for free.", parse_mode='Markdown')
        ...
        try:
            ...
            text = (
              "Commands supported:\n"
                ...
                "/admin - Admin Panel (requires password)\n"
    ...
    def handle_mode(message):
        try:
            ...
            bot_obj.send_message(message.chat.id, f"Result delivery mode: **{mode_text}**. Change it below:", reply_markup=build_result_mode_keyboard(), parse_mode='Markdown')
        except Exception:
    ...
    def on_stt_language_select(call):
        try:
            ...
            set_stt_user_lang(uid, lang_code)
            try:
                bot_obj.edit_message_text(f"✅ Language set to **{lang_label}**", chat_id=call.message.chat.id, message_id=call.message.message_id, parse_mode='Markdown')
            except Exception:
                pass
    ...
    def on_result_mode_select(call):
        try:
            ...
            try:
                bot_obj.edit_message_text(f"✅ Result mode set: **{mode_text}**", chat_id=call.message.chat.id, message_id=call.message.message_id, parse_mode='Markdown')
            except Exception:
                pass
    ...
    def get_key_points_callback(call):
        try:
            ...
            if not summary:
                ...
                try:
                    bot_obj.edit_message_text(f"**Summary**\n\n{summary}", chat_id=call.message.chat.id, message_id=status_msg.message_id, parse_mode='Markdown')
                except Exception:
                    pass
        except Exception:
        ...
        try:
            ...
            if len(cleaned) > 4000:
                if user_mode == "file":
                    ...
                    # Update message data for new message
                    try:
                        user_transcriptions.setdefault(uid_key, {})[sent.message_id] = cleaned
                        threading.Thread(target=delete_transcription_later, args=(uid_key, sent.message_id), daemon=True).start()
                        # Reset action usage for the new message
                        ...
                        # Try to update inline keyboard for the new message
                        new_markup = InlineKeyboardMarkup()
                        new_buttons = []
                        new_buttons.append(InlineKeyboardButton("⭐️Clean transcript", callback_data=f"clean_up|{call.message.chat.id}|{sent.message_id}"))
                        if len(cleaned) > 1000:
                            new_buttons.append(InlineKeyboardButton("Get Summarize", callback_data=f"get_key_points|{call.message.chat.id}|{sent.message_id}"))
                        for b in new_buttons:
                            new_markup.add(b)
                        bot_obj.edit_message_reply_markup(call.message.chat.id, sent.message_id, reply_markup=new_markup)
                    except Exception:
                        pass
                    ...
                    try:
                        ...
                        # Try to update inline keyboard for the last message
                        new_markup = InlineKeyboardMarkup()
                        new_buttons = []
                        new_buttons.append(InlineKeyboardButton("⭐️Clean transcript", callback_data=f"clean_up|{call.message.chat.id}|{last_sent.message_id}"))
                        if len(cleaned) > 1000:
                            new_buttons.append(InlineKeyboardButton("Get Summarize", callback_data=f"get_key_points|{call.message.chat.id}|{last_sent.message_id}"))
                        for b in new_buttons:
                            new_markup.add(b)
                        bot_obj.edit_message_reply_markup(call.message.chat.id, last_sent.message_id, reply_markup=new_markup)
                    except Exception:
                        pass
            else:
                try:
                    bot_obj.edit_message_text(f"**Cleaned Transcript**\n\n{cleaned}", chat_id=call.message.chat.id, message_id=status_msg.message_id, parse_mode='Markdown')
                    # Overwrite old transcription with cleaned one
                    uid_key = str(chat_id_val)
                    user_transcriptions.setdefault(uid_key, {})[status_msg.message_id] = cleaned
                    # Reset action usage since it's now a 'new' cleaned transcript
