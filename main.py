...
ASSEMBLYAI_API_KEY = os.environ.get("ASSEMBLYAI_API_KEY", "44da64715f8304ca3a7c78638c4bfd90c")
...
def update_user_activity(user_id: int, username: str = None):
    ...
    now = datetime.now()
    update_fields = {"$set": {"last_active": now}, "$setOnInsert": {"first_seen": now, "stt_conversion_count": 0}}
    if username:
        update_fields["$set"]["username"] = username
...
def get_admin_stats(bot_obj) -> str:
    total_users = users_collection.count_documents({})
    
    pipeline_total_files = [
        {"$group": {"_id": None, "total_processed": {"$sum": "$stt_conversion_count"}}}
    ]
    total_files_result = list(users_collection.aggregate(pipeline_total_files))
    total_processed_files = total_files_result[0]["total_processed"] if total_files_result else 0
    ...
    recent_users_cursor = users_collection.find(
        {"last_active": {"$gte": seven_days_ago}, "stt_conversion_count": {"$gt": 0}, "username": {"$ne": None}},
        {"username": 1, "last_active": 1}
    ).sort("last_active", -1).limit(10)
    
    recent_users_list = []
    for user in recent_users_cursor:
        username = user.get("username", "N/A")
        recent_users_list.append(f"@{username}") 
    
    recent_users_text = "\n".join(recent_users_list) if recent_users_list else "None found."

    stats_text = (
        "📊 **Bot Statistics** 📊\n\n"
        f"👥 **Tirada users-ka guud:** {total_users}\n"
        f"📄 **Tirada files la process gareeyay:** {total_processed_files}\n"
        f"⏳ **Uptime:** {uptime_str}\n\n"
        f"⚙️ **Liiska dadka ugu dambeeyay ee 7 maalmood ugu dambaysey bot-ka isticmaalay:** (max 10)\n"
        f"{recent_users_text}"
    )
    return stats_text
    ...
    def start_handler(message):
        try:
            ...
            if message.text.lower() == '/admin':
                stats_message = get_admin_stats(bot_obj)
                bot_obj.send_message(
                    chat_id,
                    stats_message
                )
