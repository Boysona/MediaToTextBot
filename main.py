...
def set_bot_commands_on_startup():
    common_cmds = [
        ...
        telebot.types.BotCommand("lang", "Set File Language"),
        telebot.types.BotCommand("mode", "Change result delivery mode"),
    ...
    for idx, bot_obj in enumerate(bots):
        ...
        try:
            bot_obj.set_my_commands(common_cmds + [admin_cmd],
                                    scope=telebot.types.BotCommandScopeAllChatAdministrators())
        except Exception as e:
            logging.error("Failed to set admin command for chat administrators on bot %s: %s", idx, str(e))
