import os,logging,requests,telebot,json,threading,time,io,subprocess,tempfile,glob,re,wave,math,queue
...
CHUNK_BATCH_SIZE=int(os.environ.get("CHUNK_BATCH_SIZE","40"))
...
def is_admin(user_id):
    try:
        ...
        try:
            ...
            return False
    return uid_int in ADMIN_USER_IDS
...
def update_user_activity(user_id):
    user_id_str=norm_user_id(user_id);now=datetime.now()
...
def get_stt_user_lang(user_id):
    ud=users_collection.find_one({"user_id":norm_user_id(user_id)});return ud.get("stt_language","en") if ud else "en"
...
def get_user_send_mode(user_id):
    ud=users_collection.find_one({"user_id":norm_user_id(user_id)});return ud.get("stt_send_mode","file") if ud else "file"

def set_user_send_mode(user_id,mode):
    if mode not in ("file","split"): mode="file"
    users_collection.update_one({"user_id":norm_userid(user_id)} if False else {"user_id":norm_user_id(user_id)},{"$set":{"stt_send_mode":mode}},upsert=True)
...
def attach_action_buttons(bot_obj,chat_id,message_id,text):
    try:
        ...
        m=InlineKeyboardMarkup();m.add(InlineKeyboardButton("⭐️Clean transcript",callback_data=f"clean_up|{chat_id}|{message_id}"))
    ...
    try:
        action_usage[f"{chat_id}|{message_id}|clean_up"]=0;action_usage[f"{chat_id}|{message_id}|get_key_points"]=0
    except: pass

processing_queue=queue.Queue()
worker_busy=threading.Event()

def enqueue_transcription_job(job):
...
def processing_worker():
    while True:
        ...
        try:
            bot_index=job.get("bot_index",0)
            if bot_index<0 or bot_index>=len(bots): bot_index=0
            bot_obj=bots[bot_index]
            chat_id=job.get("chat_id")
            reply_to=job.get("reply_to")
            file_path=job.get("file_path")
            lang=job.get("lang","en")
            user_mode=job.get("user_mode","file")
            try:
                status_msg=bot_obj.send_message(chat_id,"🔄 Processing...",reply_to_message_id=reply_to)
            except:
                try: status_msg=bot_obj.send_message(chat_id,"🔄 Processing...")
                except: status_msg=None
            status_msg_id=status_msg.message_id if status_msg else None
            stop_animation={"stop":False}
            animation_thread=None
            if status_msg_id:
                animation_thread=threading.Thread(target=animate_processing_message,args=(bot_obj,chat_id,status_msg_id,lambda:stop_animation["stop"]))
                animation_thread.start()
            worker_busy.set()
            try:
                try:
                    text,used_service=transcribe_via_selected_service(file_path,lang)
                except Exception as e:
                    error_msg=str(e)
                    logging.exception("Error during transcription in worker")
                    if "ffmpeg" in error_msg.lower():
                        try: bot_obj.send_message(chat_id,"⚠️ Server error: ffmpeg not found or conversion failed. Contact admin",reply_to_message_id=reply_to)
                        except: pass
                    elif is_transcoding_like_error(error_msg):
                        try: bot_obj.send_message(chat_id,"⚠️ Transcription error: file is not audible. Please send a different file.",reply_to_message_id=reply_to)
                        except: pass
                    else:
                        try: bot_obj.send_message(chat_id,f"Error during transcription: {error_msg}",reply_to_message_id=reply_to)
                        except: pass
                    continue
                ...
                if len(corrected_text)>4000:
                    if user_mode=="file":
                        ...
                        try:
                            sent=bot_obj.send_document(chat_id,f,reply_to_message_id=reply_to)
                            try: attach_action_buttons(bot_obj,chat_id,sent.message_id,corrected_text)
                            except: pass
                            try:
                            ...
                            logging.exception("Failed to send transcription document")
                            try: bot_obj.send_message(chat_id,"Error sending transcription file.",reply_to_message_id=reply_to)
                            except: pass
                    else:
                        chunks=split_text_into_chunks(corrected_text,limit=4096);last_sent=None
                        try:
                            for idx,chunk in enumerate(chunks):
                                if idx==0: last_sent=bot_obj.send_message(chat_id,chunk,reply_to_message_id=reply_to)
                                else: last_sent=bot_obj.send_message(chat_id,chunk)
                            try: attach_action_buttons(bot_obj,chat_id,last_sent.message_id,corrected_text)
                            except: pass
                            try:
                            ...
                            logging.exception("Failed to send split messages")
                            try: bot_obj.send_message(chat_id,"Error sending transcription messages.",reply_to_message_id=reply_to)
                            except: pass
                else:
                    try:
                        sent_msg=bot_obj.send_message(chat_id,corrected_text or "⚠️ Warning Make sure the voice is clear or speaking in the language you Choosed.",reply_to_message_id=reply_to)
                        try: attach_action_buttons(bot_obj,chat_id,sent_msg.message_id,corrected_text)
                        except: pass
                        try:
                        ...
                        logging.exception("Failed to send transcription message")
                        try: bot_obj.send_message(chat_id,"Error sending transcription result.",reply_to_message_id=reply_to)
                        except: pass
                try: increment_processing_count(job.get("user_id_str",chat_id),"stt")
                except: pass
            finally:
                worker_busy.clear()
                ...
                if status_msg_id:
                    try: bot_obj.delete_message(chat_id,status_msg_id)
                    except: pass
                try:
    ...
    try:
        ...
        if target_type in ("all","groups"):
            for g in groups_collection.find({}, {"_id": 1}):
                gid = g.get("_id")
                try:
                    recipients.append(int(gid))
                except Exception:
                    try:
                        recipients.append(int(str(gid)))
                    except Exception:
                        continue
...
def upload_large_file(token):
    ...
    job={"chat_id":chat_id,"lang":lang,"bot_index":bot_index,"file_path":tmp_path,"reply_to":None,"user_mode":get_user_send_mode(str(chat_id)),"user_id_str":str(chat_id)}
    pos=enqueue_transcription_job(job)
    try:
        bot_to_use=bots[bot_index]
        if worker_busy.is_set() or pos>1:
            try: bot_to_use.send_message(chat_id,"⏳ Fadlan sug daqiiqad. Faylkaaga waa la sugi doonaa oo la farsameyn doonaa marka faylasha hore la dhameeyo.")
            except: pass
        else:
            try: bot_to_use.send_message(chat_id,"🔄 Faylkaagu waxa uu bilaabayaa farsameynta hadda.")
            except: pass
    except: pass
    return jsonify({"status":"accepted","message":"Upload accepted. Processing queued. Your transcription will be sent to your Telegram chat when ready."})
...
def register_handlers(bot_obj,bot_token,bot_index):
    ...
    def start_handler(message):
        try:
            ...
            if message.chat.type=='private' and not is_admin(message.from_user.id) and not check_subscription(message.from_user.id,bot_obj):
    ...
    def download_telegram_file_to_tmp(file_url,filename):
        tmp=tempfile.NamedTemporaryFile(delete=False,suffix="."+ (safe_extension_from_filename(filename) or "tmp"))
        try:
    ...
    def handle_media_types(message):
        try:
            ...
            try:
                tf,file_url=telegram_file_info_and_url(BOT_TOKENS[bot_index],file_id)
            except Exception as e:
                bot_obj.send_message(message.chat.id,"Error obtaining file info from Telegram.");return
            try:
                tmp_path=download_telegram_file_to_tmp(file_url,filename)
            except Exception as e:
                bot_obj.send_message(message.chat.id,"Error downloading file from Telegram.");return
            job={"chat_id":message.chat.id,"lang":lang,"bot_index":bot_index,"file_path":tmp_path,"reply_to":message.message_id,"user_mode":get_user_send_mode(str(message.from_user.id)),"user_id_str":str(message.from_user.id)}
            pos=enqueue_transcription_job(job)
            if worker_busy.is_set() or pos>1:
                try: bot_obj.send_message(message.chat.id,"⏳ Fadlan sug daqiiqad. Faylkaaga waa la sugi doonaa oo la farsameyn doonaa marka faylasha hore la dhameeyo.",reply_to_message_id=message.message_id)
                except: pass
            else:
                try: bot_obj.send_message(message.chat.id,"🔄 Faylkaagu waxa uu bilaabayaa farsameynta hadda.",reply_to_message_id=message.message_id)
                except: pass
        ...
        try:
            ...
            if len(cleaned)>4000:
                if user_mode=="file":
                    ...
                    try:
                        action_usage[f"{call.message.chat.id}|{sent.message_id}|clean_up"]=0;action_usage[f"{call.message.chat.id}|{sent.message_id}|get_key_points"]=0
                    except: pass
                else:
                    try: bot_obj.delete_message(call.message.chat.id,status_msg.message_id)
                    except: pass
                    ...
                    try:
                        action_usage[f"{call.message.chat.id}|{last_sent.message_id}|clean_up"]=0;action_usage[f"{call.message.chat.id}|{last_sent.message_id}|get_key_points"]=0
                    except: pass
            else:
                try:
...
def webhook_root():
    ...
    if request.method=="POST":
        ...
        if content_type and content_type.startswith("application/json"):
            ...
            try:
                update=telebot.types.Update.de_json(payload);bots[bot_index_val].process_new_updates([update])
