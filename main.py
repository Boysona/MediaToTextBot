...
def set_bot_commands_on_startup():
    common_cmds=[telebot.types.BotCommand("start","👋 Get Started"),telebot.types.BotCommand("lang","Set File Language"),telebot.types.BotCommand("mode","Change result delivery mode"),telebot.types.BotCommand("help","Get help")]
    admin_cmd=telebot.types.BotCommand("admin","admin panel")
    for idx, bot_obj in enumerate(bots):
        try:
            bot_obj.set_my_commands(common_cmds)
        except Exception as e:
            logging.error("Failed to set default commands for bot %s: %s", idx, str(e))
        for admin_id in ADMIN_USER_IDS:
            try:
                bot_obj.set_my_commands(common_cmds+[admin_cmd], scope=telebot.types.BotCommandScopeChat(chat_id=int(admin_id)))
            except Exception as e:
                logging.error("Failed to set admin commands for admin %s on bot %s: %s", admin_id, idx, str(e))
