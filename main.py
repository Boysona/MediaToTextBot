...
def set_stt_user_lang(user_id: str, lang_code: str):
    users_collection.update_one({"user_id": str(user_id)}, {"$set": {"stt_language": lang_code}}, upsert=True)
...
def build_action_buttons(chat_id: int, message_id: int, include_get_key_points: bool):
    markup = InlineKeyboardMarkup()
    markup.add(InlineKeyboardButton("⭐️Clean transcript", callback_data=f"clean_up|{chat_id}|{message_id}"))
    if include_get_key_points:
...
def keep_sending_chat_action(bot_obj, chat_id, stop_event, action='typing', interval=4):
...
def handle_media_common(message, bot_obj, bot_token, bot_index=0):
    ...
    typing_thread = threading.Thread(target=keep_sending_chat_action, args=(bot_obj, message.chat.id, lambda: stop_typing["stop"]))
    typing_thread.start()
    try:
        ...
        try:
            ...
            if len(corrected_text) > 4000:
                if user_mode == "file":
                    ...
                    markup = build_action_buttons(message.chat.id, 0, len(corrected_text) > 1000)
                    sent = bot_obj.send_document(message.chat.id, f, reply_to_message_id=message.message_id, reply_markup=markup)
                    try:
                        buttons_markup = build_action_buttons(message.chat.id, sent.message_id, len(corrected_text) > 1000)
                        bot_obj.edit_message_reply_markup(message.chat.id, sent.message_id, reply_markup=buttons_markup)
                    except Exception: pass
                    try:
                    ...
                    try:
                        buttons_markup = build_action_buttons(message.chat.id, last_sent.message_id, len(corrected_text) > 1000)
                        bot_obj.edit_message_reply_markup(message.chat.id, last_sent.message_id, reply_markup=buttons_markup)
                    except Exception: pass
                    try:
                ...
                try:
                    buttons_markup = build_action_buttons(message.chat.id, sent_msg.message_id, len(corrected_text) > 1000)
                    bot_obj.edit_message_reply_markup(message.chat.id, sent_msg.message_id, reply_markup=buttons_markup)
                except Exception: pass
                try:
    ...
    def process_uploaded_file(chat_id_inner, lang_inner, path, bot_index_inner):
        try:
            ...
            try:
                ...
                try:
                    buttons_markup = build_action_buttons(chat_id_inner, sent_msg.message_id, len(corrected_text) > 1000)
                    bot_to_use.edit_message_reply_markup(chat_id_inner, sent_msg.message_id, reply_markup=buttons_markup)
                except Exception: pass
            except Exception:
...
def register_handlers(bot_obj, bot_token, bot_index):
    ...
    def get_key_points_callback(call):
        try:
            ...
            stop_typing = {"stop": False}
            typing_thread = threading.Thread(target=keep_sending_chat_action, args=(bot_obj, call.message.chat.id, lambda: stop_typing["stop"]))
            typing_thread.start()
            try:
            ...
            if not summary:
                ...
                try: bot_obj.send_message(call.message.chat.id, f"{summary}")
                except Exception: pass
        ...
        try:
            ...
            if len(cleaned) > 4000:
                if user_mode == "file":
                    ...
                    try: sent = bot_obj.send_document(call.message.chat.id, f, reply_to_message_id=call.message.message_id)
                    except Exception:
                        try: bot_obj.send_message(call.message.chat.id, "Failed to send cleaned file.")
                        except Exception: pass
                        return
                    try:
                ...
                try:
                    sent = bot_obj.send_message(call.message.chat.id, f"{cleaned}")
                    uid_key = str(chat_id_val)
                    user_transcriptions.setdefault(uid_key, {})[sent.message_id] = cleaned
                    threading.Thread(target=delete_transcription_later, args=(uid_key, sent.message_id), daemon=True).start()
                    action_usage[f"{call.message.chat.id}|{sent.message_id}|clean_up"] = 0
