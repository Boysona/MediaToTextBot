...
PREPEND_SILENCE_SEC=int(os.environ.get("PREPEND_SILENCE_SEC","3"))
...
for p in POSSIBLE_FFMPEG_PATHS:
    if not p: continue
    try:
        subprocess.run([p,"-version"],stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL,timeout=3);FFMPEG_BINARY=p;break
    except: continue
...
def get_stt_user_lang(user_id):
    ud=users_collection.find_one({"user_id":user_id});return ud.get("stt_language","en") if ud else "en"
...
def get_user_send_mode(user_id):
    ud=users_collection.find_one({"user_id":user_id});return ud.get("stt_send_mode","file") if ud else "file"
...
def build_result_mode_keyboard(prefix="result_mode"):
    m=InlineKeyboardMarkup(row_width=2);m.add(InlineKeyboardButton("📄 .txt file",callback_data=f"{prefix}|file"),InlineKeyboardButton("💬 Split messages",callback_data=f"{prefix}|split"));return m

def signed_upload_token(chat_id,lang_code,bot_index=0): return serializer.dumps({"chat_id":chat_id,"lang":lang_code,"bot_index":int(bot_index)})

def unsign_upload_token(token,max_age_seconds=3600): return serializer.loads(token,max_age=max_age_seconds)
...
def normalize_text_offline(text): return re.sub(r'\s+',' ',text).strip() if text else text
...
def safe_extension_from_filename(filename): return filename.rsplit(".",1)[-1].lower() if filename and "." in filename else ""
...
def split_wav_to_chunks(wav_path,out_dir,chunk_duration_sec):
    if FFMPEG_BINARY is None: raise RuntimeError("ffmpeg binary not found")
    os.makedirs(out_dir,exist_ok=True);pattern=os.path.join(out_dir,"chunk%03d.wav")
...
def split_wav_to_chunks_with_overlap(wav_path,out_dir,chunk_duration_sec,overlap_sec):
    ...
    total_dur=get_wav_duration(wav_path);step=chunk_duration_sec-overlap_sec
    if step<=0: raise ValueError("chunk_duration_sec must be greater than overlap_sec")
    num=max(1,math.ceil((total_dur-overlap_sec)/step));out_files=[]
    for i in range(num):
        start=round(i*step,3);out_name=os.path.join(out_dir,f"chunk{i:03d}.wav")
        cmd=[FFMPEG_BINARY,"-y","-ss",str(start),"-t",str(chunk_duration_sec),"-i",wav_path,"-ar",str(AUDIO_SAMPLE_RATE),"-ac",str(AUDIO_CHANNELS),out_name]
...
def merge_transcripts(results,max_overlap_words=20):
    ...
    for i in range(1,len(results)):
        ...
        max_ow=min(max_overlap_words,len(prev_words),len(cur_words));overlap=0
...
def transcribe_via_selected_service(input_path,lang_code):
    try:
        ...
        logging.exception("speech_recognition failed");raise RuntimeError("Speech recognition failed: "+str(e))
...
def attach_action_buttons(bot_obj, chat_id, message_id, text, cleaned_up=False, summarized=False):
    try:
        include_summarize = len(text) > 1000 if text else False
        m = InlineKeyboardMarkup()
        
        # Check usage from in-memory dictionary if not passed explicitly
        usage_key_clean = f"{chat_id}|{message_id}|clean_up"
        usage_key_summary = f"{chat_id}|{message_id}|get_key_points"
        
        # Get current usage status (1 means used/unavailable, 0 means available)
        cleaned_up = cleaned_up or action_usage.get(usage_key_clean, 0) >= 1
        summarized = summarized or action_usage.get(usage_key_summary, 0) >= 1

        clean_button_text = "⭐️Clean transcript" if not cleaned_up else "⭐️Cleaned (used)"
        clean_button_data = f"clean_up|{chat_id}|{message_id}" if not cleaned_up else "unavailable_used"
        m.add(InlineKeyboardButton(clean_button_text, callback_data=clean_button_data))

        if include_summarize:
            summary_button_text = "Get Summarize" if not summarized else "Get Summarize (used)"
            summary_button_data = f"get_key_points|{chat_id}|{message_id}" if not summarized else "unavailable_used"
            m.add(InlineKeyboardButton(summary_button_text, callback_data=summary_button_data))

        # Check if the message is already edited to avoid flooding the chat/API
        try:
            bot_obj.edit_message_reply_markup(chat_id, message_id, reply_markup=m)
        except:
            pass
            
        # Ensure usage is tracked in memory for the callback logic
        if not cleaned_up: action_usage[usage_key_clean] = 0
        else: action_usage[usage_key_clean] = 1 # Mark as used if explicitly set or already used
        if not summarized and include_summarize: action_usage[usage_key_summary] = 0
        elif summarized and include_summarize: action_usage[usage_key_summary] = 1 # Mark as used if explicitly set or already used

    except Exception as e:
        logging.exception(f"Error in attach_action_buttons: {e}")

def handle_media_common(message,bot_obj,bot_token,bot_index=0):
    user_id_str=str(message.from_user.id);chat_id_str=str(message.chat.id)
    ...
    try:
        ...
        try:
            ...
            try: text,used_service=transcribe_via_selected_service(tmpf.name,lang)
                ...
                if "ffmpeg" in error_msg.lower(): bot_obj.send_message(message.chat.id,"⚠️ Server error: ffmpeg not found or conversion failed. Contact admin @user31210",reply_to_message_id=message.message_id)
            ...
            if len(corrected_text)>4000:
                if user_mode=="file":
                    ...
                    sent_msg=bot_obj.send_document(message.chat.id,f,reply_to_message_id=message.message_id)
                else:
            ...
            if sent_msg:
                ...
                try:
                    transcriptions_collection.update_one({"chat_id":str(message.chat.id),"message_id":sent_msg.message_id},{"$set":{"original_text":corrected_text,"created_at":datetime.now(),"user_id":str(message.from_user.id)}},upsert=True)
                except: logging.exception("Failed to save transcription to MongoDB")
...
def broadcast_worker(target_type,msg_type,text,file_path,bot_index):
    bot_to_use=bots[0] if bot_index<0 or bot_index>=len(bots) else bots[bot_index];recipients=[]
    try:
        if target_type in ("all","users"):
            for u in users_collection.find({},{"user_id":1}):
                try: recipients.append(int(u["user_id"]))
                except:
                    try: recipients.append(int(str(u["user_id"])))
                    except: continue
    ...
    except: logging.exception("Failed to gather recipients for broadcast")
    sent_count=0;last_exception=None
    for r in recipients:
        try:
            ...
            last_exception=e;logging.exception("Broadcast send error")
        time.sleep(0.06)
    try:
        if file_path:
...
def admin_save_settings():
    ...
    if new_secret:
        try:
            settings_collection.update_one({"_id":"admin_panel_secret"},{"$set":{"value":new_secret}},upsert=True);ADMIN_PANEL_SECRET=new_secret
        except: pass
...
def admin_send_ads():
    secret=request.form.get("secret")
    if not secret or secret!=ADMIN_PANEL_SECRET: return abort(403)
    target=request.form.get("target") or "all";msg_type=request.form.get("msg_type") or "text"
    message_text=request.form.get("message_text") or "";bot_index_str=request.form.get("bot_index") or "0"
    ...
    if file:
        ...
        try:
            tmp.write(file.read());tmp.flush();tmp.close();tmp_path=tmp.name
        except:
            try: tmp.close();os.remove(tmp.name)
            except: pass
            tmp_path=None
    t=threading.Thread(target=broadcast_worker,args=(target,msg_type,message_text,tmp_path,bot_index),daemon=True);t.start()
...
def upload_large_file(token):
    ...
    def process_uploaded_file(chat_id_inner,lang_inner,path,bot_index_inner):
        try:
            ...
            try:
                uid_key=str(chat_id_inner);user_mode=get_user_send_mode(uid_key)
                if len(corrected_text)>4000:
                    if user_mode=="file":
                        fobj=io.BytesIO(corrected_text.encode("utf-8"));fobj.name="transcription.txt";sent_msg=bot_to_use.send_document(chat_id_inner,fobj,reply_markup=InlineKeyboardMarkup())
                    else:
                        chunks=split_text_into_chunks(corrected_text,limit=4096);last_sent=None
                        for idx,chunk in enumerate(chunks):
                ...
                if sent_msg:
                    try: attach_action_buttons(bot_to_use,chat_id_inner,sent_msg.message_id,corrected_text)
                    except: pass
                    try:
                        transcriptions_collection.update_one({"chat_id":str(chat_id_inner),"message_id":sent_msg.message_id},{"$set":{"original_text":corrected_text,"created_at":datetime.now(),"user_id":str(chat_id_inner)}},upsert=True)
                    except: logging.exception("Failed to save transcription to MongoDB after web upload")
            except:
...
def ask_gemini(text,instruction,timeout=REQUEST_TIMEOUT_GEMINI):
    ...
    payload={"contents":[{"parts":[{"text":instruction},{"text":text}]}]};headers={"Content-Type":"application/json"}
...
def cleanup_old_transcriptions():
    while True:
        try:
            cleanup_duration = timedelta(days=CLEANUP_DAYS)
            days_ago = datetime.now() - cleanup_duration
            
            result = transcriptions_collection.delete_many({"created_at": {"$lt": days_ago}})
            logging.info(f"Cleanup finished: Deleted {result.deleted_count} old transcriptions (Retention: {CLEANUP_DAYS} days).")
        except Exception as e:
            logging.error(f"Error during cleanup of old transcriptions: {e}")
        time.sleep(12 * 60 * 60) # Runs every 12 hours

def start_cleanup_scheduler():
    t=threading.Thread(target=cleanup_old_transcriptions,daemon=True);t.start()
...
def register_handlers(bot_obj,bot_token,bot_index):
    ...
    @bot_obj.callback_query_handler(lambda c: c.data == "unavailable_used")
    def unavailable_handler(call):
        try:
            bot_obj.answer_callback_query(call.id, "unavailable", show_alert=True)
        except:
            pass
    ...
    def get_key_points_callback(call):
        try:
            ...
            transcript_doc=transcriptions_collection.find_one({"chat_id":str(chat_id_val),"message_id":msg_id})
            if not transcript_doc or not transcript_doc.get("original_text"):
                bot_obj.answer_callback_query(call.id,"unavailable",show_alert=True);return
            stored=transcript_doc["original_text"]
            
            if usage>=1:
                bot_obj.answer_callback_query(call.id,"unavailable",show_alert=True);return
            action_usage[usage_key]=usage+1
            
            # Update the button to show it's used
            attach_action_buttons(bot_obj, chat_id_val, msg_id, stored, summarized=True)
            ...
            try:
                lang=get_stt_user_lang(str(call.from_user.id)) or "en"
    ...
    def clean_up_callback(call):
        try:
            ...
            # Update the button to show it's used
            attach_action_buttons(bot_obj, chat_id_val, msg_id, stored, cleaned_up=True)
            ...
            if len(cleaned)>4000:
                if user_mode=="file":
                    ...
                    sent_msg=bot_obj.send_document(call.message.chat.id,f,reply_to_message_id=call.message.message_id)
                else:
                    try: bot_obj.delete_message(call.message.chat.id,status_msg.message_id)
                    except: pass
            ...
            if sent_msg:
                # Muhiim: Haddii la helo fariin la soo diray (ma ahan mid la *edit*-gareeyay)
                if sent_msg.message_id != msg_id:
                    # Tani waxay dhacdaa haddii natiijada la soo diro split messages ama file ahaan. 
                    # Keydi fariinta nadiifka ah fariinta cusub (haddii aad rabto in fariinta cusub ay yeelato batoonada *clean up* iyo *summarize*).
                    transcriptions_collection.update_one({"chat_id":str(call.message.chat.id),"message_id":sent_msg.message_id},{"$set":{"original_text":cleaned,"created_at":datetime.now(),"user_id":str(call.from_user.id)}},upsert=True)
                    # Ku dar batoonada fariinta cusub, laakiin hadda waa inaan batoonka *clean up* aan ka saarno *button*-kaas
                    # maxaa yeelay waa la isticmaalay.
                    attach_action_buttons(bot_obj,call.message.chat.id,sent_msg.message_id,cleaned)
                else:
                    # Waa fariin la *edit*-gareeyay, waxaan hore u *edit*-gareynay batoonada.
                    pass
...
def set_webhook_route():
    ...
    for idx,bot_obj in enumerate(bots):
        try:
            url=WEBHOOK_BASE.rstrip("/")+f"/?bot_index={idx}";bot_obj.delete_webhook();time.sleep(0.2);bot_obj.set_webhook(url=url)
...
def set_bot_info_and_startup(): set_webhook_on_startup(); start_cleanup_scheduler()
...
if __name__=="__main__":
    ...
    except: logging.exception("Failed during startup")
