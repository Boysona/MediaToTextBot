import os,logging,requests,telebot,json,threading,time,io,tempfile,glob,re,wave,subprocess
...
ADMIN_USER_IDS=[]
for p in [x.strip() for x in raw_admins.split(",") if x.strip()]:
    try: ADMIN_USER_IDS.append(int(p))
    except: pass
...
db=client[DB_NAME]
users_collection=db["users"];groups_collection=db["groups"];settings_collection=db["settings"]

app=Flask(__name__)
bots=[telebot.TeleBot(t,threaded=True,parse_mode='HTML') for t in BOT_TOKENS]
...
CODE_TO_LABEL={c:lbl for lbl,c in LANG_OPTIONS}
LABEL_TO_CODE={lbl:c for lbl,c in LANG_OPTIONS}
STT_LANGUAGES={lbl.split(" ",1)[-1]:{"code":c,"emoji":lbl.split(" ",1)[0],"native":lbl.split(" ",1)[-1]} for lbl,c in LANG_OPTIONS}
...
def check_subscription(uid,bot_obj):
    if not REQUIRED_CHANNEL or not REQUIRED_CHANNEL.strip(): return True
    try:
        m=bot_obj.get_chat_member(REQUIRED_CHANNEL,uid)
...
def send_subscription_message(chat_id,bot_obj):
    ...
    try:
        ch=bot_obj.get_chat(chat_id)
        if ch.type!='private': return
    except Exception:
        return
    try:
        mk=InlineKeyboardMarkup()
        mk.add(InlineKeyboardButton("Click here to join the Group",url=f"https://t.me/{REQUIRED_CHANNEL.lstrip('@')}"))
        bot_obj.send_message(chat_id,"🔒 Access Locked You cannot use this bot until you join the Group.",reply_markup=mk)
    except Exception:
        pass

def update_user_activity(uid):
    now=datetime.now()
    users_collection.update_one({"user_id":str(uid)},{"$set":{"last_active":now},"$setOnInsert":{"first_seen":now,"stt_conversion_count":0}},upsert=True)

def increment_processing_count(uid,svc):
    users_collection.update_one({"user_id":str(uid)},{"$inc":{f"{svc}_conversion_count":1}})

def get_stt_user_lang(uid):
    u=users_collection.find_one({"user_id":str(uid)})
    return u.get("stt_language","en") if u else "en"

def set_stt_user_lang(uid,lang_code):
    users_collection.update_one({"user_id":str(uid)},{"$set":{"stt_language":lang_code}},upsert=True)

def get_user_send_mode(uid):
    u=users_collection.find_one({"user_id":str(uid)})
    return u.get("stt_send_mode","file") if u else "file"

def set_user_send_mode(uid,mode):
    if mode not in ("file","split"): mode="file"
    users_collection.update_one({"user_id":str(uid)},{"$set":{"stt_send_mode":mode}},upsert=True)

def save_pending_media(uid,media_type,data):
    with memory_lock:
        in_memory_data["pending_media"][str(uid)]={"media_type":media_type,"data":data,"saved_at":datetime.now()}

def pop_pending_media(uid):
    with memory_lock:
        return in_memory_data["pending_media"].pop(str(uid),None)

def delete_transcription_later(uid,msg_id):
    time.sleep(86400)
    with memory_lock:
        if uid in user_transcriptions and msg_id in user_transcriptions[uid]:
...
def build_lang_keyboard(prefix,row_width=3,msg_id=None):
    mk=InlineKeyboardMarkup(row_width=row_width)
    btns=[InlineKeyboardButton(lbl,callback_data=f"{prefix}|{code}|{msg_id}" if msg_id else f"{prefix}|{code}") for lbl,code in LANG_OPTIONS]
    for i in range(0,len(btns),row_width): mk.add(*btns[i:i+row_width])
    return mk

def build_result_mode_keyboard(pref="result_mode"):
    mk=InlineKeyboardMarkup(row_width=2)
    mk.add(InlineKeyboardButton("📄 .txt file",callback_data=f"{pref}|file"),InlineKeyboardButton("💬 Split messages",callback_data=f"{pref}|split"))
    return mk

def signed_upload_token(chat_id,lang_code,bot_index=0):
...
def extract_key_points_offline(text,max_points=6):
    ...
    if not words: return "\n".join(f"- {s}" for s in sents[:max_points])
    freq=Counter(words)
    scores=[(sum(freq.get(w,0) for w in re.findall(r'\w+',s.lower())),s) for s in sents]
    scores.sort(key=lambda x:x[0],reverse=True)
    top=sorted(scores[:max_points],key=lambda x:sents.index(x[1]))
    return "\n".join(f"- {s}" for _,s in top)

def safe_extension_from_filename(fn):
    return fn.rsplit(".",1)[-1].lower() if fn and "." in fn else ""

def telegram_file_info_and_url(token,file_id):
    url=f"https://api.telegram.org/bot{token}/getFile?file_id={file_id}"
    r=requests.get(url,timeout=REQUEST_TIMEOUT_TELEGRAM); r.raise_for_status()
    fp=r.json().get("result",{}).get("file_path")
    return type('D',(),{'file_path':fp})(),f"https://api.telegram.org/file/bot{token}/{fp}"

def convert_to_wav(inp,out):
    if FFMPEG_BINARY is None: raise RuntimeError("ffmpeg binary not found")
    subprocess.run([FFMPEG_BINARY,"-y","-i",inp,"-ar",str(AUDIO_SAMPLE_RATE),"-ac",str(AUDIO_CHANNELS),out],stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL,check=True)

def get_wav_duration(p):
    with wave.open(p,'rb') as wf: return wf.getnframes()/float(wf.getframerate())

def prepend_silence_to_wav(orig,out,sil_sec):
    if FFMPEG_BINARY is None: raise RuntimeError("ffmpeg binary not found")
    tmpd=os.path.dirname(out) or tempfile.gettempdir()
    silence=os.path.join(tmpd,f"silence_{int(time.time()*1000)}.wav")
    subprocess.run([FFMPEG_BINARY,"-y","-f","lavfi","-i",f"anullsrc=channel_layout=mono:sample_rate={AUDIO_SAMPLE_RATE}","-t",str(sil_sec),"-ar",str(AUDIO_SAMPLE_RATE),"-ac",str(AUDIO_CHANNELS),silence],stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL,check=True)
    subprocess.run([FFMPEG_BINARY,"-y","-i",silence,"-i",orig,"-filter_complex","[0:0][1:0]concat=n=2:v=0:a=1[out]","-map","[out]",out],stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL,check=True)
    try: os.remove(silence)
    except: pass

def split_wav_to_chunks(wav,out_dir,chunk_sec):
    if FFMPEG_BINARY is None: raise RuntimeError("ffmpeg binary not found")
    os.makedirs(out_dir,exist_ok=True)
    pattern=os.path.join(out_dir,"chunk%03d.wav")
    subprocess.run([FFMPEG_BINARY,"-y","-i",wav,"-ar",str(AUDIO_SAMPLE_RATE),"-ac",str(AUDIO_CHANNELS),"-f","segment","-segment_time",str(chunk_sec),"-reset_timestamps","1",pattern],stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL,check=True)
...
def recognize_chunk_file(recognizer,file_path,language):
    ...
    for attempt in range(1,RECOGNITION_MAX_RETRIES+1):
        try:
            ...
            use=prep if prep else file_path
            with sr.AudioFile(use) as src:
                try: recognizer.adjust_for_ambient_noise(src,duration=AMBIENT_CALIB_SEC)
                except: pass
...
def transcribe_file_with_speech_recognition(in_path,lang_code):
    tmpdir=tempfile.mkdtemp(prefix="stt_")
    try:
        base_wav=os.path.join(tmpdir,"converted.wav")
        try: convert_to_wav(in_path,base_wav)
        ...
        def tchunk(p): return recognize_chunk_file(sr.Recognizer(),p,lang_code)
        with ThreadPoolExecutor(max_workers=TRANSCRIBE_MAX_WORKERS) as ex:
            results=list(ex.map(tchunk,chunk_files))
...
def transcribe_via_selected_service(path,lang):
    try: return transcribe_file_with_speech_recognition(path,lang),"speech_recognition"
    except Exception as e:
        logging.exception("speech_recognition failed")
        raise RuntimeError("speech_recognition failed: "+str(e))
...
def make_action_markup(chat_id,message_id,text_len):
    mk=InlineKeyboardMarkup()
    btns=[InlineKeyboardButton("⭐️Clean transcript",callback_data=f"clean_up|{chat_id}|{message_id}")]
    if text_len>1000: btns.append(InlineKeyboardButton("Get Summarize",callback_data=f"get_key_points|{chat_id}|{message_id}"))
    for b in btns: mk.add(b)
    try:
...
def store_transcription_and_schedule_delete(chat_id,message_id,text):
    uid=str(chat_id)
    user_transcriptions.setdefault(uid,{})[message_id]=text
    try: threading.Thread(target=delete_transcription_later,args=(uid,message_id),daemon=True).start()
    except: pass

def telegram_send_action_typing(bot_obj,chat_id):
    try: bot_obj.send_chat_action(chat_id,'typing')
    except: pass

def handle_media_common(message,bot_obj,bot_token,bot_index=0):
    ustr=str(message.from_user.id); update_user_activity(message.from_user.id)
    if message.chat.type=='private' and ustr not in ADMIN_USER_IDS and not check_subscription(message.from_user.id,bot_obj):
        send_subscription_message(message.chat.id,bot_obj); return
    file_id=file_size=filename=None
    if message.voice: file_id=message.voice.file_id; file_size=message.voice.file_size; filename="voice.ogg"
    elif message.audio: file_id=message.audio.file_id; file_size=message.audio.file_size; filename=getattr(message.audio,"file_name","audio")
    elif message.video: file_id=message.video.file_id; file_size=message.video.file_size; filename=getattr(message.video,"file_name","video.mp4")
    ...
    telegram_send_action_typing(bot_obj,message.chat.id)
    try:
        ...
        try:
            ...
            try:
                ...
                em=str(e); logging.exception("Error during transcription")
                if "ffmpeg" in em.lower():
                ...
                elif is_transcoding_like_error(em):
                    ...
                    bot_obj.send_message(message.chat.id,f"Error during transcription: {em}",reply_to_message_id=message.message_id)
                return
            ...
            if len(corrected_text)>4000:
                if user_mode=="file":
                    ...
                    try:
                        bot_obj.edit_message_reply_markup(message.chat.id,sent.message_id,reply_markup=make_action_markup(message.chat.id,sent.message_id,len(corrected_text)))
                    except: pass
                    try: store_transcription_and_schedule_delete(message.chat.id,sent.message_id,corrected_text)
                    except: pass
                else:
                    ...
                    try: bot_obj.edit_message_reply_markup(message.chat.id,last_sent.message_id,reply_markup=make_action_markup(message.chat.id,last_sent.message_id,len(corrected_text)))
                    except: pass
                    try: store_transcription_and_schedule_delete(message.chat.id,last_sent.message_id,corrected_text)
                    except: pass
            else:
                markup=make_action_markup(message.chat.id,message.message_id,len(corrected_text))
                sent_msg=bot_obj.send_message(message.chat.id,corrected_text or "⚠️ Warning Make sure the voice is clear or speaking in the language you Choosed.",reply_to_message_id=message.message_id,reply_markup=markup)
                try: store_transcription_and_schedule_delete(message.chat.id,sent_msg.message_id,corrected_text)
                except: pass
            increment_processing_count(ustr,"stt")
        finally:
            try: os.remove(tmpf.name)
            except: pass
    except Exception as e:
        em=str(e); logging.exception("Error in transcription process")
        if is_transcoding_like_error(em):
            bot_obj.send_message(message.chat.id,"⚠️ Transcription error: file is not audible. Please send a different file.",reply_to_message_id=message.message_id)
        else:
            bot_obj.send_message(message.chat.id,f"Error during transcription: {em}",reply_to_message_id=message.message_id)
...
def broadcast_worker(target_type,msg_type,text,file_path,bot_index):
    ...
    for r in recipients:
        try:
            ...
            last_exc=e; logging.exception("Broadcast send error")
        time.sleep(0.06)
    try:
        if file_path:
    ...
    logging.info("Broadcast finished, sent=%s, last_exc=%s",sent,str(last_exc))
...
def admin_save_settings():
    ...
    if new_secret:
        try: settings_collection.update_one({"_id":"admin_panel_secret"},{"$set":{"value":new_secret}},upsert=True); ADMIN_PANEL_SECRET=new_secret
        except: pass
...
def admin_send_ads():
    secret=request.form.get("secret")
    if not secret or secret!=ADMIN_PANEL_SECRET: return abort(403)
    target=request.form.get("target") or "all"; msg_type=request.form.get("msg_type") or "text"; message_text=request.form.get("message_text") or ""
    try: bot_index=int(request.form.get("bot_index") or "0")
    except: bot_index=0
    file=request.files.get("media_file"); tmp_path=None
    if file:
        ...
        try: tmp.write(file.read()); tmp.flush(); tmp.close(); tmp_path=tmp.name
        except:
            try: tmp.close(); os.remove(tmp.name)
            except: pass
            tmp_path=None
    threading.Thread(target=broadcast_worker,args=(target,msg_type,message_text,tmp_path,bot_index),daemon=True).start()
...
def upload_large_file(token):
    ...
    def process_uploaded_file(c_id,lang_inner,path,bot_idx):
        try:
            bot_to_use=bots[bot_idx] if 0<=bot_idx<len(bots) else bots[0]
            try: text,used=transcribe_via_selected_service(path,lang_inner)
            except Exception:
                try: bot_to_use.send_message(c_id,"Error occurred while transcribing the uploaded file.")
                except: pass
                return
            corrected_text=normalize_text_offline(text); sent_msg=None
            try:
                uid_key=str(c_id); user_mode=get_user_send_mode(uid_key)
                if len(corrected_text)>4000:
                    if user_mode=="file":
                        fobj=io.BytesIO(corrected_text.encode("utf-8")); fobj.name="transcription.txt"; sent_msg=bot_to_use.send_document(c_id,fobj)
                    else:
                        last=None
                        for idx,chunk in enumerate(split_text_into_chunks(corrected_text,limit=4096)):
                            last = bot_to_use.send_message(c_id,chunk) if idx==0 else bot_to_use.send_message(c_id,chunk)
                        sent_msg=last
                else:
                    sent_msg=bot_to_use.send_message(c_id,corrected_text or "No transcription text was returned.")
                try: bot_to_use.edit_message_reply_markup(c_id,sent_msg.message_id,reply_markup=make_action_markup(c_id,sent_msg.message_id,len(corrected_text)))
                except: pass
            except Exception:
                try: bot_to_use.send_message(c_id,"Error sending transcription message. The transcription completed but could not be delivered as a message.")
                except: pass
                return
            try: store_transcription_and_schedule_delete(c_id,sent_msg.message_id,corrected_text); increment_processing_count(str(c_id),"stt")
            except: pass
        finally:
            try: os.remove(path)
            except: pass
...
def ask_gemini(text,instruction,timeout=REQUEST_TIMEOUT_GEMINI):
    ...
    res=resp.json()
    if "candidates" in res and isinstance(res["candidates"],list) and len(res["candidates"])>0:
        try: return res['candidates'][0]['content']['parts'][0]['text']
        except Exception: return json.dumps(res['candidates'][0])
...
def register_handlers(bot_obj,bot_token,bot_index):
    ...
    def handle_admin(message):
        try:
            ...
            mk=InlineKeyboardMarkup(); mk.add(InlineKeyboardButton("Open Admin Panel",url=link))
            bot_obj.send_message(message.chat.id,report_text,parse_mode='HTML',reply_markup=mk)
        except Exception:
    ...
    def on_stt_language_select(call):
        try:
            ...
            set_stt_user_lang(uid,lang_code); bot_obj.answer_callback_query(call.id,f"✅ Language set: {lang_label}")
            try: bot_obj.delete_message(call.message.chat.id,call.message.message_id)
            except: pass
        except Exception:
    ...
    def on_result_mode_select(call):
        try:
            uid=str(call.from_user.id); _,mode=call.data.split("|",1); set_user_send_mode(uid,mode)
    ...
    def handle_new_chat_members(message):
        try:
            if message.new_chat_members[0].id==bot_obj.get_me().id:
                ...
                groups_collection.update_one({'_id':group_data['_1'] if False else group_data['_id']},{'$set':group_data},upsert=True)
    ...
    def get_key_points_callback(call):
        try:
            ...
            if not stored:
                bot_obj.answer_callback_query(call.id,"Get Summarize unavailable (maybe expired)",show_alert=True); return
            bot_obj.answer_callback_query(call.id,"Generating..."); telegram_send_action_typing(bot_obj,call.message.chat.id)
            try:
                ...
                summary=""
            if not summary:
                try: bot_obj.send_message(call.message.chat.id,"No Summary returned.")
                except: pass
            else:
                try: bot_obj.send_message(call.message.chat.id,summary)
                except: pass
        except Exception:
    ...
    def clean_up_callback(call):
        try:
            ...
            if not stored:
                bot_obj.answer_callback_query(call.id,"Clean up unavailable (maybe expired)",show_alert=True); return
            bot_obj.answer_callback_query(call.id,"Cleaning up..."); telegram_send_action_typing(bot_obj,call.message.chat.id)
            try:
                lang=get_stt_user_lang(str(chat_id_val)) or "en"
                ...
                cleaned=""
            if not cleaned:
                try: bot_obj.send_message(call.message.chat.id,"No cleaned text returned.")
                except: pass
                return
            ...
            if len(cleaned)>4000:
                if user_mode=="file":
                    ...
                    try: bot_obj.send_document(call.message.chat.id,f,reply_to_message_id=call.message.message_id)
                    except: pass
                else:
                    try:
                        last_sent=None
                        for idx,chunk in enumerate(split_text_into_chunks(cleaned,limit=4096)):
                        ...
                        if last_sent:
                            store_transcription_and_schedule_delete(uid_key,last_sent.message_id,cleaned)
                ...
                try:
                    bot_obj.send_message(call.message.chat.id,cleaned)
                    store_transcription_and_schedule_delete(uid_key,call.message.message_id,cleaned)
                    action_usage[f"{call.message.chat.id}|{call.message.message_id}|clean_up"]=0
                    action_usage[f"{call.message.chat.id}|{call.message.message_id}|get_key_points"]=0
                except Exception:
                    pass
        except Exception:
...
def set_webhook_route():
    ...
    for idx,bot_obj in enumerate(bots):
        try:
            ...
            bot_obj.delete_webhook(); time.sleep(0.2); bot_obj.set_webhook(url=url)
            results.append({"index":idx,"url":url,"status":"ok"})
        except Exception as e:
            logging.error(f"Failed to set webhook for bot {idx}: {e}"); results.append({"index":idx,"error":str(e)})
...
def set_webhook_on_startup():
    for idx,bot_obj in enumerate(bots):
        try:
            ...
            url=WEBHOOK_BASE.rstrip("/")+f"/?bot_index={idx}"; bot_obj.set_webhook(url=url)
...
if __name__=="__main__":
    try:
        ...
        try: client.admin.command('ping'); logging.info("Successfully connected to MongoDB!")
