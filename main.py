...
def typing_processing(bot_obj, chat_id, stop_event):
...
def make_action_markup(corrected_text: str, chat_id: int, msg_id: int):
    markup = InlineKeyboardMarkup()
    buttons = [InlineKeyboardButton("⭐️Clean transcript", callback_data=f"clean_up|{chat_id}|{msg_id}")]
    if len(corrected_text) > 1000: buttons.append(InlineKeyboardButton("Get Summarize", callback_data=f"get_key_points|{chat_id}|{msg_id}"))
...
def handle_media_common(message, bot_obj, bot_token, bot_index=0):
    ...
    typing_thread = threading.Thread(target=typing_processing, args=(bot_obj, message.chat.id, lambda: stop_typing["stop"]))
    typing_thread.start()
    try:
        ...
        try:
            ...
            if len(corrected_text) > 4000:
                if user_mode == "file":
                    ...
                    try:
                        markup = make_action_markup(corrected_text, message.chat.id, sent.message_id)
                    ...
                    for idx, chunk in enumerate(chunks):
                        if idx == 0:
                            if len(chunks) == 1:
                                markup = make_action_markup(corrected_text, message.chat.id, 0)
                                last_sent = bot_obj.send_message(message.chat.id, chunk, reply_to_message_id=message.message_id, reply_markup=markup)
                            else:
                                last_sent = bot_obj.send_message(message.chat.id, chunk, reply_to_message_id=message.message_id)
                        else:
                            if idx == len(chunks) - 1:
                                last_sent = bot_obj.send_message(message.chat.id, chunk, reply_markup=make_action_markup(corrected_text, message.chat.id, 0))
                            else:
                                last_sent = bot_obj.send_message(message.chat.id, chunk)
                    try:
                        if last_sent:
                            mid = last_sent.message_id
                            try:
                                if mid:
                                    markup = make_action_markup(corrected_text, message.chat.id, mid)
                                    bot_obj.edit_message_reply_markup(message.chat.id, mid, reply_markup=markup)
                            except Exception: pass
                    except Exception: pass
                    try:
                ...
                uid = message.chat.id
                markup = make_action_markup(corrected_text, message.chat.id, 0)
                sent_msg = bot_obj.send_message(message.chat.id, corrected_text or "⚠️ Warning Make sure the voice is clear or speaking in the language you Choosed.", reply_to_message_id=message.message_id, reply_markup=None)
                try:
                    markup = make_action_markup(corrected_text, message.chat.id, sent_msg.message_id)
...
def register_handlers(bot_obj, bot_token, bot_index):
    ...
    def get_key_points_callback(call):
        try:
            ...
            stop_typing = {"stop": False}
            typing_thread = threading.Thread(target=typing_processing, args=(bot_obj, call.message.chat.id, lambda: stop_typing["stop"]))
            typing_thread.start()
            try:
