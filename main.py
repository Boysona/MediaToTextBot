import os,json,time,threading,tempfile,glob,subprocess,io,re,logging
...
CODE_TO_LABEL={c:l for l,c in LANG_OPTIONS}
LABEL_TO_CODE={l:c for l,c in LANG_OPTIONS}
...
if FFMPEG_BINARY is None: logging.warning("ffmpeg not found")

def check_subscription(user_id,bot_obj):
    if not REQUIRED_CHANNEL or not REQUIRED_CHANNEL.strip(): return True
    try:
        m=bot_obj.get_chat_member(REQUIRED_CHANNEL,user_id)
...
def send_subscription_message(chat_id,bot_obj):
    ...
    try:
        ...
        kb.add(InlineKeyboardButton("Click here to join the Group",url=f"https://t.me/{REQUIRED_CHANNEL.lstrip('@')}"))
        bot_obj.send_message(chat_id,"🔒 Access Locked You cannot use this bot until you join the Group.",reply_markup=kb)
    except: pass
...
def is_transcoding_like_error(msg):
    ...
    m=msg.lower()
    for ch in ["transcoding failed","file does not appear to contain audio","text/html","html document","unsupported media type","could not decode"]:
        if ch in m: return True
    return False

def build_lang_keyboard(prefix,row_width=3,message_id=None):
    kb=InlineKeyboardMarkup(row_width=row_width)
    btns=[InlineKeyboardButton(label,callback_data=f"{prefix}|{code}|{message_id}" if message_id else f"{prefix}|{code}") for label,code in LANG_OPTIONS]
    for i in range(0,len(btns),row_width): kb.add(*btns[i:i+row_width])
    return kb

def build_result_mode_keyboard(prefix="result_mode"):
    kb=InlineKeyboardMarkup(row_width=2)
    kb.add(InlineKeyboardButton("📄 .txt file",callback_data=f"{prefix}|file"),InlineKeyboardButton("💬 Split messages",callback_data=f"{prefix}|split"))
    return kb
...
def keep_typing(bot_obj,chat_id,stop_event):
...
def prepend_silence_to_wav(original_wav,output_wav,silence_sec):
    if FFMPEG_BINARY is None: raise RuntimeError("ffmpeg binary not found")
    tmpdir=os.path.dirname(output_wav) or tempfile.gettempdir()
    silence_file=os.path.join(tmpdir,f"sil_{int(time.time()*1000)}.wav")
...
def transcribe_file_with_speech_recognition(input_file_path,language_code):
    ...
    try:
        ...
        def tfn(p): return recognize_chunk_file(sr.Recognizer(),p,language_code)
        with ThreadPoolExecutor(max_workers=TRANSCRIBE_MAX_WORKERS) as ex: results=list(ex.map(tfn,chunk_files))
...
def transcribe_via_selected_service(input_path,lang_code):
    ...
    except Exception as e: raise RuntimeError("Transcription failed: "+str(e))
...
def helper_attach_buttons_and_store(bot_obj,chat_id,message_id,text,reply_to=None):
    kb=InlineKeyboardMarkup()
    kb.add(InlineKeyboardButton("⭐️Clean transcript",callback_data=f"clean_up|{chat_id}|{message_id}"))
    if len(text)>1000: kb.add(InlineKeyboardButton("Get Summarize",callback_data=f"get_key_points|{chat_id}|{message_id}"))
    try: bot_obj.edit_message_reply_markup(chat_id,message_id,reply_markup=kb)
    except: pass
    try:
        uid=str(chat_id)
        user_transcriptions.setdefault(uid,{})[message_id]=text
        threading.Thread(target=delete_transcription_later,args=(uid,message_id),daemon=True).start()
    except: pass
    try:
...
def handle_media_common(message,bot_obj,bot_token,bot_index=0):
    ...
    stop_typing={"stop":False}
    t=threading.Thread(target=keep_typing,args=(bot_obj,message.chat.id,lambda:stop_typing["stop"]))
    t.start()
    try:
        ...
        try:
            ...
            try: text,used_service=transcribe_via_selected_service(tmpf.name,lang)
                ...
                em=str(e);logging.exception("transcription")
                if "ffmpeg" in em.lower(): bot_obj.send_message(message.chat.id,"⚠️ Server error: ffmpeg not found or conversion failed. Contact admin.",reply_to_message_id=message.message_id)
                elif is_transcoding_like_error(em): bot_obj.send_message(message.chat.id,"⚠️ Transcription error: file is not audible. Please send a different file.",reply_to_message_id=message.message_id)
                else: bot_obj.send_message(message.chat.id,f"Error during transcription: {em}",reply_to_message_id=message.message_id)
                return
            ...
            if len(corrected_text)>4000:
                if user_mode=="file":
                    ...
                    helper_attach_buttons_and_store(bot_obj,message.chat.id,sent.message_id,corrected_text,reply_to=message.message_id)
                else:
                    chunks=split_text_into_chunks(corrected_text,limit=4096)
                    last=None
                    for idx,chunk in enumerate(chunks):
                        if idx==0: last=bot_obj.send_message(message.chat.id,chunk,reply_to_message_id=message.message_id)
                        else: last=bot_obj.send_message(message.chat.id,chunk)
                    helper_attach_buttons_and_store(bot_obj,message.chat.id,last.message_id,corrected_text)
            else:
                ...
                helper_attach_buttons_and_store(bot_obj,message.chat.id,sent_msg.message_id,corrected_text,reply_to=message.message_id)
            increment_processing_count(str(message.from_user.id),"stt")
        finally:
            try: os.remove(tmpf.name)
            except: pass
    except Exception as e:
        em=str(e);logging.exception("media")
        if is_transcoding_like_error(em): bot_obj.send_message(message.chat.id,"⚠️ Transcription error: file is not audible. Please send a different file.",reply_to_message_id=message.message_id)
        else: bot_obj.send_message(message.chat.id,f"Error during transcription: {em}",reply_to_message_id=message.message_id)
    finally:
        stop_typing["stop"]=True
        t.join()
...
def broadcast_worker(target_type,msg_type,text,file_path,bot_index):
    ...
    except: logging.exception("gather")
    ...
    for r in recipients:
        try:
            ...
            last_exc=e;logging.exception("send")
        time.sleep(0.06)
    try:
        if file_path:
    ...
    def process_uploaded_file(chat_id_inner,lang_inner,path,bot_index_inner):
        try:
            ...
            try:
                ...
                if len(corrected_text)>4000:
                    if user_mode=="file":
                        fobj=io.BytesIO(corrected_text.encode("utf-8"));fobj.name="transcription.txt"
                        sent_msg=bot_to_use.send_document(chat_id_inner,fobj,reply_markup=kb)
                    else:
                        ...
                        sent_msg=last
                else: sent_msg=bot_to_use.send_message(chat_id_inner,corrected_text or "No transcription text was returned.",reply_markup=kb)
                helper_attach_buttons_and_store(bot_to_use,chat_id_inner,sent_msg.message_id,corrected_text)
            except:
...
def ask_gemini(text,instruction,timeout=REQUEST_TIMEOUT_GEMINI):
    ...
    r=resp.json()
    if "candidates" in r and isinstance(r["candidates"],list) and len(r["candidates"])>0:
        try: return r['candidates'][0]['content']['parts'][0]['text']
        except: return json.dumps(r['candidates'][0])
    return json.dumps(r)
...
def register_handlers(bot_obj,bot_token,bot_index):
    ...
    def start_handler(message):
        ...
        except: logging.exception("start")
    ...
    def handle_admin(message):
        try:
            ...
            kb=InlineKeyboardMarkup();kb.add(InlineKeyboardButton("Open Admin Panel",url=link))
            bot_obj.send_message(message.chat.id,report_text,parse_mode='HTML',reply_markup=kb)
        except:
            logging.exception("admin");bot_obj.send_message(message.chat.id,"An error occurred while fetching admin stats.")
    ...
    def handle_new_chat_members(message):
        ...
        except: logging.exception("new_member")
    ...
    def handle_media_types(message):
        ...
        except: logging.exception("media_types")
    ...
    def get_key_points_callback(call):
        try:
            ...
            stop_typing={"stop":False}
            t=threading.Thread(target=keep_typing,args=(bot_obj,call.message.chat.id,lambda:stop_typing["stop"]))
            t.start()
            try:
            ...
            stop_typing["stop"]=True; t.join()
            if not summary: bot_obj.send_message(call.message.chat.id,"No Summary returned.")
            else: bot_obj.send_message(call.message.chat.id,summary)
        except: logging.exception("get_key_points")
    ...
    def clean_up_callback(call):
        try:
            ...
            stop_typing["stop"]=True; t.join()
            if not cleaned:
                bot_obj.send_message(call.message.chat.id,"No cleaned text returned.");return
            ...
            if len(cleaned)>4000:
                if user_mode=="file":
                    ...
                    last=None
                    for idx,chunk in enumerate(chunks):
                        if idx==0: last=bot_obj.send_message(call.message.chat.id,chunk,reply_to_message_id=call.message.message_id)
                        else: last=bot_obj.send_message(call.message.chat.id,chunk)
                    user_transcriptions.setdefault(uid_key,{})[last.message_id]=cleaned
                    threading.Thread(target=delete_transcription_later,args=(uid_key,last.message_id),daemon=True).start()
                    action_usage[f"{call.message.chat.id}|{last.message_id}|clean_up"]=0
                    action_usage[f"{call.message.chat.id}|{last.message_id}|get_key_points"]=0
            else:
                sent=bot_obj.send_message(call.message.chat.id,cleaned,reply_to_message_id=call.message.message_id)
                user_transcriptions.setdefault(uid_key,{})[sent.message_id]=cleaned
                threading.Thread(target=delete_transcription_later,args=(uid_key,sent.message_id),daemon=True).start()
                action_usage[f"{call.message.chat.id}|{sent.message_id}|clean_up"]=0
                action_usage[f"{call.message.chat.id}|{sent.message_id}|get_key_points"]=0
        except: logging.exception("clean_up")
...
def webhook_root():
    ...
    if request.method=="POST":
        ...
        if content_type and content_type.startswith("application/json"):
            ...
            except: logging.exception("webhook update")
            return "",200
    return abort(403)
...
def set_webhook_on_startup():
    for idx,bot_obj in enumerate(bots):
        try:
            ...
            logging.info("Main bot webhook set to %s",url)
        except Exception as e: logging.error("set webhook startup: %s",e)
...
if __name__=="__main__":
    try:
        ...
        try: client.admin.command('ping'); logging.info("Connected to MongoDB!")
        except Exception as e: logging.error("Could not connect to MongoDB: %s",e)
    except: logging.exception("startup")
