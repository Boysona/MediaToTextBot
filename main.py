...
def start_processing(bot_obj, chat_id, reply_to_message_id=None):
    status = bot_obj.send_message(chat_id, "🔄 Processing...", reply_to_message_id=reply_to_message_id)
    stop = {"stop": False}
    t = threading.Thread(target=animate_processing_message, args=(bot_obj, chat_id, status.message_id, lambda: stop["stop"]))
    t.start()
    return status, stop, t

def finish_processing(bot_obj, chat_id, status_msg, stop, thread, keep=False):
    stop["stop"] = True
    thread.join()
    if not keep:
        try:
...
def post_transcription_setup(bot_obj, chat_id, sent_msg, corrected_text, reply_to_message_id=None):
    markup = InlineKeyboardMarkup()
    markup.add(InlineKeyboardButton("⭐️Clean transcript", callback_data=f"clean_up|{chat_id}|{sent_msg.message_id}"))
    if len(corrected_text) > 1000:
        markup.add(InlineKeyboardButton("Get Summarize", callback_data=f"get_key_points|{chat_id}|{sent_msg.message_id}"))
    try:
...
def handle_media_common(message, bot_obj, bot_token, bot_index=0):
    ...
    processing_msg, stop_animation, animation_thread = start_processing(bot_obj, message.chat.id, message.message_id)
    try:
        ...
        try:
            ...
            if len(corrected_text) > 4000:
                if user_mode == "file":
                    ...
                    sent = bot_obj.send_document(message.chat.id, fobj, reply_to_message_id=message.message_id)
                    post_transcription_setup(bot_obj, message.chat.id, sent, corrected_text, message.message_id)
                else:
                    ...
                    post_transcription_setup(bot_obj, message.chat.id, last_sent, corrected_text, message.message_id)
            else:
                ...
                post_transcription_setup(bot_obj, message.chat.id, sent_msg, corrected_text, message.message_id)
        ...
        finish_processing(bot_obj, message.chat.id, processing_msg, stop_animation, animation_thread)
        try:
            bot_obj.delete_message(message.chat.id, processing_msg.message_id)
        except Exception:
            pass
...
def register_handlers(bot_obj, bot_token, bot_index):
    ...
    def get_key_points_callback(call):
        try:
            ...
            status_msg, stop_animation, animation_thread = start_processing(bot_obj, call.message.chat.id, call.message.message_id)
            try:
                ...
                summary = ""
            finish_processing(bot_obj, call.message.chat.id, status_msg, stop_animation, animation_thread, keep=True)
            if not summary:
                try:
