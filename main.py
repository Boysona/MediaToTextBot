...
# MARK: - User-Configurable Variables
# Fadlan halkan ka beddel AssemblyAI API Keyga marka aad u baahato
...
def upload_large_file(token):
    ...
    chat_id=data.get("chat_id"); lang=data.get("lang","en"); bot_index=int(data.get("bot_index",0))
    ...
    def bytes_to_tempfile(b):
        tmp=tempfile.NamedTemporaryFile(delete=False, suffix=".upload"); tmp.write(b); tmp.flush(); tmp.close(); return tmp.name
    def process_uploaded_file(chat_id_inner, lang_inner, path, bot_index_inner):
        try:
            ...
            try:
                ...
                return
            corrected_text=normalize_text_offline(text); sent_msg=None
            try:
                markup=InlineKeyboardMarkup(); uid_key=str(chat_id_inner); user_mode=get_user_send_mode(uid_key)
                if len(corrected_text)>4000:
                    if user_mode=="file":
                        fobj=io.BytesIO(corrected_text.encode("utf-8")); fobj.name="transcription.txt"; sent_msg=bot_to_use.send_document(chat_id_inner, fobj, reply_markup=markup)
                    else:
                ...
                try:
                    buttons=[]; buttons.append(InlineKeyboardButton("⭐Clean transcript", callback_data=f"clean_up|{chat_id_inner}|{sent_msg.message_id}"))
                ...
                return
            try:
                uid_key=str(chat_id_inner); user_transcriptions.setdefault(uid_key, {})[sent_msg.message_id]=corrected_text
            ...
            try: action_usage[f"{chat_id_inner}|{sent_msg.message_id}|clean_up"]=0; action_usage[f"{chat_id_inner}|{sent_msg.message_id}|get_key_points"]=0
            except Exception: pass
        finally:
            try: os.remove(path)
            except Exception: pass
...
def register_handlers(bot_obj, bot_token, bot_index):
    ...
    @check_and_send_subscription_message
    def start_handler(message, bot_obj):
        try:
    ...
    @check_and_send_subscription_message_callback
    def start_select_lang_callback(call, bot_obj):
        try:
    ...
    @check_and_send_subscription_message
    def handle_help(message, bot_obj):
        try:
            update_user_activity(message.from_user.id)
    ...
    @check_and_send_subscription_message
    def handle_lang(message, bot_obj):
        try:
    ...
    @check_and_send_subscription_message
    def handle_mode(message, bot_obj):
        try:
    ...
    def handle_admin(message):
        try:
            ...
            report_text = (
                ...
                f"📢 Required Channel: <code>{REQUIRED_CHANNEL}</code>\n"
            )
            bot_obj.send_message(message.chat.id, report_text, parse_mode='HTML')
        except Exception:
    ...
    @check_and_send_subscription_message_callback
    def on_stt_language_select(call, bot_obj):
        try:
    ...
    @check_and_send_subscription_message_callback
    def on_result_mode_select(call, bot_obj):
        try:
