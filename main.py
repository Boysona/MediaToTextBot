...
def register_handlers(bot_obj,bot_token,bot_index):
    ...
    def get_key_points_callback(call):
        try:
            ...
            # **SIXOOSHA BADHANKA:** Ka saar badhamada fariintii hore ee user-ku riixay
            try:
                bot_obj.edit_message_reply_markup(call.message.chat.id,call.message.message_id,reply_markup=InlineKeyboardMarkup())
            except: pass
            
            if not summary:
                try: 
                    # Tani waa natiijadii cusbeyd, waa inaan badhamada lagu darin
                ...
                try: 
                    ...
                    # Haddii edit-ku guuldareysto, dir fariin cusub.
                    try: bot_obj.delete_message(call.message.chat.id,status_msg.message_id)
                    except: pass
                    bot_obj.send_message(call.message.chat.id,f"{summary}",reply_to_message_id=call.message.message_id)
        ...
        try:
            ...
            if len(cleaned)>4000:
                if user_mode=="file":
                    ...
                    # MA JIRTO 'attach_action_buttons' Halkan
                ...
                try:
                    # Halkan waxaan ku daray 'reply_markup=InlineKeyboardMarkup()' si aan u xaqiijiyo in badhamada cusub aysan soo noqon.
                    bot_obj.edit_message_text(f"{cleaned}",chat_id=call.message.chat.id,message_id=status_msg.message_id,reply_markup=InlineKeyboardMarkup())
                    sent_msg=status_msg
                except: 
                    # Haddii edit-ku guuldareysto, dir fariin cusub.
                    try: bot_obj.delete_message(call.message.chat.id,status_msg.message_id)
                    except: pass
                    sent_msg=bot_obj.send_message(call.message.chat.id,cleaned,reply_to_message_id=call.message.message_id)
            
            if sent_msg:
                # Save the new cleaned transcription text to the database, laakiin HA DIB U FURA BADHANKA
                ...
                # Ka saar 'attach_action_buttons(bot_obj,call.message.chat.id,sent_msg.message_id,cleaned)'
