...
MAX_WEB_UPLOAD_MB=int(os.environ.get("MAX_WEB_UPLOAD_MB","150"))
...
for part in [p.strip() for p in raw_admins.split(",") if p.strip()]:
    try: ADMIN_USER_IDS.append(int(part))
    except: pass
...
def start_processing(bot_obj,chat_id,reply_to_message_id):
    msg=bot_obj.send_message(chat_id,"🔄 Processing...",reply_to_message_id=reply_to_message_id)
    stop_flag={"stop":False}
    t=threading.Thread(target=animate_processing_message,args=(bot_obj,chat_id,msg.message_id,lambda:stop_flag["stop"]))
    t.start()
    return {"stop_flag":stop_flag,"thread":t,"msg_id":msg.message_id}

def stop_processing(bot_obj,chat_id,processing_handle):
    try:
        processing_handle["stop_flag"]["stop"]=True
        processing_handle["thread"].join()
    except: pass
    try: bot_obj.delete_message(chat_id,processing_handle["msg_id"])
    except: pass
...
def recognize_chunk_file(recognizer,file_path:str,language:str):
    ...
    for attempt in range(1,RECOGNITION_MAX_RETRIES+1):
        try:
            ...
            time.sleep(RECOGNITION_RETRY_WAIT*attempt); continue
...
def make_action_markup(chat_id,msg_id,text_len):
    m=InlineKeyboardMarkup()
    m.add(InlineKeyboardButton("⭐️Clean transcript",callback_data=f"clean_up|{chat_id}|{msg_id}"))
    if text_len>1000: m.add(InlineKeyboardButton("Get Summarize",callback_data=f"get_key_points|{chat_id}|{msg_id}"))
    return m

def replace_with_unavailable(bot_obj,chat_id,message_id):
    m=InlineKeyboardMarkup()
    m.add(InlineKeyboardButton("Unavailable (maybe expired)",callback_data="noop"))
    try: bot_obj.edit_message_reply_markup(chat_id,message_id,reply_markup=m)
    except: pass

def deliver_transcription(bot_obj,chat_id,reply_to_id,text,uid_key):
    user_mode=get_user_send_mode(uid_key)
    if len(text)>4000:
        if user_mode=="file":
            f=io.BytesIO(text.encode("utf-8")); f.name="transcription.txt"
            markup=InlineKeyboardMarkup()
            sent=bot_obj.send_document(chat_id,f,reply_to_message_id=reply_to_id,reply_markup=markup)
            try:
                markup=make_action_markup(chat_id,sent.message_id,len(text))
            ...
            chunks=split_text_into_chunks(text,limit=4096); last_sent=None
            for idx,chunk in enumerate(chunks):
                if idx==0: last_sent=bot_obj.send_message(chat_id,chunk,reply_to_message_id=reply_to_id)
                else: last_sent=bot_obj.send_message(chat_id,chunk)
            try:
                markup=make_action_markup(chat_id,last_sent.message_id,len(text))
                bot_obj.edit_message_reply_markup(chat_id,last_sent.message_id,reply_markup=markup)
            except: pass
            user_transcriptions.setdefault(uid_key,{})[last_sent.message_id]=text
            ...
            return last_sent.message_id
    else:
        markup=make_action_markup(chat_id,reply_to_id,len(text))
        sent_msg=bot_obj.send_message(chat_id,text or "⚠️ Warning Make sure the voice is clear or speaking in the language you Choosed.",reply_to_message_id=reply_to_id,reply_markup=markup)
        try:
            markup=make_action_markup(chat_id,sent_msg.message_id,len(text))
...
def handle_media_common(message,bot_obj,bot_token,bot_index=0):
    ...
    proc_handle=start_processing(bot_obj,message.chat.id,message.message_id)
    try:
        ...
        try:
            ...
            mid=deliver_transcription(bot_obj,message.chat.id,message.message_id,corrected_text,uid_key)
        ...
        stop_processing(bot_obj,message.chat.id,proc_handle)
    ...
    def process_uploaded_file(chat_id_inner,lang_inner,path,bot_index_inner):
        try:
            ...
            try:
                ...
                try:
                    markup=make_action_markup(chat_id_inner,sent_msg.message_id,len(corrected_text))
...
def register_handlers(bot_obj,bot_token,bot_index):
    ...
    def get_key_points_callback(call):
        try:
            ...
            try: replace_with_unavailable(bot_obj,call.message.chat.id,call.message.message_id)
            except: pass
            ...
            stop_flag={"stop":False}
            animation_thread=threading.Thread(target=animate_processing_message,args=(bot_obj,call.message.chat.id,status_msg.message_id,lambda:stop_flag["stop"]))
            ...
            stop_flag["stop"]=True; animation_thread.join()
            if not summary:
...
def delete_webhook_route():
    ...
    for idx,bot_obj in enumerate(bots):
        try:
            bot_obj.delete_webhook(); results.append({"index":idx,"status":"deleted"})
        except Exception as e:
