...
def animate_typing(bot_obj, chat_id, stop_event):
...
def create_action_markup_and_init(chat_id, message_id, text_len):
    markup = InlineKeyboardMarkup()
...
def telegram_send_document_bytes(bot_obj, chat_id, bytes_obj, reply_to=None, reply_markup=None):
    try:
        bytes_obj.seek(0)
    except Exception:
        pass
    return bot_obj.send_document(chat_id, bytes_obj, reply_to_message_id=reply_to, reply_markup=reply_markup)
...
def handle_media_common(message, bot_obj, bot_token, bot_index=0):
    ...
    typing_thread = threading.Thread(target=animate_typing, args=(bot_obj, message.chat.id, lambda: stop_typing["stop"]))
    typing_thread.start()
    try:
        ...
        try:
            ...
            if len(corrected_text) > 4000:
                if user_mode == "file":
                    ...
                    sent = telegram_send_document_bytes(bot_obj, message.chat.id, f, reply_to=message.message_id)
                    try:
                        markup = create_action_markup_and_init(message.chat.id, sent.message_id, len(corrected_text))
                        try: bot_obj.edit_message_reply_markup(message.chat.id, sent.message_id, reply_markup=markup)
                        except Exception: pass
                    except Exception: pass
                    try:
                    ...
                    try:
                        markup = create_action_markup_and_init(message.chat.id, last_sent.message_id, len(corrected_text))
                        try: bot_obj.edit_message_reply_markup(message.chat.id, last_sent.message_id, reply_markup=markup)
                        except Exception: pass
                    except Exception: pass
                    try:
                ...
                try:
                    markup = create_action_markup_and_init(message.chat.id, sent_msg.message_id, len(corrected_text))
                    try: bot_obj.edit_message_reply_markup(message.chat.id, sent_msg.message_id, reply_markup=markup)
                    except Exception: pass
                except Exception: pass
                try:
    ...
    def process_uploaded_file(chat_id_inner, lang_inner, path, bot_index_inner):
        try:
            ...
            try:
                ...
                try:
                    markup = create_action_markup_and_init(chat_id_inner, sent_msg.message_id, len(corrected_text))
                    try: bot_to_use.edit_message_reply_markup(chat_id_inner, sent_msg.message_id, reply_markup=markup)
                    except Exception: pass
                except Exception: pass
            except Exception:
...
def register_handlers(bot_obj, bot_token, bot_index):
    ...
    def get_key_points_callback(call):
        try:
            ...
            stop_typing = {"stop": False}
            typing_thread = threading.Thread(target=animate_typing, args=(bot_obj, call.message.chat.id, lambda: stop_typing["stop"]))
            typing_thread.start()
            try:
            ...
            if not summary:
                ...
                try:
                    sent = bot_obj.send_message(call.message.chat.id, f"{summary}", reply_to_message_id=call.message.message_id)
                    try:
        ...
        try:
            ...
            if len(cleaned) > 4000:
                if user_mode == "file":
                    ...
                    try: sent = telegram_send_document_bytes(bot_obj, call.message.chat.id, f, reply_to=call.message.message_id)
                    except Exception:
                        try: bot_obj.send_message(call.message.chat.id, "Failed to send cleaned file.", reply_to_message_id=call.message.message_id)
                        except Exception: pass
                        return
                    try:
                ...
                try:
                    sent = bot_obj.send_message(call.message.chat.id, f"{cleaned}", reply_to_message_id=call.message.message_id)
                    uid_key = str(chat_id_val)
                    user_transcriptions.setdefault(uid_key, {})[sent.message_id] = cleaned
                    threading.Thread(target=delete_transcription_later, args=(uid_key, sent.message_id), daemon=True).start()
                    action_usage[f"{call.message.chat.id}|{sent.message_id}|clean_up"] = 0
