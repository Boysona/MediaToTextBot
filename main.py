import os, logging, requests, telebot, json, threading, time, io, subprocess, tempfile, glob, math, re, wave
...
LANG_OPTIONS = [
    ...
    ("🇨🇳 中文", "zh"),("🇳🇱 Nederlands", "nl"),("🇸🇪 Svenska", "sv"),("🇳🇴 Norsk", "no"),("🇮🇱 עברית", "he"),
...
for label, code in LANG_OPTIONS:
    STT_LANGUAGES[label.split(" ", 1)[-1]] = {"code": code, "emoji": label.split(" ", 1)[0], "native": label.split(" ", 1)[-1]}
...
def load_settings():
    ...
    try:
        doc = settings_collection.find_one({"_id": "required_channel"})
        if doc and isinstance(doc.get("value"), str) and doc.get("value").strip():
            REQUIRED_CHANNEL = doc.get("value")
    except Exception:
        pass
...
def update_user_activity(user_id:int):
    user_id_str=str(user_id); now=datetime.now()
    users_collection.update_one({"_id":user_id_str},{"$set":{"last_active":now},"$setOnInsert":{"first_seen":now,"stt_conversion_count":0}},upsert=True)

def increment_processing_count(user_id:str, service_type:str):
    users_collection.update_one({"_id":str(user_id)},{"$inc":{f"{service_type}_conversion_count":1}})

def get_stt_user_lang(user_id:str)->str:
    u=users_collection.find_one({"_id":user_id}); return u.get("stt_language") if u and "stt_language" in u else "en"

def set_stt_user_lang(user_id:str, lang_code:str):
    users_collection.update_one({"_id":user_id},{"$set":{"stt_language":lang_code}},upsert=True)

def get_user_send_mode(user_id:str)->str:
    u=users_collection.find_one({"_id":user_id}); return u.get("stt_send_mode") if u and "stt_send_mode" in u else "file"

def set_user_send_mode(user_id:str, mode:str):
    if mode not in ("file","split"): mode="file"
    users_collection.update_one({"_id":user_id},{"$set":{"stt_send_mode":mode}},upsert=True)

def user_has_stt_setting(user_id:str)->bool:
    u=users_collection.find_one({"_id":user_id}); return u is not None and "stt_language" in u
...
def select_speech_model_for_lang(language_code:str): return "universal"

def is_transcoding_like_error(msg:str)->bool:
    if not msg: return False
    m=msg.lower(); checks=["transcoding failed","file does not appear to contain audio","text/html","html document","unsupported media type","could not decode"]
    return any(ch in m for ch in checks)

def build_lang_keyboard(callback_prefix:str, row_width:int=3, message_id:int=None):
    markup=InlineKeyboardMarkup(row_width=row_width); buttons=[]
    for label,code in LANG_OPTIONS:
        cb=f"{callback_prefix}|{code}" if message_id is None else f"{callback_prefix}|{code}|{message_id}"
...
def build_result_mode_keyboard(prefix:str="result_mode"):
    m=InlineKeyboardMarkup(row_width=2); m.add(InlineKeyboardButton("📄 .txt file", callback_data=f"{prefix}|file")); m.add(InlineKeyboardButton("💬 Split messages", callback_data=f"{prefix}|split")); return m

def signed_upload_token(chat_id:int, lang_code:str, bot_index:int=0): return serializer.dumps({"chat_id":chat_id,"lang":lang_code,"bot_index":int(bot_index)})
...
def extract_key_points_offline(text:str, max_points:int=6)->str:
    ...
    freq=Counter(words); scores=[]
    for s in sentences:
        s_words=re.findall(r'\w+',s.lower()); score=sum(freq.get(w,0) for w in s_words); scores.append((score,s))
    scores.sort(key=lambda x:x[0],reverse=True)
    top=sorted(scores[:max_points], key=lambda x: sentences.index(x[1]))
...
def telegram_file_info_and_url(bot_token:str, file_id):
    ...
    resp=requests.get(url, timeout=REQUEST_TIMEOUT_TELEGRAM); resp.raise_for_status(); j=resp.json()
    file_path=j.get("result",{}).get("file_path"); file_url=f"https://api.telegram.org/file/bot{bot_token}/{file_path}"
    class D: pass
    d=D(); d.file_path=file_path; return d, file_url

def convert_to_wav(input_path:str, output_wav_path:str):
    if FFMPEG_BINARY is None: raise RuntimeError("ffmpeg binary not found")
    cmd=[FFMPEG_BINARY,"-y","-i",input_path,"-ar",str(AUDIO_SAMPLE_RATE),"-ac",str(AUDIO_CHANNELS),output_wav_path]
...
def create_prepended_chunk(chunk_path:str, silence_sec:int):
    tmp=tempfile.NamedTemporaryFile(delete=False, suffix=".wav"); tmp.close(); out_path=tmp.name
    try: prepend_silence_to_wav(chunk_path, out_path, silence_sec); return out_path
    except Exception:
...
def recognize_chunk_file(recognizer, file_path:str, language:str):
    last_exc=None; prepended_path=None
    for attempt in range(1, RECOGNITION_MAX_RETRIES+1):
        try:
            try: prepended_path=create_prepended_chunk(file_path, PREPEND_SILENCE_SEC)
            except Exception: prepended_path=None
...
def transcribe_file_with_speech_recognition(input_file_path:str, language_code:str):
    ...
    try:
        ...
        def transcribe_chunk(chunk_path):
            recognizer=sr.Recognizer(); return recognize_chunk_file(recognizer, chunk_path, language_code)
...
def transcribe_with_assemblyai(file_path:str, language_code:str, timeout_seconds:int=REQUEST_TIMEOUT_ASSEMBLY):
    ...
    with open(file_path,"rb") as f:
        try:
            resp=requests.post("https://api.assemblyai.com/v2/upload", headers=headers, data=f, timeout=timeout_seconds); resp.raise_for_status(); j=resp.json()
    ...
    try:
        payload={"audio_url":upload_url, "speaker_diarization": False}
        if language_code: payload["language_code"]=language_code
        if language_code not in ASSEMBLY_LANG_SET:
            payload["model"]=select_speech_model_for_lang(language_code)
        ...
        job=resp.json(); job_id=job.get("id")
        if not job_id: raise RuntimeError("AssemblyAI transcript creation failed")
        poll_url=f"https://api.assemblyai.com/v2/transcript/{job_id}"; start=time.time()
        while True:
            r=requests.get(poll_url, headers=headers, timeout=30); r.raise_for_status(); status_json=r.json(); status=status_json.get("status")
...
def transcribe_via_selected_service(input_path:str, lang_code:str):
    use_assembly = lang_code in ASSEMBLY_LANG_SET and ASSEMBLYAI_API_KEY and ASSEMBLYAI_API_KEY!="3da64715f8304ca3a7c78638c4bfd90c"
    if use_assembly:
        try:
            text, service_model = transcribe_with_assemblyai(input_path, lang_code), "assemblyai"
            if text is None: raise RuntimeError("AssemblyAI returned no text")
            return text, service_model
        except Exception:
            logging.exception("AssemblyAI failed, falling back to speech_recognition")
            try:
                text=transcribe_file_with_speech_recognition(input_path, lang_code); return text, "speech_recognition"
        ...
        try:
            ...
            if ASSEMBLYAI_API_KEY and ASSEMBLYAI_API_KEY!="3da64715f8304ca3a7c78638c4bfd90c":
                try:
                    text=transcribe_with_assemblyai(input_path, lang_code); return text, "assemblyai"
                except Exception as e2: raise RuntimeError("Both speech_recognition and AssemblyAI failed: "+str(e2))
            else: raise RuntimeError("speech_recognition failed and AssemblyAI is not configured or not available for this language.")
...
def split_text_into_chunks(text:str, limit:int=4096):
    ...
    while start<n:
        ...
        if not chunk:
            end=start+limit; chunk=text[start:end].strip()
...
def check_subscription(user_id:int, bot_obj)->bool:
    ...
    try:
        member=bot_obj.get_chat_member(REQUIRED_CHANNEL, user_id); return member.status in ['member','administrator','creator']
    except telebot.apihelper.ApiTelegramException as e:
        if "chat not found" in str(e) or "user not found" in str(e): return False
...
def send_subscription_message(chat_id:int, bot_obj):
    ...
    try:
        ...
        bot_obj.send_message(chat_id, "🔒 Access Locked: You cannot use this bot until you join the Channel.", reply_markup=m)
    except Exception: pass

def check_and_send_subscription_message(func):
    @wraps(func)
    def wrapper(message, bot_obj, *args, **kwargs):
        if message.chat.type=='private' and not check_subscription(message.from_user.id, bot_obj):
            ...
            return
        return func(message, bot_obj, *args, **kwargs)
    return wrapper

def check_and_send_subscription_message_callback(func):
    @wraps(func)
    def wrapper(call, bot_obj, *args, **kwargs):
        if call.message.chat.type=='private' and not check_subscription(call.from_user.id, bot_obj):
            ...
            return
        return func(call, bot_obj, *args, **kwargs)
    return wrapper

def verify_user_in_channel(user_id:int, bot_obj):
    if not REQUIRED_CHANNEL or not REQUIRED_CHANNEL.strip(): return True
    try:
        ...
        logging.warning("Telegram API exception checking membership: %s", e)
        return None
    except Exception as e:
        logging.exception("Error checking membership")
        return None

def ensure_user_verified(user_id:int, chat_id:int, bot_obj, send_notice=True):
    res = verify_user_in_channel(user_id, bot_obj)
    if res is True:
        return True
    if res is False:
        if send_notice:
...
def handle_media_common(message, bot_obj, bot_token, bot_index=0):
    user_id_str=str(message.from_user.id); chat_id_str=str(message.chat.id); update_user_activity(message.from_user.id)
    file_id=None; file_size=None; filename=None
    if message.voice:
        ...
        mime=getattr(message.document,"mime_type",None); filename=getattr(message.document,"file_name",None) or "file"; ext=safe_extension_from_filename(filename)
        if mime and ("audio" in mime or "video" in mime): file_id=message.document.file_id; file_size=message.document.file_size
        elif ext in ALLOWED_EXTENSIONS: file_id=message.document.file_id; file_size=message.document.file_size
        else:
            try: bot_obj.send_message(message.chat.id, "Sorry, I can only transcribe audio or video files.")
            except Exception: pass
            return

    lang=get_stt_user_lang(user_id_str)
    if file_size and file_size>TELEGRAM_MAX_BYTES:
        token=signed_upload_token(message.chat.id, lang, bot_index); upload_link=f"{WEBHOOK_BASE.rstrip('/')}/upload/{token}"; max_display_mb=TELEGRAM_MAX_BYTES//(1024*1024)
        ...
        try: bot_obj.send_message(message.chat.id, text, disable_web_page_preview=True, parse_mode='HTML', reply_to_message_id=message.message_id)
        except Exception: pass
        return

    processing_msg=bot_obj.send_message(message.chat.id, "🔄 Processing...", reply_to_message_id=message.message_id); processing_msg_id=processing_msg.message_id
    stop_animation={"stop":False}
    def stop_event(): return stop_animation["stop"]
    animation_thread=threading.Thread(target=animate_processing_message, args=(bot_obj, message.chat.id, processing_msg_id, stop_event)); animation_thread.start()
    try:
        ...
        try:
            ...
            try:
                ...
                if "ffmpeg" in error_msg.lower():
                    try: bot_obj.send_message(message.chat.id, "⚠️ Server error: ffmpeg not found or conversion failed. Contact admin @boyso.", reply_to_message_id=message.message_id)
                    except Exception: pass
                elif is_transcoding_like_error(error_msg):
                    try: bot_obj.send_message(message.chat.id, "⚠️ Transcription error: file is not audible. Please send a different file.", reply_to_message_id=message.message_id)
                    except Exception: pass
                else:
                    try: bot_obj.send_message(message.chat.id, f"Error during transcription: {error_msg}", reply_to_message_id=message.message_id)
                    except Exception: pass
                return
            corrected_text=normalize_text_offline(text); uid_key=str(message.chat.id); user_mode=get_user_send_mode(uid_key)
            if len(corrected_text)>4000:
                if user_mode=="file":
                    f=io.BytesIO(corrected_text.encode("utf-8")); f.name="transcription.txt"; markup=InlineKeyboardMarkup(); sent=bot_obj.send_document(message.chat.id, f, reply_to_message_id=message.message_id, reply_markup=markup)
                    try:
                        buttons=[]; buttons.append(InlineKeyboardButton("⭐️Clean transcript", callback_data=f"clean_up|{message.chat.id}|{sent.message_id}"))
                    ...
                    try: user_transcriptions.setdefault(uid_key, {})[sent.message_id]=corrected_text; threading.Thread(target=delete_transcription_later, args=(uid_key, sent.message_id), daemon=True).start()
                    except Exception: pass
                    try: action_usage[f"{message.chat.id}|{sent.message_id}|clean_up"]=0; action_usage[f"{message.chat.id}|{sent.message_id}|get_key_points"]=0
                    except Exception: pass
                else:
                    chunks=split_text_into_chunks(corrected_text, limit=4096); last_sent=None; markup=InlineKeyboardMarkup()
                    ...
                    try:
                        buttons=[]; buttons.append(InlineKeyboardButton("⭐️Clean transcript", callback_data=f"clean_up|{message.chat.id}|{last_sent.message_id}"))
                    ...
                    try: user_transcriptions.setdefault(uid_key, {})[last_sent.message_id]=corrected_text; threading.Thread(target=delete_transcription_later, args=(uid_key, last_sent.message_id), daemon=True).start()
                    except Exception: pass
                    try: action_usage[f"{message.chat.id}|{last_sent.message_id}|clean_up"]=0; action_usage[f"{message.chat.id}|{last_sent.message_id}|get_key_points"]=0
                    except Exception: pass
            else:
                markup=InlineKeyboardMarkup(); sent_msg=bot_obj.send_message(message.chat.id, corrected_text or "No transcription text was returned.", reply_to_message_id=message.message_id, reply_markup=markup)
                try:
                    buttons=[]; buttons.append(InlineKeyboardButton("⭐️Clean transcript", callback_data=f"clean_up|{message.chat.id}|{sent_msg.message_id}"))
                ...
                try: user_transcriptions.setdefault(uid_key, {})[sent_msg.message_id]=corrected_text; threading.Thread(target=delete_transcription_later, args=(uid_key, sent_msg.message_id), daemon=True).start()
                except Exception: pass
                try: action_usage[f"{message.chat.id}|{sent_msg.message_id}|clean_up"]=0; action_usage[f"{message.chat.id}|{sent_msg.message_id}|get_key_points"]=0
                except Exception: pass
        ...
        error_msg=str(e); logging.exception("Error in transcription process")
        ...
        stop_animation["stop"]=True; animation_thread.join()
...
<html>
  <head>
    ...
    <style>
      ...
      input[type="text"], textarea, select { width: 100%; padding: 8px; margin: 6px 0 12px 0; box-sizing: border-box; }
  ...
  <body>
    <div class="box">
      <h2>Admin Panel</h2>
      <p>Required Channel: <code>{{ required_channel }}</code></p>
      <p>Users: {{ users_count }} | Groups: {{ groups_count }}</p>
      <hr>
      <h3>Send Broadcast / Ads</h3>
      <form method="post" action="/admin/send_ads" enctype="multipart/form-data">
        ...
        <label>Message Text (for text or caption)</label>
        <textarea name="message_text" rows="6"></textarea>
        <label>Attach File (optional)</label>
        <input type="file" name="media_file">
        <label>Use Bot Index (0 for first bot)</label>
        <input type="text" name="bot_index" value="0">
        <div style="margin-top:12px;">
          <button type="submit">Send Ads</button>
        </div>
      </form>
      <hr>
      <form method="post" action="/admin/save_settings">
        ...
        <input type="text" name="required_channel" value="{{ required_channel }}">
        <label>Admin Panel Secret</label>
        <input type="text" name="admin_secret" value="{{ admin_secret }}">
        <div style="margin-top:12px;">
...
def broadcast_worker(target_type, msg_type, text, file_path, bot_index):
        ...
        if target_type in ("all","users"):
            for u in users_collection.find({}, {"_id":1}):
                try:
                    ...
                    try:
                        recipients.append(int(str(u["_id"])))
                    except Exception:
                        continue
        if target_type in ("all","groups"):
            for g in groups_collection.find({}, {"_id":1}):
                try:
                    ...
                    try:
                        recipients.append(int(str(g["_id"])))
                    except Exception:
                        continue
    except Exception:
    ...
    for r in recipients:
        try:
            if msg_type == "text":
                bot_to_use.send_message(r, text, parse_mode='HTML')
            elif msg_type == "photo":
                if file_path:
                    with open(file_path, "rb") as f:
                        bot_to_use.send_photo(r, f, caption=text or None)
                else:
                    bot_to_use.send_message(r, text or "Photo not provided.")
            elif msg_type == "video":
                if file_path:
                    with open(file_path, "rb") as f:
                        bot_to_use.send_video(r, f, caption=text or None)
                else:
                    bot_to_use.send_message(r, text or "Video not provided.")
            elif msg_type == "audio":
                if file_path:
                    with open(file_path, "rb") as f:
                        bot_to_use.send_audio(r, f, caption=text or None)
                else:
                    bot_to_use.send_message(r, text or "Audio not provided.")
            elif msg_type == "voice":
                if file_path:
                    with open(file_path, "rb") as f:
                        bot_to_use.send_voice(r, f, caption=text or None)
                else:
                    bot_to_use.send_message(r, text or "Voice not provided.")
            elif msg_type == "document":
                if file_path:
                    with open(file_path, "rb") as f:
                        bot_to_use.send_document(r, f, caption=text or None)
                else:
                    bot_to_use.send_message(r, text or "Document not provided.")
            sent_count += 1
        except Exception as e:
            last_exception = e
...
def admin_ui():
    ...
    try:
        ...
        groups_count = 0
    return render_template_string(ADMIN_HTML, users_count=users_count, groups_count=groups_count, required_channel=REQUIRED_CHANNEL, secret=ADMIN_PANEL_SECRET, admin_secret=ADMIN_PANEL_SECRET)

@app.route("/admin/save_settings", methods=["POST"])
def admin_save_settings():
    global REQUIRED_CHANNEL, ADMIN_PANEL_SECRET
    secret = request.form.get("secret")
    if not secret or secret != ADMIN_PANEL_SECRET:
        return abort(403)
    rc = request.form.get("required_channel")
    new_secret = request.form.get("admin_secret")
    if rc:
        try:
            settings_collection.update_one({"_id":"required_channel"}, {"$set":{"value":rc}}, upsert=True)
            REQUIRED_CHANNEL = rc
        except Exception:
            pass
    if new_secret:
...
def ask_gemini(text:str, instruction:str, timeout=REQUEST_TIMEOUT_GEMINI)->str:
    ...
    resp=requests.post(url, headers=headers, json=payload, timeout=timeout); resp.raise_for_status(); result=resp.json()
    if "candidates" in result and isinstance(result["candidates"], list) and len(result["candidates"])>0:
        cand=result["candidates"][0]
        try: return cand['content']['parts'][0]['text']
        except Exception: return json.dumps(cand)
    return json.dumps(result)

@app.route("/assemblyai", methods=["POST"])
def assemblyai_endpoint():
    lang=request.form.get("language","en"); f=request.files.get("file")
    if not f: return jsonify({"error":"no file provided"}),400
    b=f.read(); tmp=tempfile.NamedTemporaryFile(delete=False, suffix=".upload")
    try:
        tmp.write(b); tmp.flush(); tmp.close()
        try:
            text=transcribe_with_assemblyai(tmp.name, lang); return jsonify({"text":text}),200
        except Exception as e:
            try:
                text=transcribe_file_with_speech_recognition(tmp.name, lang); return jsonify({"text":text,"fallback":"speech_recognition"}),200
...
def web_upload_file(token):
    max_mb = MAX_WEB_UPLOAD_MB
    try:
    ...
    bot_obj = bots[bot_index] if 0 <= bot_index < len(bots) else bots[0]
    user_id = chat_id if chat_id > 0 else 0 # Should be chat_id, use 0 if private chat logic is needed elsewhere

    if request.method == "GET":
        return render_template_string(HTML_UPLOAD, max_mb=max_mb)
    ...
    if request.method == "POST":
        ...
        if ext not in ALLOWED_EXTENSIONS:
             return f"Unsupported file type. Allowed: {', '.join(ALLOWED_EXTENSIONS)}", 400
        ...
        if file_size > file_size_limit:
            return f"File too large. Max size is {max_mb}MB.", 400
        ...
        try:
            ...
            bot_obj.send_message(chat_id, "✅ File received. Processing web upload...", disable_web_page_preview=True)

            def background_transcribe(tmp_path, chat_id, lang_code, bot_obj):
                user_id_str = str(chat_id)
                update_user_activity(user_id)
                processing_msg = bot_obj.send_message(chat_id, "🔄 Processing web upload...", reply_to_message_id=None)
                processing_msg_id = processing_msg.message_id
                
                stop_animation = {"stop": False}
                def stop_event(): return stop_animation["stop"]
                animation_thread = threading.Thread(target=animate_processing_message, args=(bot_obj, chat_id, processing_msg_id, stop_event), daemon=True)
                animation_thread.start()

                try:
                    text, used_service = transcribe_via_selected_service(tmp_path, lang_code)
                    ...
                    if len(corrected_text) > 4000:
                        if user_mode == "file":
                            ...
                            sent = bot_obj.send_document(chat_id, f, reply_markup=markup)
                            
                            buttons = []
                            buttons.append(InlineKeyboardButton("⭐️Clean transcript", callback_data=f"clean_up|{chat_id}|{sent.message_id}"))
                            if len(corrected_text) > 1000: buttons.append(InlineKeyboardButton("Get Summarize", callback_data=f"get_key_points|{chat_id}|{sent.message_id}"))
                            for b in buttons: markup.add(b)
                            bot_obj.edit_message_reply_markup(chat_id, sent.message_id, reply_markup=markup)
                            ...
                            buttons = []
                            buttons.append(InlineKeyboardButton("⭐️Clean transcript", callback_data=f"clean_up|{chat_id}|{last_sent.message_id}"))
                            if len(corrected_text) > 1000: buttons.append(InlineKeyboardButton("Get Summarize", callback_data=f"get_key_points|{chat_id}|{last_sent.message_id}"))
                            for b in buttons: markup.add(b)
                            bot_obj.edit_message_reply_markup(chat_id, last_sent.message_id, reply_markup=markup)
                        ...
                        markup = InlineKeyboardMarkup()
                        sent_msg = bot_obj.send_message(chat_id, corrected_text or "No transcription text was returned.", reply_markup=markup)
                        
                        buttons = []
                        buttons.append(InlineKeyboardButton("⭐️Clean transcript", callback_data=f"clean_up|{chat_id}|{sent_msg.message_id}"))
                        if len(corrected_text) > 1000: buttons.append(InlineKeyboardButton("Get Summarize", callback_data=f"get_key_points|{chat_id}|{sent_msg.message_id}"))
                        for b in buttons: markup.add(b)
                    ...
                    error_msg = str(e)
                    logging.exception("Error in web upload transcription process")
                    if "ffmpeg" in error_msg.lower():
                        bot_obj.send_message(chat_id, "⚠️ Server error: ffmpeg not found or conversion failed.")
                    elif is_transcoding_like_error(error_msg):
                        bot_obj.send_message(chat_id, "⚠️ Transcription error: file is not audible or format is unsupported.")
                    else:
                        bot_obj.send_message(chat_id, f"Error during transcription: {error_msg}")
                finally:
                    stop_animation["stop"] = True
                    animation_thread.join()
                    try: bot_obj.delete_message(chat_id, processing_msg_id)
                    except Exception: pass
                    try: os.remove(tmp_path)
                    except Exception: pass

            threading.Thread(target=background_transcribe, args=(tmp_file.name, chat_id, lang_code, bot_obj), daemon=True).start()

            return "Upload successful. Check your Telegram chat for the transcript.", 200

        except Exception as e:
            try: os.remove(tmp_file.name)
            except Exception: pass
            return f"Server Error during file save/processing: {str(e)}", 500
...
def register_handlers(bot_obj, bot_token, bot_index):
    ...
    def start_select_lang_callback(call):
        try:
            if call.message.chat.type=='private' and not check_subscription(call.from_user.id, bot_obj): send_subscription_message(call.message.chat.id, bot_obj);
            uid=str(call.from_user.id); _,lang_code=call.data.split("|",1); lang_label=CODE_TO_LABEL.get(lang_code, lang_code); set_stt_user_lang(uid, lang_code)
            try: bot_obj.delete_message(call.message.chat.id, call.message.message_id)
            except Exception: pass
            welcome_text=("👋 Salaam!    \n" "• Send me\n" "• voice message\n" "• audio file\n" "• video\n" "• to transcribe for free")
            bot_obj.send_message(call.message.chat.id, welcome_text); bot_obj.answer_callback_query(call.id, f"✅ Language set to {lang_label}")
        except Exception:
    ...
    def handle_lang(message):
        try:
            if message.chat.type=='private' and not check_subscription(message.from_user.id, bot_obj): send_subscription_message(message.chat.id, bot_obj); return
            kb=build_lang_keyboard("stt_lang"); bot_obj.send_message(message.chat.id, "Choose your file language for transcription using the below buttons:", reply_markup=kb)
    ...
    def handle_mode(message):
        try:
            if message.chat.type=='private' and not check_subscription(message.from_user.id, bot_obj): send_subscription_message(message.chat.id, bot_obj); return
            current_mode=get_user_send_mode(str(message.from_user.id)); mode_text="📄 .txt file" if current_mode=="file" else "💬 Split messages"
    ...
    def handle_admin(message):
        try:
            ...
            report_text = (
                ...
                f"🗣️ Total Transcriptions: <b>{total_stt_conversions:,}</b>\n"
                f"🟢 Active Users (Last 7 Days): <b>{active_users_7d}</b>\n\n"
            )
    ...
    def on_stt_language_select(call):
        try:
            if call.message.chat.type=='private' and not check_subscription(call.from_user.id, bot_obj): send_subscription_message(call.message.chat.id, bot_obj);
            uid=str(call.from_user.id); _,lang_code=call.data.split("|",1); lang_label=CODE_TO_LABEL.get(lang_code, lang_code); set_stt_user_lang(uid, lang_code); bot_obj.answer_callback_query(call.id, f"✅ Language set: {lang_label}")
            try: bot_obj.delete_message(call.message.chat.id, call.message.message_id)
            except Exception: pass
        except Exception:
    ...
    def on_result_mode_select(call):
        try:
            if call.message.chat.type=='private' and not check_subscription(call.from_user.id, bot_obj): send_subscription_message(call.message.chat.id, bot_obj);
            uid=str(call.from_user.id); _,mode=call.data.split("|",1); set_user_send_mode(uid, mode); mode_text="📄 .txt file" if mode=="file" else "💬 Split messages"
            try: bot_obj.delete_message(call.message.chat.id, call.message.message_id)
            except Exception: pass
    ...
    def get_key_points_callback(call):
        try:
            if call.message.chat.type=='private' and not check_subscription(call.message.chat.id, bot_obj):
                send_subscription_message(call.message.chat.id, bot_obj)
                try: bot_obj.answer_callback_query(call.id)
                except Exception: pass
                return
            ...
            else: bot_obj.answer_callback_query(call.id,"Invalid request", show_alert=True); return
            try: chat_id_val=int(chat_id_part); msg_id=int(msg_id_part)
            except Exception: bot_obj.answer_callback_query(call.id,"Invalid message id", show_alert=True); return
            usage_key=f"{chat_id_val}|{msg_id}|get_key_points"; usage=action_usage.get(usage_key,0)
            if usage>=2: bot_obj.answer_callback_query(call.id,"⚠️ Get Summarize unavailable (maybe expired)", show_alert=True); return
            action_usage[usage_key]=usage+1; uid_key=str(chat_id_val); stored=user_transcriptions.get(uid_key,{}).get(msg_id)
            if not stored: bot_obj.answer_callback_query(call.id,"⚠️ Get Summarize unavailable (maybe expired)", show_alert=True); return
            bot_obj.answer_callback_query(call.id,"Generating..."); status_msg=bot_obj.send_message(call.message.chat.id,"🔄 Processing...", reply_to_message_id=call.message.message_id)
            stop_animation={"stop":False}
            def stop_event(): return stop_animation["stop"]
            animation_thread=threading.Thread(target=animate_processing_message, args=(bot_obj, call.message.chat.id, status_msg.message_id, stop_event)); animation_thread.start()
            try:
                lang=get_stt_user_lang(str(chat_id_val)) or "en"; instruction=f"Summarize this text (lang={lang}) without adding any introductions, notes, or extra phrases."
    ...
    def clean_up_callback(call):
        try:
            ...
            if usage>=2: bot_obj.answer_callback_query(call.id,"⚠️ Clean up unavailable (maybe expired)", show_alert=True); return
            action_usage[usage+1]; uid_key=str(chat_id_val); stored=user_transcriptions.get(uid_key,{}).get(msg_id)
            if not stored: bot_obj.answer_callback_query(call.id,"⚠️ Clean up unavailable (maybe expired)", show_alert=True); return
            bot_obj.answer_callback_query(call.id,"Cleaning up..."); status_msg=bot_obj.send_message(call.message.chat.id,"🔄 Processing...", reply_to_message_id=call.message.message_id)
            stop_animation={"stop":False}
            def stop_event(): return stop_animation["stop"]
            animation_thread=threading.Thread(target=animate_processing_message, args=(bot_obj, call.message.chat.id, status_msg.message_id, stop_event)); animation_thread.start()
            try:
            ...
            if len(cleaned)>4000:
                if user_mode=="file":
                    ...
                    try: user_transcriptions.setdefault(uid_key, {})[sent.message_id]=cleaned; threading.Thread(target=delete_transcription_later, args=(uid_key, sent.message_id), daemon=True).start()
                    except Exception: pass
                    try: action_usage[f"{call.message.chat.id}|{sent.message_id}|clean_up"]=0; action_usage[f"{call.message.chat.id}|{sent.message_id}|get_key_points"]=0
                    except Exception: pass
                else:
                    try: bot_obj.delete_message(call.message.chat.id, status_msg.message_id)
                    except Exception: pass
                    ...
                    try: user_transcriptions.setdefault(uid_key, {})[last_sent.message_id]=cleaned; threading.Thread(target=delete_transcription_later, args=(uid_key, last_sent.message_id), daemon=True).start()
                    except Exception: pass
                    try: action_usage[f"{call.message.chat.id}|{last_sent.message_id}|clean_up"]=0; action_usage[f"{call.message.chat.id}|{last_sent.message_id}|get_key_points"]=0
                    except Exception: pass
            else:
                try:
                    bot_obj.edit_message_text(f"{cleaned}", chat_id=call.message.chat.id, message_id=status_msg.message_id)
                    uid_key=str(chat_id_val); user_transcriptions.setdefault(uid_key, {})[status_msg.message_id]=cleaned
                    threading.Thread(target=delete_transcription_later, args=(uid_key, status_msg.message_id), daemon=True).start()
                    action_usage[f"{call.message.chat.id}|{status_msg.message_id}|clean_up"]=0; action_usage[f"{call.message.chat.id}|{status_msg.message_id}|get_key_points"]=0
                except Exception: pass
...
def webhook_root():
    ...
    if request.method=="POST":
        ...
        if content_type and content_type.startswith("application/json"):
            ...
            try:
                update=telebot.types.Update.de_json(raw); bots[bot_index_val].process_new_updates([update])
...
def set_webhook_route():
    ...
    for idx,bot_obj in enumerate(bots):
        try:
            url=WEBHOOK_BASE.rstrip("/")+f"/?bot_index={idx}"; bot_obj.delete_webhook(); time.sleep(0.2); bot_obj.set_webhook(url=url); results.append({"index":idx,"url":url,"status":"ok"})
        except Exception as e: logging.error(f"Failed to set webhook for bot {idx}: {e}"); results.append({"index":idx,"error":str(e)})
...
def delete_webhook_route():
    ...
    for idx,bot_obj in enumerate(bots):
        ...
        except Exception as e: logging.error(f"Failed to delete webhook for bot {idx}: {e}"); results.append({"index":idx,"error":str(e)})
    return jsonify({"results":results}),200
...
def set_webhook_on_startup():
    for idx,bot_obj in enumerate(bots):
        try:
            ...
            url=WEBHOOK_BASE.rstrip("/")+f"/?bot_index={idx}"; bot_obj.set_webhook(url=url); logging.info(f"Main bot webhook set successfully to {url}")
        except Exception as e: logging.error(f"Failed to set main bot webhook on startup: {e}")

def set_bot_info_and_startup(): set_webhook_on_startup()
