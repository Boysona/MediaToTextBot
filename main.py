...
def process_media_file(message,bot_obj,bot_token,bot_index,file_id,file_size,filename):
    ...
    try:
        ...
        try:
            ...
            try: text,used_service=transcribe_via_selected_service(tmpf_name,lang)
            except Exception as e:
...
def worker_thread():
    while True:
        try:
            ...
            else: transcript_semaphore.release()
        except: logging.exception("Error in worker thread")
        finally:
...
def handle_media_common(message,bot_obj,bot_token,bot_index=0):
    ...
    if len(PENDING_QUEUE)>0:
        bot_obj.send_message(message.chat.id,f"⏳ Your request has been added to the queue. Current queue length: {len(PENDING_QUEUE)}.",reply_to_message_id=message.message_id)
