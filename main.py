...
BOT_TOKENS = ["7770743573:AAEhVz2jKwdJjhrm3XH0bAhhLucg2H4AvMY", "7790991731:AAF4NHGm0BJCf08JTdBaUWKzwfs82_Y9Ecw"]
...
def load_settings():
    ...
    try:
        ...
        if doc and isinstance(doc.get("value"), str) and doc.get("value").strip(): REQUIRED_CHANNEL = doc.get("value")
...
def check_subscription(user_id: int, bot_obj) -> bool:
    ...
    try: return bot_obj.get_chat_member(REQUIRED_CHANNEL, user_id).status in ['member', 'administrator', 'creator']
    except (telebot.apihelper.ApiTelegramException, Exception): return False
