...
ASSEMBLYAI_SPEECH_MODEL = os.environ.get("ASSEMBLYAI_SPEECH_MODEL", "large") 
...
def get_bot_statistics():
    ...
    agg = list(users_collection.aggregate([{"$group":{"_id":None,"total_files":{"$sum":"$stt_conversion_count"}}}]))
    files_processed = agg[0]["total_files"] if agg else 0
    now = datetime.utcnow()
    since_day = now - timedelta(days=1)
    since_week = now - timedelta(days=7)
    since_month = now - timedelta(days=30)
    active_day = users_collection.count_documents({"last_active": {"$gte": since_day}})
    active_week = users_collection.count_documents({"last_active": {"$gte": since_week}})
    active_month = users_collection.count_documents({"last_active": {"$gte": since_month}})
...
def register_handlers(bot_obj, bot_token, bot_index):
    ...
    @bot_obj.message_handler(commands=['admin', 'status'])
    def admin_status_handler(message):
        try:
            ...
            text = (
                ...
                f"Active users last 30 days: <b>{stats['active_last_month']}</b>\n"
                f"Active users last 7 days: <b>{stats['active_last_week']}</b>\n"
                f"Active users last 1 day: <b>{stats['active_last_day']}</b>\n\n"
                f"AssemblyAI model: <code>{ASSEMBLYAI_SPEECH_MODEL}</code>"
            )
            bot_obj.send_message(message.chat.id, text, parse_mode='HTML', reply_to_message_id=message.message_id)
        except Exception:
            logging.exception("Error in admin_status_handler")
