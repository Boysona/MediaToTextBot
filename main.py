...
def send_chat_action(bot_obj,chat_id,action='typing'):
    try: bot_obj.send_chat_action(chat_id,action)
    except: pass

def animate_processing_message(bot_obj,chat_id,message_id,stop_event):
    while not stop_event():
        try: send_chat_action(bot_obj,chat_id)
        except: pass
        time.sleep(5)
...
def file_process_thread(file_data,bot_obj,bot_token,bot_index):
    global processing_count
    message=file_data["message"];filename=file_data["filename"];file_id=file_data["file_id"];file_size=file_data["file_size"];lang=file_data["lang"];user_id_str=file_data["user_id_str"];chat_id_str=file_data["chat_id_str"];message_id=message.message_id

    processing_msg_id=None;stop_animation={"stop":False};animation_thread=None

    try:
        processing_msg=bot_obj.send_message(message.chat.id,"Processing started...",reply_to_message_id=message.message_id)
...
def check_queue():
    global processing_count
    with processing_lock:
        while processing_count<MAX_CONCURRENT_PROCESSING and processing_queue:
            file_data=processing_queue.pop(0)
            processing_count+=1
            threading.Thread(target=file_process_thread,args=(file_data,file_data["bot_obj"],file_data["bot_token"],file_data["bot_index"]),daemon=True).start()
    ...
    def process_uploaded_file(chat_id_inner,lang_inner,path,bot_index_inner):
        try:
            ...
            try:
                ...
                threading.Thread(target=delete_transcription_later,args=(uid_key,sent_msg.message_id),daemon=True).start();increment_processing_count(str(chat_id_inner),"stt")
            except: pass
        finally:
            try: os.remove(path)
            except: pass
...
def register_handlers(bot_obj,bot_token,bot_index):
    ...
    def get_key_points_callback(call):
        try:
            ...
            if not stored:
                bot_obj.answer_callback_query(call.id,"Get Summarize unavailable (maybe expired)",show_alert=True);return
            bot_obj.answer_callback_query(call.id,"Generating...");send_chat_action(bot_obj,call.message.chat.id,'typing')
            status_msg=bot_obj.send_message(call.message.chat.id,"Generating summary...",reply_to_message_id=call.message.message_id)
    ...
    def clean_up_callback(call):
        try:
            ...
            if not stored:
                bot_obj.answer_callback_query(call.id,"Clean up unavailable (maybe expired)",show_alert=True);return
            bot_obj.answer_callback_query(call.id,"Cleaning up...");send_chat_action(bot_obj,call.message.chat.id,'typing')
            status_msg=bot_obj.send_message(call.message.chat.id,"Cleaning up transcription...",reply_to_message_id=call.message.message_id)
            stop_animation={"stop":False};animation_thread=threading.Thread(target=animate_processing_message,args=(bot_obj,call.message.chat.id,status_msg.message_id,lambda:stop_animation["stop"]));animation_thread.start()
            try:
                lang=get_stt_user_lang(str(chat_id_val)) or "en"
