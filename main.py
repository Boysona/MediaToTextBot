...
def build_action_buttons(chat_id: int, message_id: int, transcription_text: str):
    markup = InlineKeyboardMarkup()
    buttons = [InlineKeyboardButton("⭐️Clean transcript", callback_data=f"clean_up|{chat_id}|{message_id}")]
    if len(transcription_text) > 1000: buttons.append(InlineKeyboardButton("Get Summarize", callback_data=f"get_key_points|{chat_id}|{message_id}"))
...
def get_processing_message_text(frame_index: int) -> str:
...
def animate_processing_message(bot_obj, chat_id, message_id, stop_event):
    ...
    while not stop_event():
        try: bot_obj.edit_message_text(get_processing_message_text(idx), chat_id=chat_id, message_id=message_id)
...
def handle_media_common(message, bot_obj, bot_token, bot_index=0):
    ...
    if file_size and file_size > TELEGRAM_MAX_BYTES:
        ...
        return
    processing_msg = bot_obj.send_message(message.chat.id, get_processing_message_text(0), reply_to_message_id=message.message_id)
    ...
    try:
        ...
        try:
            ...
            if sent_msg:
                try:
                    markup = build_action_buttons(message.chat.id, sent_msg.message_id, corrected_text)
    ...
    def process_uploaded_file(chat_id_inner, lang_inner, path, bot_index_inner):
        try:
            ...
            try:
                ...
                if sent_msg:
                    markup = build_action_buttons(chat_id_inner, sent_msg.message_id, corrected_text)
...
def register_handlers(bot_obj, bot_token, bot_index):
    ...
    def get_key_points_callback(call):
        try:
            ...
            status_msg = bot_obj.send_message(call.message.chat.id, get_processing_message_text(0), reply_to_message_id=call.message.message_id)
            stop_animation = {"stop": False}
            ...
            try:
                ...
                logging.exception("Error during Gemini API call for summary")
                stop_animation["stop"] = True
                animation_thread.join()
    ...
    def clean_up_callback(call):
        try:
            ...
            try:
                ...
                logging.exception("Error during Gemini API call for clean up")
                stop_animation["stop"] = True
                animation_thread.join()
                try: bot_obj.delete_message(call.message.chat.id, status_msg.message_id)
                except Exception: pass
