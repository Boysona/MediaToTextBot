...
ASSEMBLYAI_MODEL = os.environ.get("ASSEMBLYAI_MODEL", "default")

BOT_TOKENS = [
    "7770743573:AAEhVz2jKwdJjhrm3XH0bAhhLucg2H4AvMY",
    "7790991731:AAF4NHGm0BJCf08JTdBaUWKzwfs82_Y9Ecw",
]
GEMINI_API_KEY = os.environ.get("GEMINI_API_KEY", "AIzaSyBRWP4BaPLPYdB5_E3C3TVDGqiHrjv4vQ")
...
in_memory_data = {"pending_media": {}, "admin_pending": {}}
action_usage = {}
...
def update_user_activity(user_id: int, username: str = None, full_name: str = None):
    ...
    set_on_insert = {"first_seen": now, "stt_conversion_count": 0}
    if username is not None:
        update["$set"]["username"] = username
    if full_name is not None:
        update["$set"]["full_name"] = full_name
    users_collection.update_one({"_id": user_id_str}, {**update, "$setOnInsert": set_on_insert}, upsert=True)
...
def transcribe_with_assemblyai(file_path: str, language_code: str, timeout_seconds: int = REQUEST_TIMEOUT_ASSEMBLY):
    ...
    try:
        ...
        if ASSEMBLYAI_MODEL:
            payload["model"] = ASSEMBLYAI_MODEL
...
def handle_media_common(message, bot_obj, bot_token, bot_index=0):
    ...
    update_user_activity(message.from_user.id, getattr(message.from_user, "username", None), getattr(message.from_user, "first_name", None))
    file_id = None
    file_size = None
    filename = None
    if message.voice:
...
def register_handlers(bot_obj, bot_token, bot_index):
    ...
    @bot_obj.callback_query_handler(lambda c: c.data and (c.data.startswith("admin_stats|") or c.data.startswith("admin_logout|")))
    def admin_callback_handler(call):
        try:
            ...
            if not is_admin_in_db(caller_id):
                bot_obj.answer_callback_query(call.id, "🔒 You are not an admin", show_alert=True)
                return
            if action == "admin_logout":
                admins_collection.delete_one({"_id": str(caller_id)})
                bot_obj.answer_callback_query(call.id, "🔓 Logged out")
                try:
                    bot_obj.edit_message_text("You have been logged out from admin panel.", chat_id=call.message.chat.id, message_id=call.message.message_id)
                except Exception:
                    pass
                return
            if action == "admin_stats":
                try:
                    ...
                    recent_users_cursor = users_collection.find({"last_active": {"$gte": seven_days_ago}, "stt_conversion_count": {"$gt": 0}}).sort([("last_active", -1)])
                    recent_usernames = []
                    for u in recent_users_cursor:
                        uname = u.get("username") or u.get("full_name") or u["_id"]
                        recent_usernames.append(uname)
                        if len(recent_usernames) >= 50:
                            break
                    recent_list_text = "\n".join(f"- {x}" for x in recent_usernames) if recent_usernames else "No recent users found"
                    stats_text = (
                        f"📊 Admin Panel Stats\n\n"
                        f"Total users: {total_users}\n"
                        f"Files processed: {total_files}\n"
                        f"Uptime: {uptime_str}\n\n"
                        f"Recent users (last 7 days) who processed files:\n{recent_list_text}"
                    )
                    bot_obj.answer_callback_query(call.id)
                    try:
                        bot_obj.edit_message_text(stats_text, chat_id=call.message.chat.id, message_id=call.message.message_id)
                    except Exception:
                        bot_obj.send_message(call.message.chat.id, stats_text)
                except Exception as e:
                    logging.exception("Error generating admin stats")
                    try:
                        bot_obj.answer_callback_query(call.id, "Error generating stats", show_alert=True)
                    except Exception:
                        pass
        except Exception:
            logging.exception("Error in admin_callback_handler")
    ...
    def handle_text_messages(message):
        try:
            ...
            with memory_lock:
                pending_time = in_memory_data["admin_pending"].get(uid)
            if pending_time:
                try:
                    ans = message.text.strip()
                    with memory_lock:
                        in_memory_data["admin_pending"].pop(uid, None)
                    if ans == ADMIN_PASSWORD:
                        admins_collection.update_one({"_id": uid}, {"$set": {"username": getattr(message.from_user, "username", None), "full_name": getattr(message.from_user, "first_name", None), "granted_at": datetime.utcnow()}}, upsert=True)
                        markup = InlineKeyboardMarkup()
                        markup.add(InlineKeyboardButton("📊 Show Stats", callback_data=f"admin_stats|{uid}"))
                        markup.add(InlineKeyboardButton("🔒 Logout", callback_data=f"admin_logout|{uid}"))
                        bot_obj.send_message(message.chat.id, "✅ Admin access granted. Use the buttons below to view stats or logout.", reply_markup=markup)
                    else:
                        bot_obj.send_message(message.chat.id, "❌ Incorrect admin password.")
                except Exception:
                    logging.exception("Error handling admin password")
                return
            if message.text and message.text.strip().lower() == "/admin":
                try:
                    if not check_subscription(message.from_user.id, bot_obj):
                        send_subscription_message(message.chat.id, bot_obj)
                        return
                    with memory_lock:
                        in_memory_data["admin_pending"][uid] = time.time()
                    bot_obj.send_message(message.chat.id, "🔐 Send the admin password to access the admin panel.")
                except Exception:
                    logging.exception("Error initiating admin flow")
                return
