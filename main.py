...
def make_action_markup(chat_id: int, msg_id: int, text_len: int):
    ...
    if text_len and text_len > 1000: m.add(InlineKeyboardButton("Get Summarize", callback_data=f"get_key_points|{chat_id}|{msg_id}"))
    return m
    ...
    try:
        ...
        try:
            ...
            if len(corrected_text) > 4000:
                if user_mode == "file":
                    f = io.BytesIO(corrected_text.encode("utf-8")); f.name = "transcription.txt"; markup = make_action_markup(message.chat.id, 0, len(corrected_text))
                    sent = bot_obj.send_document(message.chat.id, f, reply_to_message_id=message.message_id, reply_markup=markup)
                    try:
                        markup = make_action_markup(message.chat.id, sent.message_id, len(corrected_text))
                    ...
                    try:
                        markup = make_action_markup(message.chat.id, last_sent.message_id, len(corrected_text))
                ...
                markup = make_action_markup(message.chat.id, 0, len(corrected_text))
                ...
                try:
                    markup = make_action_markup(message.chat.id, sent_msg.message_id, len(corrected_text))
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    bot_obj.answer_callback_query(call.id, "The API is not working, please wait until this issue is resolved", show_alert=True)
                    stop_animation["stop"] = True; animation_thread.join()
