import os,logging,requests,telebot,json,threading,time,io,tempfile,glob,re
...
def with_chat_action(bot_obj,chat_id,action,fn,*a,**kw):
    ...
    def ping():
        while not stop.is_set():
            ...
            time.sleep(4)
    t = threading.Thread(target=ping); t.daemon=True; t.start()
    try:
        return fn(*a,**kw)
    finally:
        stop.set(); t.join()
...
def make_action_markup(chat_id:int,msg_id:int,text_len:int):
    m = InlineKeyboardMarkup()
    btns = [InlineKeyboardButton("⭐️Clean transcript",callback_data=f"clean_up|{chat_id}|{msg_id}")]
    if text_len>1000: btns.append(InlineKeyboardButton("Get Summarize",callback_data=f"get_key_points|{chat_id}|{msg_id}"))
    for i in range(0,len(btns),2): m.add(*btns[i:i+2]) if i+1<len(btns) else m.add(btns[i])
    return m

def deliver_text_result(bot_obj,chat_id,reply_to_message_id,text,uid_key):
    user_mode = get_user_send_mode(uid_key)
    if len(text)>4000:
        if user_mode=="file":
            f = io.BytesIO(text.encode("utf-8")); f.name="transcription.txt"
            sent = bot_obj.send_document(chat_id,f,reply_to_message_id=reply_to_message_id,reply_markup=InlineKeyboardMarkup())
            try:
                markup = make_action_markup(chat_id,sent.message_id,len(text)); bot_obj.edit_message_reply_markup(chat_id,sent.message_id,reply_markup=markup)
            except Exception: pass
            try:
            ...
            for idx,chunk in enumerate(chunks):
                if idx==0: last_sent = bot_obj.send_message(chat_id,chunk,reply_to_message_id=reply_to_message_id)
                else: last_sent = bot_obj.send_message(chat_id,chunk)
            try:
                markup = make_action_markup(chat_id,last_sent.message_id,len(text)); bot_obj.edit_message_reply_markup(chat_id,last_sent.message_id,reply_markup=markup)
            except Exception: pass
            try:
        ...
        sent_msg = bot_obj.send_message(chat_id, text or "⚠️ Warning Make sure the voice is clear or speaking in the language you Choosed.", reply_to_message_id=reply_to_message_id, reply_markup=InlineKeyboardMarkup())
        try:
            markup = make_action_markup(chat_id,sent_msg.message_id,len(text)); bot_obj.edit_message_reply_markup(chat_id,sent_msg.message_id,reply_markup=markup)
        except Exception: pass
        try:
...
def upload_large_file(token):
    ...
    def process_uploaded_file(chat_id_inner,lang_inner,path,bot_index_inner):
        try:
            ...
            try:
                text,used = with_chat_action(bot_to_use,chat_id_inner,'typing',transcribe_via_selected_service,path,lang_inner)
            except Exception:
            ...
            try:
                ...
                deliver_text_result(bot_to_use,chat_id_inner,None,corrected_text,uid_key)
            except Exception:
...
def register_handlers(bot_obj,bot_token,bot_index):
    ...
    def handle_admin(message):
        try:
            ...
            markup = InlineKeyboardMarkup(); markup.add(InlineKeyboardButton("Open Admin Panel",url=link))
    ...
    def get_key_points_callback(call):
        try:
            ...
            try:
                summary = with_chat_action(bot_obj,call.message.chat.id,'typing',lambda s: ask_gemini(s,f"What is this report about? What are the most important points? Please summarize them for me into (lang={get_stt_user_lang(uid_key) or 'en'}) without adding any introductions, notes, or extra phrases."), stored)
            except Exception:
            ...
            if not summary:
                ...
                try: bot_obj.send_message(call.message.chat.id,summary,reply_to_message_id=call.message.message_id)
                except Exception: pass
    ...
    def clean_up_callback(call):
        try:
            ...
            try:
                def make_clean(s):
                    instruction = f"Clean and normalize this transcription (lang={get_stt_user_lang(uid_key) or 'en'}). Remove ASR artifacts like [inaudible], repeated words, filler noises, timestamps, and incorrect punctuation. Produce a clean, well-punctuated, readable text in the same language. Do not add introductions or explanations."
                    try: return ask_gemini(s,instruction)
                    except Exception: return normalize_text_offline(s)
                cleaned = with_chat_action(bot_obj,call.message.chat.id,'typing',make_clean,stored)
            except Exception:
                cleaned = ""
            if not cleaned:
            ...
            if len(cleaned) > 4000:
                ...
                try:
                    bot_obj.send_message(call.message.chat.id,cleaned,reply_to_message_id=call.message.message_id)
                    uid_key = str(chat_id_val)
                    user_transcriptions.setdefault(uid_key,{})[call.message.message_id] = cleaned
                    threading.Thread(target=delete_transcription_later,args=(uid_key,call.message.message_id),daemon=True).start()
...
def handle_media_common(message,bot_obj,bot_token,bot_index=0):
    ...
    try:
        ...
        try:
            ...
            try:
                text,used_service = with_chat_action(bot_obj,message.chat.id,'typing',transcribe_via_selected_service,tmpf.name,lang)
            except Exception as e:
            ...
            deliver_text_result(bot_obj,message.chat.id,message.message_id,corrected_text,uid_key)
