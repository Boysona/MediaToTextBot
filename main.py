...
def cleanup_old_transcriptions():
    while True:
        try:
            # Halkan ayaa laga beddelay in la isticmaalo CLEANUP_DAYS
...
def register_handlers(bot_obj,bot_token,bot_index):
    ...
    def get_key_points_callback(call):
        try:
            ...
            if not transcript_doc or not transcript_doc.get("original_text"):
                bot_obj.answer_callback_query(call.id,"Get Summarize unavailable (maybe expired/removed)",show_alert=True);return
            stored=transcript_doc["original_text"]
            
            if usage>=1:
                # MARK: Halkan waxaa laga joojiyay badhamada ficillada si aysan u soo noqon
                try: bot_obj.edit_message_reply_markup(call.message.chat.id, call.message.message_id, reply_markup=None)
                except: pass
                bot_obj.answer_callback_query(call.id,"Get Summarize unavailable (maybe expired/used)",show_alert=True);return
            ...
            if not summary:
                ...
                try: 
                    # MARK: Halkan waxaa la dirayaa fariin cusub. Waxaan isticmaalaynaa reply_markup=None si aan u hubinno inaysan badhamadu ku lifaaqnaan
                    bot_obj.edit_message_text(f"{summary}",chat_id=call.message.chat.id,message_id=status_msg.message_id,reply_markup=None)
                except: pass
    ...
    def clean_up_callback(call):
        try:
            ...
            transcript_doc=transcriptions_collection.find_one({"chat_id":str(chat_id_val),"message_id":msg_id})
            if not transcript_doc or not transcript_doc.get("original_text"):
                bot_obj.answer_callback_query(call.id,"Clean up unavailable (maybe expired/removed)",show_alert=True);return
            stored=transcript_doc["original_text"]
            ...
            if usage>=1:
                ...
                bot_obj.answer_callback_query(call.id,"Clean up unavailable (maybe expired/used)",show_alert=True);return
            action_usage[usage_key]=usage+1
            ...
            if len(cleaned)>4000:
                if user_mode=="file":
                    ...
                    # MARK: Waxaan hubineynaa inaanan badhamada ku lifaaqin dokumentiga cusub
                    sent_msg=bot_obj.send_document(call.message.chat.id,f,reply_to_message_id=call.message.message_id,reply_markup=None)
                else:
                    try: bot_obj.delete_message(call.message.chat.id,status_msg.message_id)
                    except: pass
                    ...
                    for idx,chunk in enumerate(chunks):
                        if idx==0: 
                            # MARK: Waxaan hubineynaa inaanan badhamada ku lifaaqin fariinta cusub
                            last_sent=bot_obj.send_message(call.message.chat.id,chunk,reply_to_message_id=call.message.message_id,reply_markup=None)
                        else: 
                            # MARK: Waxaan hubineynaa inaanan badhamada ku lifaaqin fariinta cusub
                            last_sent=bot_obj.send_message(call.message.chat.id,chunk,reply_markup=None)
                    sent_msg=last_sent
            else:
                try:
                    # MARK: Waxaan hubineynaa inaanan badhamada ku lifaaqin fariinta cusub ee la bedelay
                    bot_obj.edit_message_text(f"{cleaned}",chat_id=call.message.chat.id,message_id=status_msg.message_id,reply_markup=None)
                    sent_msg=status_msg
                except: pass
            ...
            if sent_msg:
                ...
                # MARK: Xarriiqda 'attach_action_buttons' halkan waa la tirtiray si badhamada aan dib loo gelin.
