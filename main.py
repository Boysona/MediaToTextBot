...
def build_action_markup(chat_id, message_id, include_summarize):
    markup = InlineKeyboardMarkup()
    buttons = [InlineKeyboardButton("⭐️Clean transcript", callback_data=f"clean_up|{chat_id}|{message_id}")]
    if include_summarize: buttons.append(InlineKeyboardButton("Get Summarize", callback_data=f"get_key_points|{chat_id}|{message_id}"))
        ...
        try:
            ...
            if len(corrected_text) > 4000:
                if user_mode == "file":
                    ...
                    try:
                        include_summarize = len(corrected_text) > 1000
                        markup = build_action_markup(message.chat.id, sent.message_id, include_summarize)
                    ...
                    try:
                        include_summarize = len(corrected_text) > 1000
                        markup = build_action_markup(message.chat.id, last_sent.message_id, include_summarize)
                ...
                try:
                    include_summarize = len(corrected_text) > 1000
                    markup = build_action_markup(message.chat.id, sent_msg.message_id, include_summarize)
            ...
            try:
                ...
                if len(corrected_text) > 4000:
                    if user_mode == "file":
                        ...
                        try:
                            include_summarize = len(corrected_text) > 1000
                            markup = build_action_markup(chat_id_inner, sent_msg.message_id, include_summarize)
        ...
        try:
            ...
            if len(cleaned) > 4000:
                if user_mode == "file":
                    ...
                    try:
                        include_summarize = len(cleaned) > 1000
                        markup = build_action_markup(call.message.chat.id, sent.message_id, include_summarize)
                        bot_obj.edit_message_reply_markup(call.message.chat.id, sent.message_id, reply_markup=markup)
                    except Exception: pass
                else:
                    try: bot_obj.delete_message(call.message.chat.id, status_msg.message_id)
                    except Exception: pass
                    ...
                    try:
                        include_summarize = len(cleaned) > 1000
                        markup = build_action_markup(call.message.chat.id, last_sent.message_id, include_summarize)
                        bot_obj.edit_message_reply_markup(call.message.chat.id, last_sent.message_id, reply_markup=markup)
                    except Exception: pass
            else:
                try:
                    ...
                    include_summarize = len(cleaned) > 1000
                    markup = build_action_markup(call.message.chat.id, status_msg.message_id, include_summarize)
                    try: bot_obj.edit_message_reply_markup(call.message.chat.id, status_msg.message_id, reply_markup=markup)
                    except Exception: pass
                except Exception: pass
