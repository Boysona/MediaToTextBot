...
ASSEMBLYAI_MODEL_NAME=os.environ.get("ASSEMBLYAI_MODEL_NAME","universal")
...
db=client[DB_NAME];users_collection=db["users"];groups_collection=db["groups"];settings_collection=db["settings"]
...
user_transcriptions={};memory_lock=threading.Lock();in_memory_data={"pending_media":{}};action_usage={}
...
def send_subscription_message(chat_id:int,bot_obj):
    ...
    try:
        markup=InlineKeyboardMarkup();markup.add(InlineKeyboardButton("Click here to join the Group",url=f"https://t.me/{REQUIRED_CHANNEL.lstrip('@')}"))
...
def unsign_upload_token(token:str,max_age_seconds:int=3600): return serializer.loads(token,max_age=max_age_seconds)
...
def start_processing(bot_obj,chat_id,reply_to=None):
    msg=bot_obj.send_message(chat_id,"🔄 Processing...",reply_to_message_id=reply_to) if reply_to else bot_obj.send_message(chat_id,"🔄 Processing...")
    stop={"stop":False}
    t=threading.Thread(target=animate_processing_message,args=(bot_obj,chat_id,msg.message_id,lambda: stop["stop"]))
    t.start()
    return msg.message_id,stop,t

def end_processing_and_delete(bot_obj,chat_id,proc_msg_id,stop,thread):
    stop["stop"]=True;thread.join()
    try: bot_obj.delete_message(chat_id,proc_msg_id)
    except: pass
...
def extract_key_points_offline(text:str,max_points:int=6)->str:
    ...
    freq=Counter(words)
    scores=[(sum(freq.get(w,0) for w in re.findall(r'\w+',s.lower())),s) for s in sentences]
...
def telegram_file_info_and_url(bot_token:str,file_id):
    ...
    return type('D',(),{'file_path':file_path})(),file_url
...
def transcribe_with_assemblyai(file_path:str,language_code:str,timeout_seconds:int=REQUEST_TIMEOUT_ASSEMBLY):
    ...
    with open(file_path,"rb") as f:
        try:
            resp=requests.post("https://api.assemblyai.com/v2/upload",headers=headers,data=f,timeout=timeout_seconds);resp.raise_for_status()
            j=resp.json();upload_url=j.get("upload_url") or j.get("url") or j.get("data") or None
            if not upload_url:
    ...
    try:
        payload={"audio_url":upload_url,"model":ASSEMBLYAI_MODEL_NAME}
        ...
        start=time.time()
        while True:
            r=requests.get(poll_url,headers=headers,timeout=30);r.raise_for_status()
            status_json=r.json();status=status_json.get("status")
...
def attach_actions_and_store(bot_obj,chat_id,message_obj,text):
    uid_key=str(chat_id)
    try:
        user_transcriptions.setdefault(uid_key,{})[message_obj.message_id]=text
        threading.Thread(target=delete_transcription_later,args=(uid_key,message_obj.message_id),daemon=True).start()
    except: pass
    try:
        action_usage[f"{chat_id}|{message_obj.message_id}|clean_up"]=0
        action_usage[f"{chat_id}|{message_obj.message_id}|get_key_points"]=0
    except: pass
    try:
        markup=InlineKeyboardMarkup()
        buttons=[InlineKeyboardButton("⭐️Clean transcript",callback_data=f"clean_up|{chat_id}|{message_obj.message_id}")]
        if len(text)>1000: buttons.append(InlineKeyboardButton("Get Summarize",callback_data=f"get_key_points|{chat_id}|{message_obj.message_id}"))
        for b in buttons: markup.add(b)
        bot_obj.edit_message_reply_markup(chat_id,message_obj.message_id,reply_markup=markup)
    except: pass

def send_transcription_with_actions(bot_obj,chat_id,reply_to_message_id,text):
    uid_key=str(chat_id);user_mode=get_user_send_mode(uid_key)
    if len(text)>4000:
        if user_mode=="file":
            f=io.BytesIO(text.encode("utf-8"));f.name="transcription.txt"
            sent=bot_obj.send_document(chat_id,f,reply_to_message_id=reply_to_message_id)
            attach_actions_and_store(bot_obj,chat_id,sent,text);return sent
        else:
            chunks=split_text_into_chunks(text,limit=4096);last=None
            for i,ch in enumerate(chunks):
                if i==0: last=bot_obj.send_message(chat_id,ch,reply_to_message_id=reply_to_message_id)
                else: last=bot_obj.send_message(chat_id,ch)
            attach_actions_and_store(bot_obj,chat_id,last,text);return last
    else:
        sent=bot_obj.send_message(chat_id,text or "⚠️ Warning Make sure the voice is clear or speaking in the language you Choosed.",reply_to_message_id=reply_to_message_id)
        attach_actions_and_store(bot_obj,chat_id,sent,text);return sent
...
def handle_media_common(message,bot_obj,bot_token,bot_index=0):
    ...
    proc_msg_id,stop,thread=start_processing(bot_obj,message.chat.id,message.message_id)
    try:
        ...
        try:
            ...
            try:
                ...
                err=str(e);logging.exception("Error during transcription")
                if "ffmpeg" in err.lower(): bot_obj.send_message(message.chat.id,"⚠️ Server error: ffmpeg not found or conversion failed. Contact admin @boyso.",reply_to_message_id=message.message_id)
                elif is_transcoding_like_error(err): bot_obj.send_message(message.chat.id,"⚠️ Transcription error: file is not audible. Please send a different file.",reply_to_message_id=message.message_id)
                else: bot_obj.send_message(message.chat.id,f"Error during transcription: {err}",reply_to_message_id=message.message_id)
                return
            corrected_text=normalize_text_offline(text)
            uid_key=str(message.chat.id)
            send_transcription_with_actions(bot_obj,message.chat.id,message.message_id,corrected_text)
            increment_processing_count(user_id_str,"stt")
        finally:
            try: os.remove(tmpf.name)
            except: pass
    except Exception as e:
        err=str(e);logging.exception("Error in transcription process")
        if is_transcoding_like_error(err): bot_obj.send_message(message.chat.id,"⚠️ Transcription error: file is not audible. Please send a different file.",reply_to_message_id=message.message_id)
        else: bot_obj.send_message(message.chat.id,f"Error during transcription: {err}",reply_to_message_id=message.message_id)
    finally:
        end_processing_and_delete(bot_obj,message.chat.id,proc_msg_id,stop,thread)
...
def broadcast_worker(target_type,msg_type,text,file_path,bot_index):
    ...
    logging.info("Broadcast finished, sent=%s, last_exc=%s",sent_count,str(last_exc))
...
def upload_large_file(token):
    ...
    def process_uploaded_file(chat_id_inner,lang_inner,path,bot_index_inner):
        try:
            ...
            corrected_text=normalize_text_offline(text)
            try:
                sent_msg=None;uid_key=str(chat_id_inner);user_mode=get_user_send_mode(uid_key)
                if len(corrected_text)>4000:
                    if user_mode=="file":
                        fobj=io.BytesIO(corrected_text.encode("utf-8"));fobj.name="transcription.txt";sent_msg=bot_to_use.send_document(chat_id_inner,fobj)
                    else:
                        chunks=split_text_into_chunks(corrected_text,limit=4096);last=None
                        for idx,chunk in enumerate(chunks):
                            if idx==0: last=bot_to_use.send_message(chat_id_inner,chunk)
                            else: last=bot_to_use.send_message(chat_id_inner,chunk)
                        sent_msg=last
                else: sent_msg=bot_to_use.send_message(chat_id_inner,corrected_text or "No transcription text was returned.")
                attach_actions_and_store(bot_to_use,chat_id_inner,sent_msg,corrected_text)
            except:
            ...
            try:
                action_usage[f"{chat_id_inner}|{sent_msg.message_id}|clean_up"]=0
                action_usage[f"{chat_id_inner}|{sent_msg.message_id}|get_key_points"]=0
            except: pass
        finally:
            try: os.remove(path)
            except: pass
...
def register_handlers(bot_obj,bot_token,bot_index):
    ...
    def start_select_lang_callback(call):
        try:
            ...
            lang_label=CODE_TO_LABEL.get(lang_code,lang_code);set_stt_user_lang(uid,lang_code)
    ...
    def handle_admin(message):
        try:
            ...
            link=f"{WEBHOOK_BASE.rstrip('/')}/admin?secret={ADMIN_PANEL_SECRET}";markup=InlineKeyboardMarkup();markup.add(InlineKeyboardButton("Open Admin Panel",url=link))
    ...
    def get_key_points_callback(call):
        try:
            ...
            proc_msg_id,stop,thread=start_processing(bot_obj,call.message.chat.id,call.message.message_id)
            try:
            ...
            stop["stop"]=True;thread.join()
            if not summary:
                try: bot_obj.edit_message_text("No Summary returned.",chat_id=call.message.chat.id,message_id=proc_msg_id)
                except: pass
            else:
                try: bot_obj.edit_message_text(f"{summary}",chat_id=call.message.chat.id,message_id=proc_msg_id)
                except: pass
    ...
    def clean_up_callback(call):
        try:
            ...
            try:
                lang=get_stt_user_lang(str(chat_id_val)) or "en"
                instruction=f"Clean and normalize this transcription (lang={lang}). Perform speaker diarization and preserve speaker turns. If speaker names are mentioned in the audio or can be confidently inferred, label the turns using those names. If names are not available, label speakers as Speaker 1, Speaker 2, etc. Prepend each turn with the speaker label followed by a colon. Remove ASR artifacts like [inaudible], repeated words, filler noises, timestamps, and incorrect punctuation. Produce a clean, well-punctuated, readable transcript in the same language without adding introductions or extra commentary."
                try: cleaned=ask_gemini(stored,instruction)
                except: cleaned=normalize_text_offline(stored)
            except: cleaned=""
            stop["stop"]=True;thread.join()
            if not cleaned:
                try: bot_obj.edit_message_text("No cleaned text returned.",chat_id=call.message.chat.id,message_id=proc_msg_id)
                except: pass
                return
            ...
            if len(cleaned)>4000:
                if user_mode=="file":
                    ...
                    try: bot_obj.delete_message(call.message.chat.id,proc_msg_id)
                    except: pass
                ...
                try:
                    bot_obj.edit_message_text(f"{cleaned}",chat_id=call.message.chat.id,message_id=proc_msg_id)
                    user_transcriptions.setdefault(uid_key,{})[proc_msg_id]=cleaned
                    threading.Thread(target=delete_transcription_later,args=(uid_key,proc_msg_id),daemon=True).start()
                    action_usage[f"{call.message.chat.id}|{proc_msg_id}|clean_up"]=0
                    action_usage[f"{call.message.chat.id}|{proc_msg_id}|get_key_points"]=0
                except: pass
...
def set_webhook_on_startup():
    for idx,bot_obj in enumerate(bots):
        try:
            bot_obj.delete_webhook();time.sleep(0.2);url=WEBHOOK_BASE.rstrip("/") + f"/?bot_index={idx}";bot_obj.set_webhook(url=url)
