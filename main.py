...
def create_action_markup(chat_id_val: int, msg_id: int, text_len: int):
    markup = InlineKeyboardMarkup()
    btn_clean = InlineKeyboardButton("⭐️Clean transcript", callback_data=f"clean_up|{chat_id_val}|{msg_id}")
    markup.add(btn_clean)
    if text_len > 1000:
        btn_sum = InlineKeyboardButton("Get Summarize", callback_data=f"get_key_points|{chat_id_val}|{msg_id}")
        markup.add(btn_sum)
    return markup
        ...
        try:
            ...
            if len(cleaned) > 4000:
                if user_mode == "file":
                    ...
                    try: bot_obj.send_message(call.message.chat.id, "Here is the cleaned transcript:", reply_to_message_id=call.message.message_id)
                    except Exception: pass
