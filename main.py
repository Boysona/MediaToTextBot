...
def attach_action_buttons(bot_obj,chat_id,message_id,text):
    ...
    try:
        # Hadda labada ficilba waxay noqonayaan 0 marka hore
...
def register_handlers(bot_obj,bot_token,bot_index):
    ...
    def get_key_points_callback(call):
        try:
            ...
            if usage>=1:
                bot_obj.answer_callback_query(call.id,"Get Summarize unavailable (already used)",show_alert=True);return
            action_usage[usage_key]=1 # Kadhig 1 mar

            # Ka saar badhamadii hore si aan dib loo isticmaalin/loo muujin
            try: bot_obj.edit_message_reply_markup(chat_id_val,msg_id,reply_markup=None)
            except: pass
    ...
    def clean_up_callback(call):
        try:
            ...
            if usage>=1:
                bot_obj.answer_callback_query(call.id,"Clean up unavailable (already used)",show_alert=True);return
            action_usage[usage_key]=1 # Kadhig 1 mar
            ...
            if len(cleaned)>4000:
                if user_mode=="file":
                    ...
                    bot_obj.send_document(call.message.chat.id,f,reply_to_message_id=call.message.message_id)
                else:
                    try: bot_obj.delete_message(call.message.chat.id,status_msg.message_id)
                    except: pass
                    chunks=split_text_into_chunks(cleaned,limit=4096)
                    for idx,chunk in enumerate(chunks):
                        if idx==0: bot_obj.send_message(call.message.chat.id,chunk,reply_to_message_id=call.message.message_id)
                        else: bot_obj.send_message(call.message.chat.id,chunk)
            else:
                try:
                    bot_obj.edit_message_text(f"✨ **Cleaned Transcript**:\n\n{cleaned}",chat_id=call.message.chat.id,message_id=status_msg.message_id)
                except: pass
                
            # Ma jiro wicitaan cusub oo loo sameeyay attach_action_buttons fariintan cusub.
