...
VOSK_MODEL_PATH=os.environ.get("VOSK_MODEL_PATH","./vosk-model-small-en")
...
def load_vosk_model():
    ...
    if vosk_model_instance is not None: return vosk_model_instance
    path=VOSK_MODEL_PATH
    if os.path.exists(path):
        try:
            vosk_model_instance=Model(path)
            return vosk_model_instance
        except Exception as e:
            logging.exception("Failed to load VOSK model: %s",e)
            vosk_model_instance=None
            return None
    else:
        logging.warning("VOSK model path not found: %s", path)
        return None

def recognize_chunk_with_vosk(file_path):
    model=load_vosk_model()
    if model is None: raise RuntimeError("VOSK model unavailable")
    wf=wave.open(file_path,'rb')
    if wf.getnchannels()!=1 or wf.getframerate()!=AUDIO_SAMPLE_RATE:
        wf.close()
        tmp=tempfile.NamedTemporaryFile(delete=False,suffix=".wav");tmp.close()
        convert_to_wav(file_path,tmp.name)
    ...
    try:
        ...
        final_txt=final.get('text','').strip()
        if final_txt: texts.append(final_txt)
    except: pass
    try:
        wf.close()
    except: pass
    if cleanup_tmp:
        try: os.remove(cleanup_tmp)
        except: pass
    return " ".join(texts)
    ...
    for attempt in range(1,RECOGNITION_MAX_RETRIES+1):
        try:
            ...
            if lang_code.startswith("en"):
                try:
                    text=recognize_chunk_with_vosk(use_path)
                    if prepended_path:
