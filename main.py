...
def make_action_markup(chat_id: int, message_id: int, text_len: int):
    markup = InlineKeyboardMarkup()
    btns = [InlineKeyboardButton("⭐️Clean transcript", callback_data=f"clean_up|{chat_id}|{message_id}")]
    if text_len > 1000:
        btns.append(InlineKeyboardButton("Get Summarize", callback_data=f"get_key_points|{chat_id}|{message_id}"))
    for b in btns:
        markup.add(b)
    try:
...
def store_transcription_and_schedule_delete(chat_id: int, message_id: int, text: str):
    uid_key = str(chat_id)
    user_transcriptions.setdefault(uid_key, {})[message_id] = text
    try:
        threading.Thread(target=delete_transcription_later, args=(uid_key, message_id), daemon=True).start()
    except Exception:
        pass

def telegram_send_action_typing(bot_obj, chat_id):
    try:
        bot_obj.send_chat_action(chat_id, 'typing')
    except Exception:
        pass
            ...
            if len(corrected_text) > 4000:
                if user_mode == "file":
                    ...
                    try:
                        ...
                        pass
                    try:
                        store_transcription_and_schedule_delete(message.chat.id, sent.message_id, corrected_text)
                    except Exception:
                        pass
                else:
                    ...
                    try:
                        ...
                        pass
                    try:
                        store_transcription_and_schedule_delete(message.chat.id, last_sent.message_id, corrected_text)
                    except Exception:
                        pass
            else:
                ...
                try:
                    store_transcription_and_schedule_delete(message.chat.id, sent_msg.message_id, corrected_text)
                except Exception:
                    pass
    ...
    def process_uploaded_file(chat_id_inner, lang_inner, path, bot_index_inner):
        try:
            ...
            try:
                ...
                return
            try:
                store_transcription_and_schedule_delete(chat_id_inner, sent_msg.message_id, corrected_text)
...
def register_handlers(bot_obj, bot_token, bot_index):
    ...
    def get_key_points_callback(call):
        try:
            ...
            telegram_send_action_typing(bot_obj, call.message.chat.id)
            try:
            ...
            if not summary:
                try:
                    bot_obj.send_message(call.message.chat.id, "No Summary returned.")
                except Exception:
                    pass
            else:
                try:
                    bot_obj.send_message(call.message.chat.id, summary)
                except Exception:
                    pass
        except Exception:
    ...
    def clean_up_callback(call):
        try:
            ...
            if not cleaned:
                try:
                    bot_obj.send_message(call.message.chat.id, "No cleaned text returned.")
                except Exception:
                    pass
                return
            uid_key = str(chat_id_val)
            ...
            if len(cleaned) > 4000:
                if user_mode == "file":
                    ...
                    try:
                        sent = None
                        if sent:
                            store_transcription_and_schedule_delete(uid_key, sent.message_id, cleaned)
