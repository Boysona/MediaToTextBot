...
def unsign_upload_token(token: str):
...
def prepare_action_markup(chat_id:int, message_id:int, text_len:int):
    markup = InlineKeyboardMarkup()
    b1 = InlineKeyboardButton("⭐️Clean transcript", callback_data=f"clean_up|{chat_id}|{message_id}")
    markup.add(b1)
    if text_len > 1000:
        b2 = InlineKeyboardButton("Get Summarize", callback_data=f"get_key_points|{chat_id}|{message_id}")
        markup.add(b2)
    action_usage[f"{chat_id}|{message_id}|clean_up"] = 0
    action_usage[f"{chat_id}|{message_id}|get_key_points"] = 0
    return markup

def send_transcription_message(bot_obj, chat_id, text, reply_to_id=None):
    uid = str(chat_id)
    user_mode = get_user_send_mode(uid)
    if len(text) > 4000:
        if user_mode == "file":
            f = io.BytesIO(text.encode("utf-8"))
            f.name = "transcription.txt"
            sent = bot_obj.send_document(chat_id, f, reply_to_message_id=reply_to_id)
            markup = prepare_action_markup(chat_id, sent.message_id, len(text))
            try: bot_obj.edit_message_reply_markup(chat_id, sent.message_id, reply_markup=markup)
            except Exception: pass
        else:
            chunks = split_text_into_chunks(text, limit=4096)
            last = None
            for idx, chunk in enumerate(chunks):
                if idx == 0: last = bot_obj.send_message(chat_id, chunk, reply_to_message_id=reply_to_id)
                else: last = bot_obj.send_message(chat_id, chunk)
            markup = prepare_action_markup(chat_id, last.message_id, len(text))
            try: bot_obj.edit_message_reply_markup(chat_id, last.message_id, reply_markup=markup)
            except Exception: pass
            sent = last
    else:
        sent = bot_obj.send_message(chat_id, text or "⚠️ Warning Make sure the voice is clear or speaking in the language you Choosed.", reply_to_message_id=reply_to_id)
        markup = prepare_action_markup(chat_id, sent.message_id, len(text))
        try: bot_obj.edit_message_reply_markup(chat_id, sent.message_id, reply_markup=markup)
        except Exception: pass
    try:
        user_transcriptions.setdefault(str(chat_id), {})[sent.message_id] = text
        threading.Thread(target=delete_transcription_later, args=(str(chat_id), sent.message_id), daemon=True).start()
    except Exception: pass
    return sent
...
def handle_media_common(message, bot_obj, bot_token, bot_index=0):
    ...
    try:
        ...
        try:
            ...
            send_transcription_message(bot_obj, message.chat.id, corrected_text, reply_to_id=message.message_id)
...
def upload_large_file(token):
    try: data = unsign_upload_token(token)
    ...
    def process_uploaded_file(chat_id_inner, lang_inner, path, bot_index_inner):
        try:
            ...
            corrected_text = normalize_text_offline(text)
            try:
                send_transcription_message(bot_to_use, chat_id_inner, corrected_text)
            except Exception:
...
def register_handlers(bot_obj, bot_token, bot_index):
    ...
    def clean_up_callback(call):
        ...
        try:
            ...
            send_transcription_message(bot_obj, call.message.chat.id, cleaned, reply_to_id=call.message.message_id)
        except Exception: pass
